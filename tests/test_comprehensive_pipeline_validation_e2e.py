...
class ComprehensivePipelineValidator:
    ...
    def _create_test_documents(self) -> List[Document]:
        ...
        documents = [
            ...
            Document(
                ...
                """,
                metadata={"source": "test_infectious_disease.txt", "category": "infectious_disease"}
            ),
            Document(
                id="doc5",
                page_content="""
    ...
    def validate_pipeline(self, pipeline_class, pipeline_name: str) -> PipelineMetrics:
        ...
        try:
            ...
            logger.info(f"✅ {pipeline_name} initialized in {metrics.initialization_time:.3f}s")
        ...
        try:
            ...
            logger.info(f"✅ {pipeline_name} ingested {metrics.documents_ingested} docs in {metrics.ingestion_time:.3f}s")
        except Exception as e:
        ...
        for query in self.test_queries:
            try:
                ...
                assert "retrieved_documents" in result, "Must return retrieved_documents"
                ...
                logger.info(f"✅ {pipeline_name} query '{query[:30]}...': {docs_retrieved} docs, {answer_length} chars, {query_time:.3f}s")
        ...
        if metrics.initialization_success and metrics.document_ingestion_success and metrics.query_execution_success:
            metrics.overall_status = "WORKING"
        elif metrics.initialization_success and (metrics.document_ingestion_success or metrics.query_execution_success):
    ...
    def validate_all_pipelines(self) -> Dict[str, PipelineMetrics]:
        ...
        logger.info(f"📊 Testing {len(pipelines_to_test)} pipelines with {len(self.test_documents)} documents")
        
        for pipeline_class, pipeline_name in pipelines_to_test:
            self.results[pipeline_name] = self.validate_pipeline(pipeline_class, pipeline_name)
            
        return self.results
    
    def generate_validation_report(self) -> Dict[str, Any]:
        """Generate comprehensive validation report."""
        working_pipelines = [name for name, metrics in self.results.items() if metrics.overall_status == "WORKING"]
        partial_pipelines = [name for name, metrics in self.results.items() if metrics.overall_status == "PARTIAL"]
        broken_pipelines = [name for name, metrics in self.results.items() if metrics.overall_status == "BROKEN"]
        ...
        if working_pipelines:
            ...
            avg_init_time = sum(m.initialization_time for m in working_metrics) / len(working_metrics)
            avg_ingestion_time = sum(m.ingestion_time for m in working_metrics) / len(working_metrics)
            avg_query_time = sum(m.query_time for m in working_metrics) / len(working_metrics)
            avg_docs_retrieved = sum(m.documents_retrieved for m in working_metrics) / len(working_metrics)
        else:
...
def test_basic_rag_pipeline():
    ...
    assert metrics.initialization_success, f"BasicRAG initialization failed: {metrics.initialization_error}"
    assert metrics.document_ingestion_success, f"BasicRAG ingestion failed: {metrics.ingestion_error}"
    assert metrics.query_execution_success, f"BasicRAG query failed: {metrics.query_error}"
    assert metrics.overall_status == "WORKING", f"BasicRAG not working: {metrics.overall_status}"
...
def test_crag_pipeline():
    ...
    assert metrics.initialization_success, f"CRAG initialization failed: {metrics.initialization_error}"
    assert metrics.document_ingestion_success, f"CRAG ingestion failed: {metrics.ingestion_error}"
    assert metrics.query_execution_success, f"CRAG query failed: {metrics.query_error}"
    assert metrics.overall_status == "WORKING", f"CRAG not working: {metrics.overall_status}"


def test_basic_rag_reranking_pipeline():
    """Test BasicRAGReranking pipeline end-to-end."""
    validator = ComprehensivePipelineValidator()
    metrics = validator.validate_pipeline(BasicRAGRerankingPipeline, "BasicRAGReranking")
    
    assert metrics.initialization_success, f"BasicRAGReranking initialization failed: {metrics.initialization_error}"
    assert metrics.document_ingestion_success, f"BasicRAGReranking ingestion failed: {metrics.ingestion_error}"
    assert metrics.query_execution_success, f"BasicRAGReranking query failed: {metrics.query_error}"
    assert metrics.overall_status == "WORKING", f"BasicRAGReranking not working: {metrics.overall_status}"
...
def test_graph_rag_pipeline():
    ...
    assert metrics.initialization_success, f"GraphRAG initialization failed: {metrics.initialization_error}"
    assert metrics.document_ingestion_success, f"GraphRAG ingestion failed: {metrics.ingestion_error}"
    assert metrics.query_execution_success, f"GraphRAG query failed: {metrics.query_error}"
    assert metrics.overall_status == "WORKING", f"GraphRAG not working: {metrics.overall_status}"
...
def test_comprehensive_validation():
    ...
    assert len(report['summary']['working_pipeline_names']) > 0, "No working pipelines found!"
