...
TEST_E2E_DOC_DIR = os.path.join(os.path.dirname(__file__), '..', 'test_data', 'e2e_docs') # Adjusted path for new location
...
def hyde_e2e_db_connection():
    ...
    # Adjust path for new location of test file
    current_test_dir = os.path.dirname(__file__)
    test_data_dir_for_hyde = os.path.abspath(os.path.join(current_test_dir, '..', '..', 'tests', 'test_data', 'e2e_docs'))

    if not os.path.exists(test_data_dir_for_hyde):
        os.makedirs(test_data_dir_for_hyde)
        logger.info(f"Created test_data directory for HyDE: {test_data_dir_for_hyde}")

    doc_a_path = os.path.join(test_data_dir_for_hyde, "DOCA.xml")
    doc_b_path = os.path.join(test_data_dir_for_hyde, "DOCB.xml")
    ...
    logger.info(f"HyDE E2E: Ingesting E2E test documents from {test_data_dir_for_hyde}")
    e2e_embedding_func = get_embedding_func() # Use real embedding function
    ingestion_stats = process_and_load_documents(
        pmc_directory=test_data_dir_for_hyde,
        connection=conn,
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Adjust path for new location of test file
        current_test_dir_main = os.path.dirname(__file__) # This will be tests/experimental/hyde
        test_data_dir_main = os.path.abspath(os.path.join(current_test_dir_main, '..', '..', 'tests', 'test_data', 'e2e_docs'))


        if not os.path.exists(test_data_dir_main): os.makedirs(test_data_dir_main)
        doc_a_path_main = os.path.join(test_data_dir_main, "DOCA.xml")
        doc_b_path_main = os.path.join(test_data_dir_main, "DOCB.xml")
        if not os.path.exists(doc_a_path_main):
            with open(doc_a_path_main, "w") as f: f.write('<article><front><article-meta><article-id pub-id-type="pmc">DOCA</article-id><title-group><article-title>Mitochondrial DNA</article-title></title-group></article-meta></front><body><p>Mitochondrial DNA is crucial for cellular respiration.</p></body></article>')
        if not os.path.exists(doc_b_path_main):
            with open(doc_b_path_main, "w") as f: f.write('<article><front><article-meta><article-id pub-id-type="pmc">DOCB</article-id><title-group><article-title>CRISPR Gene Editing</article-title></title-group></article-meta></front><body><p>CRISPR allows for precise gene editing.</p></body></article>')
        ...
        ingestion_stats_direct = process_and_load_documents(
            pmc_directory=test_data_dir_main,
