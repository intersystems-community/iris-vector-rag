...
class TestHybridIFindRetrievalPaths:
    ...
    def test_ifind_working_path(self, pipeline, mock_connection_manager):
        ...
        with patch.object(pipeline.embedding_manager, 'embed_text') as mock_embed:
            ...
            # Mock vector store for vector search (HybridIFind now uses vector store)
            from iris_rag.core.models import Document
            mock_vector_docs = [
                (Document(id="doc1", page_content="Content 1", metadata={}), 0.9),
                (Document(id="doc2", page_content="Content 2", metadata={}), 0.8)
            ]
            
            with patch.object(pipeline.vector_store, 'similarity_search_with_score', return_value=mock_vector_docs):
                # Mock IFind search results (only need one fetchall call now)
                cursor.fetchall.side_effect = [
                    # IFind search results (working) (doc_id, text_content, score)  
                    [
                        ("doc1", "Content 1", 0.85),
                        ("doc3", "Content 3", 0.75)
                    ]
    ...
    def test_ifind_fallback_to_like_search(self, pipeline, mock_connection_manager):
        ...
        with patch.object(pipeline.embedding_manager, 'embed_text') as mock_embed:
            ...
            # Mock vector store for vector search
            from iris_rag.core.models import Document
            mock_vector_docs = [
                (Document(id="doc1", page_content="Content 1", metadata={}), 0.9),
                (Document(id="doc2", page_content="Content 2", metadata={}), 0.8)
            ...
            with patch.object(pipeline.vector_store, 'similarity_search_with_score', return_value=mock_vector_docs):
                ...
                def execute_side_effect(sql, params=None):
                    ...
                    # For LIKE search, allow normal execution
                    return None
                
                def fetchall_side_effect():
                    # Check the last execute call to determine which search this is
                    last_call = cursor.execute.call_args_list[-1]
                    last_sql = last_call[0][0] if last_call and last_call[0] else ""
                    
                    if "LIKE" in last_sql:
                        # LIKE search results (fallback) (doc_id, text_content, score)
                        return [
                            ("doc1", "Content 1", 1.0),
                            ("doc4", "Content 4", 1.0)
                        ]
                ...
                calls = cursor.execute.call_args_list
                # Should have failed IFind, then LIKE search
                assert len(calls) >= 2
    ...
    def test_result_fusion(self, pipeline, mock_connection_manager):
        ...
        with patch.object(pipeline.embedding_manager, 'embed_text') as mock_embed:
            ...
            from iris_rag.core.models import Document
            mock_vector_docs = [
                (Document(id="doc1", page_content="Content 1", metadata={}), 0.9),  # High vector score
                (Document(id="doc2", page_content="Content 2", metadata={}), 0.7),  # Medium vector score
                (Document(id="doc3", page_content="Content 3", metadata={}), 0.5)   # Low vector score
            ]
            
            with patch.object(pipeline.vector_store, 'similarity_search_with_score', return_value=mock_vector_docs):
                # Mock IFind results
                cursor.fetchall.side_effect = [
                    # IFind results (doc_id, text_content, score)
                    [
                        ("doc2", "Content 2", 0.95), # High IFind score
                        ("doc3", "Content 3", 0.8),  # Medium IFind score
                        ("doc4", "Content 4", 0.6)   # IFind only
                    ]
                ]
    ...
    def test_empty_results_handling(self, pipeline, mock_connection_manager):
        ...
        with patch.object(pipeline.embedding_manager, 'embed_text') as mock_embed:
            ...
            with patch.object(pipeline.vector_store, 'similarity_search_with_score', return_value=[]):
                cursor.fetchall.side_effect = [
                    []   # Empty IFind results
                ]
    ...
    def test_score_normalization(self, pipeline, mock_connection_manager):
        ...
        with patch.object(pipeline.embedding_manager, 'embed_text') as mock_embed:
            ...
            # Mock vector store for vector search with varying scores
            from iris_rag.core.models import Document
            mock_vector_docs = [
                (Document(id="doc1", page_content="Content 1", metadata={}), 0.9),
                (Document(id="doc2", page_content="Content 2", metadata={}), 0.5),
                (Document(id="doc3", page_content="Content 3", metadata={}), 0.1)
            ]
            
            with patch.object(pipeline.vector_store, 'similarity_search_with_score', return_value=mock_vector_docs):
                # Mock IFind results with different scale
                cursor.fetchall.side_effect = [
                    # IFind results with different scale (doc_id, text_content, score)
                    [
                        ("doc1", "Content 1", 100.0),
                        ("doc2", "Content 2", 50.0),
                        ("doc4", "Content 4", 10.0)
                    ]
                ]
