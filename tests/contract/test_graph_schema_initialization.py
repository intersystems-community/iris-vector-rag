...
from iris_vector_rag.integrations.ivg import IVG_REQUIRED_TABLES, InitializationResult
...
class TestGraphTableInitialization:
    ...
    def test_graph_tables_created_when_package_installed(self, schema_manager_with_db):
        ...
        with patch.object(
            ...
            assert result.package_detected is True, "Should detect installed package"

            # Validate tables attempted
            expected_tables = list(IVG_REQUIRED_TABLES)
            assert (
                result.tables_attempted == expected_tables
            ), f"Should attempt all IVG core tables: {expected_tables}"

            # Validate all tables created successfully
            assert len(result.tables_created) == len(
                expected_tables
            ), "Should have creation status for all IVG core tables"
            for table in expected_tables:
                assert (
                    table in result.tables_created
                ), f"Missing creation status for {table}"
                assert (
                    result.tables_created[table] is True
                ), f"Table {table} should be created successfully"

            # Validate performance
            assert (
                result.total_time_seconds < 5.0
    ...
    def test_idempotent_table_creation(self, schema_manager_with_db):
        ...
        with patch.object(
            ...
            assert all(
                result2.tables_created.values()
            ), "Second call should succeed (idempotent)"
            assert len(result2.error_messages) == 0, "Second call should have no errors"
    ...
    def test_partial_table_creation_failure(self, schema_manager_with_db):
        ...
        with patch.object(
            ...
            assert (
                result.tables_created["rdf_labels"] is True
            ), "rdf_labels should succeed"
            assert (
                result.tables_created["rdf_props"] is True
            ), "rdf_props should succeed"
            assert (
                result.tables_created["rdf_edges"] is False
            ), "rdf_edges should fail (simulated permission error)"
            assert (
                result.tables_created["kg_NodeEmbeddings_optimized"] is False
            ), "kg_NodeEmbeddings_optimized should fail"

            # Validate error tracking
            assert (
                "rdf_edges" in result.error_messages
                or "kg_NodeEmbeddings_optimized" in result.error_messages
    ...
    def test_initialization_performance(self, schema_manager_with_db):
        ...
        with patch.object(
            ...
            assert (
                abs(result.total_time_seconds - elapsed) < 0.5
    ...
    def test_tables_created_in_dependency_order(self, schema_manager_with_db):
        ...
        with patch.object(
            ...
            # Validate order
            expected_order = list(IVG_REQUIRED_TABLES)
            assert (
                creation_order == expected_order
...
class TestInitializationResultDataStructure:
    ...
    def test_initialization_result_invariants(self):
        ...
        assert set(result.error_messages.keys()).issubset(
            set(result.tables_attempted)
        ), "error_messages keys must be subset of tables_attempted"
        assert result.total_time_seconds >= 0, "total_time_seconds must be non-negative"
