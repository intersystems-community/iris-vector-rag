...
class TestRAGToolSchemas:
    ...
    @pytest.mark.parametrize(
        "tool_name,expected_description_keywords",
        [
...
class TestUtilityToolSchemas:
    ...
    def test_health_check_tool_schema(self):
        ...
        assert (
            "health" in schema["description"].lower()
            or "status" in schema["description"].lower()
        )
    ...
    def test_metrics_tool_schema(self):
        ...
        assert (
            "metric" in schema["description"].lower()
            or "performance" in schema["description"].lower()
        )
...
class TestSchemaValidation:
    ...
    def test_validate_params_accepts_valid_parameters(self):
        ...
        validated = validate_params(
            "rag_basic", {"query": "What is diabetes?", "top_k": 5}
        )
    ...
    def test_validate_params_applies_defaults(self):
        ...
        validated = validate_params("rag_basic", {"query": "test query"})
    ...
    def test_validate_params_rejects_invalid_range(self):
        ...
        with pytest.raises(ValidationError):
            validate_params("rag_basic", {"query": "test", "top_k": 100})  # max is 50
    ...
    def test_validate_params_rejects_invalid_enum(self):
        ...
        with pytest.raises(ValidationError):
            validate_params(
                "rag_graphrag", {"query": "test", "search_method": "invalid_method"}
            )
    ...
    def test_validate_params_rejects_missing_required(self):
        ...
        with pytest.raises(ValidationError):
            validate_params("rag_basic", {"top_k": 5})
