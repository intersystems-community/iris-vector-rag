...
and falls back to vector search when iris_graph_core fails or returns 0 results.
...
class TestHybridFusionContract:
    ...
    def test_hybrid_fusion_executes_successfully(self, graphrag_pipeline):
        ...
        Given: HybridGraphRAG pipeline initialized with iris_graph_core
        ...
        # Verify documents retrieved (may be via fallback if iris_graph_core unavailable)
    ...
    def test_hybrid_fusion_fallback_on_exception(self, graphrag_pipeline, mocker, caplog):
        """
        FR-003: Hybrid fusion MUST fall back when iris_graph_core raises exception.
