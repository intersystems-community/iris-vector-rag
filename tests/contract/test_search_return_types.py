...
def _make_store():
    ...
    store.similarity_search_by_embedding = MagicMock(return_value=[(stub_doc, 0.9)])
...
class TestSearchByText:
    ...
    def test_returns_list_of_documents(self):
        ...
        with patch.object(
            type(store), "_embed_query", return_value=embedding, create=True
        ):
    ...
    def test_passes_filter_through(self):
        ...
        passed_filter = (
            call_args[0][2] if len(call_args[0]) > 2 else call_args[1].get("filter")
        )
        assert (
            passed_filter == filt
