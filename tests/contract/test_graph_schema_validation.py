...
from iris_vector_rag.integrations.ivg import ValidationResult
...
class TestGraphPrerequisiteValidation:
    ...
    def test_prerequisite_validation_all_met(self, schema_manager_with_tables):
        ...
        with patch.object(
            ...
            with patch.object(schema_manager, "_ivg_table_exists", return_value=True):
                ...
                assert (
                    result.package_installed is True
                ), "Should detect package installed"
                assert result.missing_tables == [], "Should have no missing tables"
                assert (
    ...
    def test_prerequisite_validation_package_missing(self, schema_manager):
        ...
        with patch.object(
            schema_manager, "_detect_iris_vector_graph", return_value=False
        ):
    ...
    def test_prerequisite_validation_tables_missing(self, schema_manager_with_db):
        ...
        GIVEN iris-vector-graph is installed
        AND only 2 of 6 graph tables exist (rdf_labels and rdf_props present, others missing)
        WHEN validate_graph_prerequisites() is called
        THEN ValidationResult.is_valid == False
        AND ValidationResult.package_installed == True
        AND ValidationResult.missing_tables == ["nodes", "rdf_edges", "kg_NodeEmbeddings", "kg_NodeEmbeddings_optimized"]
        ...
        with patch.object(
            ...
            with patch.object(
                schema_manager, "_ivg_table_exists", side_effect=mock_table_exists
            ):
                # This will fail initially with AttributeError
                result = schema_manager.validate_graph_prerequisites()

                assert result.is_valid is False, "Should be invalid when tables missing"
                assert (
                    result.package_installed is True
                ), "Should detect package installed"
                assert (
                    "rdf_edges" in result.missing_tables
                ...
                assert (
                    len(result.missing_tables) == 4
                ), "Should have exactly 4 missing tables"
                assert (
                    "rdf_edges" in result.error_message
                ), "Error message should mention rdf_edges"
                assert (
                    "kg_NodeEmbeddings_optimized" in result.error_message
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                if not result.is_valid:
                    ...
                    assert (
                        "rdf_edges" in error_msg
                    ), "Error should list specific missing tables"
                    assert (
                        "kg_NodeEmbeddings_optimized" in error_msg
                    ), "Error should list specific missing tables"
    ...
    def test_clear_error_when_tables_missing(self, schema_manager):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                assert "rdf_edges" in error, "Error should list rdf_edges as missing"
                assert (
                    "kg_NodeEmbeddings_optimized" in error
                ...
                assert (
                    "not installed" not in error.lower()
    ...
    def test_validation_performance(self, schema_manager_with_tables):
        ...
        with patch.object(
            ...
            with patch.object(schema_manager, "_ivg_table_exists", return_value=True):
                ...
                assert result.is_valid is True, "Validation should succeed"
    ...
    def test_validation_is_stateless(self, schema_manager):
        ...
        with patch.object(
            ...
            with patch.object(schema_manager, "_ivg_table_exists", return_value=True):
                ...
                assert (
                    result1.package_installed == result2.package_installed
                ), "Repeated calls should return same package status"
                assert (
                    result1.missing_tables == result2.missing_tables
...
class TestValidationResultDataStructure:
    ...
    def test_validation_result_invariants(self):
        ...
        assert result.missing_tables == [], "missing_tables must be empty when valid"
        assert result.error_message == "", "error_message must be empty when valid"
        ...
        assert result.is_valid is False, "is_valid must be False when tables missing"
        assert (
...
class TestBackwardCompatibility:
    ...
    def test_backward_compatibility_without_package(self, schema_manager):
        ...
        with patch.object(
            ...
            assert (
                result.package_installed is False
