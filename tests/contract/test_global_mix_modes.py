...
def test_check_prerequisites_global_no_kg_raises_named_error():
    ...
    from iris_vector_rag.retrieval.modes import (
...
def test_retrieve_global_empty_index_returns_degraded_result():
    ...
    assert result["metadata"].get(
        "degradation_reason"
    ), "Expected non-empty degradation_reason"
...
def test_retrieve_engine_global_dispatches_to_retrieve_global():
    ...
    with patch.object(
        engine,
        "_retrieve_global",
        return_value={"retrieved_documents": [], "metadata": {}},
    ) as mock_g, patch("iris_vector_rag.retrieval.modes.check_prerequisites"):
...
def test_retrieve_engine_mix_dispatches_to_retrieve_mix():
    ...
    with patch.object(
        ...
        "_retrieve_mix",
        return_value={"retrieved_documents": [], "metadata": {}},
    ) as mock_m, patch("iris_vector_rag.retrieval.modes.check_prerequisites"):
        engine.retrieve(opts)
...
def test_mix_with_weights_fusion_method_is_weighted_score():
    ...
    opts = QueryOptions(
        query="test", retrieval="mix", top_k=3, weights={"relation": 0.6, "vector": 0.4}
    )
...
def test_mix_docs_have_retrieval_source_metadata():
    ...
    from iris_vector_rag.core.query_options import QueryOptions
    from iris_vector_rag.retrieval.engine import RetrievalEngine, _make_doc
