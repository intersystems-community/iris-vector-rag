...
@pytest.fixture(scope="session") 
def e2e_connection_manager(e2e_config_manager: ConfigurationManager) -> ConnectionManager:
    """
...
@pytest.fixture(scope="session")
def e2e_iris_vector_store(e2e_connection_manager: ConnectionManager, e2e_config_manager: ConfigurationManager) -> IRISVectorStore:
    """
...
def e2e_embedding_function():
    ...
    return get_embedding_func(model_name_override='sentence-transformers/all-MiniLM-L6-v2')
...
def e2e_database_cleanup(e2e_connection_manager: ConnectionManager):
    ...
    def _clear_test_data():
        ...
        try:
            ...
            # Clear test documents (be careful not to clear production data)
            cursor.execute("DELETE FROM RAG.SourceDocuments WHERE doc_id LIKE 'e2e_test_%'")
            connection.commit()
...
def _get_biomedical_test_documents() -> List[Document]:
    ...
    return [
        Document(
            ...
            metadata={"source": "e2e_test", "type": "biomedical", "topic": "cardiovascular", "test_type": "e2e"}
        ),
        Document(
            ...
            metadata={"source": "e2e_test", "type": "biomedical", "topic": "diabetes", "test_type": "e2e"}
        ),
        Document(
            ...
            metadata={"source": "e2e_test", "type": "biomedical", "topic": "oncology", "test_type": "e2e"}
        ),
        Document(
            ...
            memantine.""",
            metadata={"source": "e2e_test", "type": "biomedical", "topic": "neurology", "test_type": "e2e"}
        ),
        Document(
            ...
            metadata={"source": "e2e_test", "type": "biomedical", "topic": "respiratory", "test_type": "e2e"}
        )
    ]
...
def e2e_document_validator():
    ...
    class DocumentValidator:
        ...
        def validate_vector_search_results(self, results: List[Any], min_expected: int = 1) -> bool:
