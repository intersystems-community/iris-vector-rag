...
class MockInfrastructure:
    ...
    def create_mock_config_manager(self):
        ...
        mock_config_mgr.get_embedding_config.return_value = {"model": "mock", "dimension": 384}
...
class ComprehensivePipelineValidator:
    ...
    def _create_test_documents(self) -> List[Document]:
        ...
        return [
            ...
            Document(
                ...
                """,
                metadata={"source": "immunology_textbook.txt", "category": "immunology"}
            ),
            Document(
                id="test_doc_3",
                page_content="""
    ...
    def _test_pipeline_mock_mode(self, pipeline_class, pipeline_name: str) -> PipelineValidationResult:
        ...
        try:
            ...
            logger.info(f"✅ {pipeline_name} mock constructor: {result.constructor_time:.3f}s")
        ...
        try:
            ...
            logger.info(f"✅ {pipeline_name} mock ingestion: {result.documents_ingested} docs in {result.ingestion_time:.3f}s")
        except Exception as e:
        ...
        for query in self.test_queries:
            try:
                ...
                assert "retrieved_documents" in query_result, "Must have retrieved_documents"
                ...
                logger.info(f"✅ {pipeline_name} mock query: {retrieved_docs} docs, {answer_length} chars, {query_time:.3f}s")
        ...
        if result.constructor_success and result.ingestion_success and result.query_success:
            result.status = "WORKING" if result.infrastructure_ready else "INFRASTRUCTURE_MISSING"
        elif result.constructor_success and (result.ingestion_success or result.query_success):
    ...
    def _test_pipeline_full_mode(self, pipeline_class, pipeline_name: str) -> PipelineValidationResult:
        ...
        try:
            ...
            result.constructor_time = time.time() - start_time
            result.constructor_success = True
            logger.info(f"✅ {pipeline_name} full constructor: {result.constructor_time:.3f}s")
    ...
    def validate_pipeline(self, pipeline_class, pipeline_name: str) -> PipelineValidationResult:
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        infra_missing = [name for name, r in self.results.items() if r.status == "INFRASTRUCTURE_MISSING"]
        
        infrastructure_ready = any(r.infrastructure_ready for r in self.results.values())
        test_mode = self.results[list(self.results.keys())[0]].test_mode if self.results else "unknown"
        ...
        report = {
            ...
            "infrastructure_assessment": {
                "database_required": all(r.database_required for r in self.results.values()),
                "database_available": any(r.database_available for r in self.results.values()),
                "can_test_full_functionality": infrastructure_ready,
                "recommendation": "Start database infrastructure for full validation" if not infrastructure_ready else "Infrastructure ready for full testing"
            },
...
def test_comprehensive_pipeline_validation():
    ...
    if report['test_mode'] == 'mock':
        ...
        success_count = report['summary']['working'] + report['summary']['infrastructure_missing']
        assert success_count > 0, "No pipelines working even in mock mode!"
        logger.info(f"Mock validation successful: {success_count}/{len(results)} pipelines working logically")
    else:
        # In full mode, we expect WORKING status
        assert report['summary']['working'] > 0, "No pipelines working in full mode!"
        logger.info(f"Full validation successful: {report['summary']['working']}/{len(results)} pipelines working")
