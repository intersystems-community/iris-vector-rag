...
"""
        ...
        with patch("iris_rag.storage.schema_manager.dbapi.connect") as mock_connect:
            ...
            mock_cursor.fetchone.return_value = [
                384
            ]  # Simulate table having correct dimension
            ...
            assert (
                schema_manager1._schema_validation_cache
                is schema_manager2._schema_validation_cache
            )
            assert (
                schema_manager1.__class__._config_loaded
                == schema_manager2.__class__._config_loaded
            )
        ...
        with patch("iris_rag.storage.schema_manager.dbapi.connect") as mock_connect:
            ...
            with patch.object(mock_cursor, "execute") as mock_execute:
                ...
                if create_calls:
                    ...
                    assert (
                        "REFERENCES RAG.SourceDocuments(doc_id)" in create_sql
                    ), "Foreign key must reference doc_id, not id!"

                    # Should NOT reference non-existent 'id' column
                    assert (
                        "REFERENCES RAG.SourceDocuments(id)" not in create_sql
...
"""
        ...
        with patch("iris_rag.storage.schema_manager.dbapi.connect") as mock_connect:
            ...
            with patch.object(mock_cursor, "execute") as mock_execute:
                schema_manager._migrate_source_documents_table(
                    "RAG.SourceDocuments", "basic", 384
                )
                ...
                if create_calls:
                    ...
                    assert (
                        "id VARCHAR" not in create_sql
                        or "id" not in create_sql.split("PRIMARY KEY")[0]
                    )
...
"""
        ...
        with patch("iris_rag.storage.schema_manager.dbapi.connect"):
            ...
            for i in range(100):
                ...
                with patch.object(
                    SchemaManager, "_validate_table_dimension", return_value=True
                ):
...
"""
        ...
        with patch("iris_rag.storage.schema_manager.dbapi.connect"):
            with patch.object(
                ...
                side_effect=mock_load_and_validate,
            ):
                # Create 10 instances
                for i in range(10):
                    config_manager = ConfigurationManager(str(config_path))
