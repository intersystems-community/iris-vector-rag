...
class TestMultiQueryRRFE2E:
    ...
    def test_parameter_variations(self):
        ...
        for num_q in [2, 3, 4, 5]:
            pipeline = create_pipeline(
                "multi_query_rrf", validate_requirements=False, num_queries=num_q
            )
        ...
        for k in [30, 60, 100]:
            pipeline = create_pipeline(
                "multi_query_rrf", validate_requirements=False, rrf_k=k
            )
    ...
    def test_document_metadata_enrichment(self):
        ...
        pipeline = create_pipeline(
            "multi_query_rrf", validate_requirements=False, num_queries=3
        )
