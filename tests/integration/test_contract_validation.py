...
class TestRegistryValidation:
    ...
    def test_registry_initialization_with_validation(self):
        ...
        registry = TechniqueHandlerRegistry(
            strict_mode=False, validate_on_register=True
        )
    ...
    def test_strict_mode_treats_warnings_as_errors(self):
        ...
        registry_normal.register_handler(
            "warning_test", handler_normal, WarningPipeline
        )
        ...
        registry_strict = TechniqueHandlerRegistry(
            strict_mode=True, validate_on_register=True
        )
        ...
        with pytest.raises(ValidationError):
            registry_strict.register_handler(
                "warning_test_strict", handler_strict, WarningPipeline
            )
    ...
    def test_validate_all_handlers(self):
        ...
        errors = [
            v
            for v in results.get("valid_test", [])
...
class TestRealPipelineValidation:
    ...
    @pytest.mark.skip(
        reason="HybridGraphRAG requires iris_vector_graph and IRIS connection"
    )
...
class TestValidationConfiguration:
    ...
    def test_validation_config_exists(self):
        ...
        assert (
            validate_on_register is not None
        ), "validation:validate_on_register config missing"
    ...
    def test_validation_contract_structure(self):
        ...
        assert (
            required_methods is not None
        ), "validation:contract:required_methods missing"
        assert isinstance(required_methods, list), "required_methods should be list"

        required_response_fields = config_manager.get(
            "validation:contract:required_response_fields"
        )
        assert required_response_fields is not None, "required_response_fields missing"
        assert isinstance(
            required_response_fields, list
        ), "required_response_fields should be list"

        required_metadata_fields = config_manager.get(
            "validation:contract:required_metadata_fields"
        )
        assert required_metadata_fields is not None, "required_metadata_fields missing"
        assert isinstance(
            required_metadata_fields, list
        ), "required_metadata_fields should be list"
    ...
    def test_registry_uses_config(self):
        ...
        validate_on_register = config_manager.get(
            "validation:validate_on_register", default=True
        )

        # Create registry with config settings
        registry = TechniqueHandlerRegistry(
            strict_mode=strict_mode, validate_on_register=validate_on_register
        )
