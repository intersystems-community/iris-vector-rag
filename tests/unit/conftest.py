...
import pytest
import iris_vector_rag  # noqa: F401
...
def mock_iris_connection():
    ...
    mock_conn.execute = Mock(
        return_value=Mock(fetchall=Mock(return_value=[]), rowcount=0)
    )
...
def mock_schema_manager():
    ...
    mock_manager.get_table_info = Mock(
        return_value={
            "name": "test_table",
...
def mock_storage_service():
    ...
    mock_service.get_storage_info = Mock(
        return_value={"total_documents": 100, "total_size_bytes": 1024000}
    )
...
def mock_validation_orchestrator():
    ...
    mock_orchestrator.validate = Mock(
...
def mock_pipeline_factory():
    ...
    mock_factory.get_available_pipelines = Mock(
        return_value=["basic", "crag", "graphrag"]
    )
    return mock_factory
...
def unit_test_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "vector_store": {"provider": "iris", "table_name": "test_vectors"},
    }
...
def patch_external_dependencies(monkeypatch):
    ...
    mock_sentence_transformers_module.SentenceTransformer = Mock(
    ...
    try:
        monkeypatch.setattr("openai.OpenAI", Mock(return_value=Mock()))
