...
class MCPBridge:
    ...
    async def invoke_technique(
        ...
        try:
            ...
            if handler is not None:
                ...
                validated_params = self._validate_additional_handler_params(
                    handler, technique, params
                )
                execution = handler.execute(technique, validated_params)
                result = (
                    await execution if inspect.isawaitable(execution) else execution
                )
    ...
    async def health_check(
        self, include_details: bool = False, include_performance_metrics: bool = True
    ) -> Dict[str, Any]:
    ...
    def _validate_additional_handler_params(
        self, handler: Any, technique: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
