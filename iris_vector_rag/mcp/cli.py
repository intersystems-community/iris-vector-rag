    ...
    def stop(self) -> int:
        ...
        try:
            if config.deployment_mode == "standalone":
                ...
                subprocess.run(
                    ["pkill", "-f", "uvicorn.*iris_rag.api.main"], check=False
                )
        ...
        if config.deployment_mode == "standalone":
            ...
            try:
                response = requests.get(
                    f"{config.python_bridge_url}/mcp/health_check", timeout=2
                )
                if response.status_code == 200:
            ...
            if config.transport in ("http", "both"):
                try:
                    response = requests.get(
                        f"http://localhost:{config.http_port}/health", timeout=2
                    )
                    if response.status_code == 200:
    ...
    def health(self) -> int:
        ...
        try:
            ...
            response = requests.get(
                url,
                timeout=5,
                params={"include_details": True, "include_performance_metrics": True},
            )
