...
class PLAIDBuilder:
    def __init__(self, conn, token_dim: int = TOKEN_DIM, index_name: str = _DEFAULT_INDEX):
        self._conn = conn
    ...
    def build(self, n_clusters: Optional[int] = None) -> Dict:
        ...
        logger.info(f"PLAID build: n_tokens={n_tokens}, k={k}, index={self._index_name}")
        ...
        return {"n_clusters": k, "n_tokens": n_tokens, "elapsed_s": round(elapsed, 2), **result}
...
class PLAIDSearcher:
    ...
    def build(self, n_clusters: Optional[int] = None) -> Dict:
        ...
        engine = _get_engine(self._conn)
        engine.plaid_drop(self._index_name)
        result = engine.plaid_build(self._index_name, docs, n_clusters=k, dim=self._token_dim)
    ...
    def _fetch_docs_with_tokens(self) -> List[Dict]:
        ...
        docs_map: Dict[str, List] = {}
        for doc_id, tok_pos, raw in rows:
            v = (
                [float(x) for x in raw.strip("[]").split(",")]
                if isinstance(raw, str)
                else list(raw)
