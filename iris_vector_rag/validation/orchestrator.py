...
class SetupOrchestrator:
    ...
    # Tables whose canonical DDL lives in SchemaManager (doc_id VARCHAR primary key,
    # embedding VECTOR(FLOAT, n), ...). The inline DDL further down predates it and
    # creates a legacy layout (id INTEGER IDENTITY, filename, ...) that the vector
    # store cannot insert into, which broke auto_setup on a fresh IRIS.
    _SCHEMA_MANAGER_TABLES = (
        "SourceDocuments",
        "DocumentChunks",
        "Entities",
        "EntityRelationships",
    ...
    def _create_table(self, cursor, table_name: str) -> bool:
        ...
        try:
            ...
            if table in self._SCHEMA_MANAGER_TABLES:
                from iris_vector_rag.storage.schema_manager import SchemaManager

                schema_manager = SchemaManager(
                    self.connection_manager, self.config_manager
                )
                created = schema_manager.ensure_table_schema(table)
                if created:
                    self.logger.info(
                        f"Created {table} via SchemaManager canonical schema"
                    )
                else:
                    self.logger.error(f"SchemaManager could not create {table}")
                return created
        ...
        table_schemas = {
            ...
            "DocumentChunks": """
                CREATE TABLE RAG.DocumentChunks (
                    ...
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    metadata TEXT,
                    FOREIGN KEY (source_document_id) REFERENCES RAG.SourceDocuments(doc_id)
                )
            """,
            "VectorEmbeddings": """
