        ...
        try:
            ...
            sql = f"""
                SELECT TOP {top_k} 
                    ...
                    title, 
                    text_content,
                    VECTOR_COSINE(document_embedding_vector, TO_VECTOR('{full_vector_str}', 'FLOAT')) as similarity_score
                FROM {self.schema}.SourceDocuments
                WHERE document_embedding_vector IS NOT NULL
                AND VECTOR_COSINE(document_embedding_vector, TO_VECTOR('{full_vector_str}', 'FLOAT')) > {similarity_threshold}
