"""
Basic RAG Pipeline - RESTORED WORKING VERSION
This restores the version that was working earlier today before the V2 migration broke it.
Uses string-based embeddings with manual cosine similarity calculation.
"""
...
class BasicRAGPipeline:
    ...
    def retrieve_documents(self, query_text: str, top_k: int = 5, similarity_threshold: float = 0.1) -> List[Document]:
        """
        Retrieves documents from IRIS using string-based embeddings and manual cosine similarity.
        This is the approach that was working earlier today.
        """
        ...
        try:
            ...
            # Use the original approach that was working - fetch all docs and calculate similarity in Python
            sql = f"""
            ...
            logger.info(f"BasicRAG: Fetched {len(all_docs)} documents from database")
            
            # Calculate similarities in Python (this was the working approach)
            doc_scores = []
            ...
            for row in all_docs:
                ...
                try:
                    # Parse the stored embedding (comma-separated string)
            ...
            logger.info(f"BasicRAG: Found {len(doc_scores)} documents above threshold {similarity_threshold}")
            
            # Convert to Document objects
            for doc_data in doc_scores:
                # Handle JDBC stream objects for content
                ...
                doc._title = title  # Store title separately for compatibility
                retrieved_docs.append(doc)
            
            logger.info(f"BasicRAG: Successfully processed {len(retrieved_docs)} documents")
...
if __name__ == '__main__':
    ...
    print("Running RESTORED BasicRAGPipeline Demo...")
    ...
    print("\nRESTORED BasicRAGPipeline Demo Finished.")