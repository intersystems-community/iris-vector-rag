...
class BasicRAGPipeline:
    ...
    def retrieve_documents(self, query_text: str, top_k: int = 5, similarity_threshold: float = 0.1) -> List[Document]:
        ...
        try:
            ...
            logger.info(f"BasicRAG: Native VECTOR_COSINE retrieved {len(results)} documents")
            ...
            logger.error(f"BasicRAG: Error during native VECTOR document retrieval: {e}", exc_info=True)
            return []
        finally:
            if cursor:
                cursor.close()
