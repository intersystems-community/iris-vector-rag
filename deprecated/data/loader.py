    ...
    try:
        ...
        for batch_idx, current_doc_batch in enumerate(doc_batches):
            ...
            for doc in current_doc_batch:
                ...
                if embedding_func:
                    text_to_embed = doc.get("text_content") or doc.get("abstract") or doc.get("title", "")
                    if text_to_embed:
            ...
            try:
                ...
                if colbert_doc_encoder_func:
                    ...
                    for doc_for_tokens in docs_for_token_embedding:
                        ...
                        text_for_colbert = doc_for_tokens.get("text_content") or doc_for_tokens.get("abstract") or doc_for_tokens.get("title", "")
...
def process_and_load_documents(
    ...
    if not connection:
        from common.iris_connection_manager import get_iris_connection as get_dbapi_connection
        connection = get_dbapi_connection()
        if not connection:
            return {
                "success": False,
