...
class BasicRAGPipelineTempTable:
    ...
    def retrieve_documents(self, query_text: str, top_k: int = 5, similarity_threshold: float = 0.1) -> List[Document]:
        ...
        try:
            ...
            # Create a regular table for query vector (IRIS doesn't support TEMPORARY TABLE directly in this way for all drivers)
            # Or, if it does, the syntax might vary. This approach creates a persistent table that's dropped.
            logger.info("Creating table for query vector RAG.TempQueryVector...")
            ...
            """
            # Note: The original SQL assumes 'document_embedding_vector' is a VECTOR type.
            # If it's a string, it would also need TO_VECTOR().
            
            logger.info(f"Executing vector similarity search with temporary table approach...")
            cursor.execute(sql)
            ...
            cursor.execute("DROP TABLE RAG.TempQueryVector")
            logger.info("Dropped table RAG.TempQueryVector.")
            
        except Exception as e:
            logger.error(f"Error retrieving documents: {e}")
            # Attempt to drop table even on error if cursor is valid
            if cursor:
                try:
                    cursor.execute("DROP TABLE RAG.TempQueryVector")
                    logger.info("Dropped table RAG.TempQueryVector in error handler.")
                except:
