...
class BasicRAGPipelineStoredProc:
    ...
    def _create_stored_procedure(self):
        ...
        try:
            ...
            END
            """
            # Note: The original SQL in the SP might need adjustment if 'document_embedding_vector'
            # is not the correct column or if it's not a VECTOR type.
            # For this archival purpose, keeping it as is.
            
            cursor.execute(create_proc)
            logger.info("Created stored procedure RAG.VectorSearch")
            
        except Exception as e:
            logger.warning(f"Could not create stored procedure RAG.VectorSearch: {e}")
    ...
    def retrieve_documents(self, query_text: str, top_k: int = 5, similarity_threshold: float = 0.1) -> List[Document]:
        ...
        query_embedding_str = ','.join(map(str, query_embedding)) # Stored proc expects comma-separated string
        ...
        try:
            ...
            try:
                logger.info("Calling stored procedure RAG.VectorSearch for vector search...")
                cursor.execute(
                ...
                logger.warning(f"Stored procedure RAG.VectorSearch failed: {sp_error}")
                logger.info("Falling back to direct query for StoredProc pipeline...")
                
                # Fallback: Use direct query with string interpolation
                # This fallback should ideally match the logic intended for the SP
                sql = f"""
                    SELECT TOP {top_k} 
                        doc_id, 
                        title, 
            ...
            "retrieved_documents": [
                {
                    ...
                    "score": doc.score # Ensure score is part of the main dict if not in metadata
                }
