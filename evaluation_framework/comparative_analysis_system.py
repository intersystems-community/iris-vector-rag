...
class PipelineEvaluationConfig:
    ...
    # Question generation
    question_complexity_distribution: Dict[str, float] = field(default_factory=lambda: {
        'basic': 0.4, 'intermediate': 0.4, 'advanced': 0.2
    })
    question_types_distribution: Dict[str, float] = field(default_factory=lambda: {
        'factual': 0.3, 'analytical': 0.25, 'procedural': 0.2, 'comparative': 0.15, 'causal': 0.1
    })
    
    # Evaluation configuration
    ragas_metrics: List[str] = field(default_factory=lambda: [
        'faithfulness', 'answer_relevancy', 'context_precision', 
        'context_recall', 'context_utilization', 'answer_correctness', 'answer_similarity'
    ])
...
class PipelinePerformanceResult:
    ...
    def get_metric_confidence_interval(self, metric_name: str) -> Optional[Tuple[float, float]]:
...
class ComparativeAnalysisResults:
    ...
    # Summary metrics
    overall_rankings: Dict[str, List[Tuple[str, float]]]  # metric -> [(pipeline, score), ...]
...
class ComparativeAnalysisSystem:
    ...
    def run_comprehensive_comparison(self, 
                                   ...
                                   pipeline_names: Optional[List[str]] = None) -> ComparativeAnalysisResults:
        """
        ...
        """
        experiment_id = f"biomedical_rag_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        summary_metrics = self._generate_summary_metrics(pipeline_results, statistical_results)
        ...
        results = ComparativeAnalysisResults(
            ...
            statistical_significance_summary=statistical_results['significance_summary'],
    ...
    def _generate_evaluation_questions(self, documents: List[Dict[str, Any]]) -> List[BiomedicalQuestion]:
        """Generate biomedical questions for evaluation."""
        
        # Check cache first
        cache_file = self.cache_dir / "evaluation_questions.pkl" if self.config.enable_caching else None
        ...
        # Generate questions
        questions = self.question_generator.generate_questions_from_documents(sampled_docs)
    ...
    def _evaluate_all_pipelines(self, 
                               evaluation_questions: List[BiomedicalQuestion]) -> Dict[str, PipelinePerformanceResult]:
        ...
        for pipeline_name, pipeline in self.initialized_pipelines.items():
            ...
            try:
                result = self._evaluate_single_pipeline(pipeline, pipeline_name, test_questions)
                pipeline_results[pipeline_name] = result
                
                logger.info(f"Completed evaluation of {pipeline_name}: "
    ...
    def _evaluate_single_pipeline(self, 
                                pipeline: Any,
                                pipeline_name: str,
                                questions: List[BiomedicalQuestion]) -> PipelinePerformanceResult:
        ...
        for question in questions:
            try:
                ...
                error_messages.append(f"Question '{question.question[:50]}...': {str(e)}")
    ...
    def _conduct_statistical_analysis(self, 
                                    pipeline_results: Dict[str, PipelinePerformanceResult]) -> Dict[str, Any]:
        ...
        for metric_name in self.config.ragas_metrics:
            ...
            if len(metric_data) >= 2:
                ...
                try:
                    comparison_results = self.statistical_framework.compare_pipelines_statistical(
                        metric_data, metric_name, paired=False
                    )
                    statistical_results['comparisons'][metric_name] = comparison_results
                    
                    # Summarize significance
                    significant_tests = [r for r in comparison_results 
                                       if (r.p_value_adjusted or r.p_value) < self.config.statistical_alpha]
                    ...
                    statistical_results['significance_summary'][metric_name] = {
                        ...
                        'largest_effect_size': max([abs(r.effect_size) for r in comparison_results], default=0.0),
                    ...
                    logger.warning(f"Statistical comparison failed for {metric_name}: {e}")
    ...
    def _generate_summary_metrics(self, 
        ...
        for pipeline_name, result in pipeline_results.items():
            summary['success_rates'][pipeline_name] = result.ragas_results.successful_evaluations if result.ragas_results else 0
        ...
        serializable_results = {
            ...
            'pipeline_results': {
                name: {
                    ...
                    'avg_execution_time': np.mean(result.execution_times) if result.execution_times else 0.0,
                    'ragas_scores': result.ragas_results.to_dict() if result.ragas_results else {},
    ...
    def _create_significance_heatmap(self, results: ComparativeAnalysisResults, plots_dir: Path):
    ...
    def _create_performance_distributions(self, results: ComparativeAnalysisResults, plots_dir: Path):
        ...
        plt.tight_layout()
        plt.savefig(plots_dir / "performance_distributions.png", dpi=300, bbox_inches='tight')
        plt.close()
    
    def _create_execution_time_analysis(self, results: ComparativeAnalysisResults, plots_dir: Path):
    ...
    def _create_performance_summary(self, results: ComparativeAnalysisResults, plots_dir: Path):
        ...
        for pipeline_name in pipelines:
            ...
            for metric in metrics:
                metric_value = result.get_metric_value(metric) if result.ragas_results else 0.0
                row_data[metric] = metric_value if metric_value is not None else 0.0
            
            # Add overall score
            row_data['Overall'] = result.ragas_results.overall_score if result.ragas_results else 0.0
    ...
    def _generate_ranking_table(self, results: ComparativeAnalysisResults) -> str:
        ...
        # Sort by score
        sorted_pipelines = sorted(pipeline_scores.items(), key=lambda x: x[1], reverse=True)
    ...
    def _generate_best_per_metric_table(self, results: ComparativeAnalysisResults) -> str:
    ...
    def _generate_significance_summary_html(self, results: ComparativeAnalysisResults) -> str:
    ...
    def _generate_detailed_metrics_table(self, results: ComparativeAnalysisResults) -> str:
...
def create_comparative_analysis_system(config: Optional[PipelineEvaluationConfig] = None) -> ComparativeAnalysisSystem:
...
if __name__ == "__main__":
    ...
    results = system.run_comprehensive_comparison(
        ...
        pipeline_names=['BasicRAG', 'CRAG']  # Test subset
    )
