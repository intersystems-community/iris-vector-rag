...
from scipy.stats import (
    ...
    bootstrap, permutation_test, combine_pvalues
)
...
class StatisticalEvaluationFramework:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.multiple_correction_method = self.config.get('multiple_correction_method', 'fdr_bh')
    ...
    def conduct_power_analysis(self, 
                             ...
                             metric_name: str) -> PowerAnalysisResult:
        """
        ...
        if current_min_n < sample_size:
            recommendations.append(f"Increase sample size to {int(np.ceil(sample_size))} per group")
        if abs(effect_size) < 0.2:
            recommendations.append("Small effect size detected - consider larger sample or different metrics")
        if achieved_power < self.power_target:
            recommendations.append(f"Current power ({achieved_power:.3f}) below target ({self.power_target})")
        ...
        return PowerAnalysisResult(
            ...
            sample_size=int(np.ceil(sample_size)) if np.isfinite(sample_size) else current_min_n,
            alpha=self.alpha,
            test_type=test_type,
    ...
    def compare_pipelines_statistical(self, 
                                    pipelines_data: Dict[str, List[float]], 
                                    metric_name: str,
                                    paired: bool = False) -> List[StatisticalTestResult]:
        """
        ...
        """
        logger.info(f"Statistical comparison of {len(pipelines_data)} pipelines for {metric_name}")
        ...
        if len(pipeline_names) == 2:
            ...
            # First, overall test
            overall_result = self._compare_multiple_groups(pipelines_data, metric_name, paired)
            ...
            if overall_result.p_value < self.alpha:
                pairwise_results = self._pairwise_comparisons(pipelines_data, metric_name, paired)
    ...
    def _validate_statistical_data(self, pipelines_data: Dict[str, List[float]]):
        ...
        for pipeline_name, data in pipelines_data.items():
            ...
            if missing_rate > self.config['missing_data_threshold']:
                logger.warning(f"High missing data rate ({missing_rate:.2%}) for {pipeline_name}")
    ...
    def _compare_two_groups(self, 
        ...
        if paired:
            ...
            if assumptions['normality'] and assumptions['homoscedasticity']:
                ...
                test_statistic, p_value = stats.mannwhitneyu(g1_clean, g2_clean, alternative='two-sided')
                test_name = "Mann-Whitney U test"
        
        # Calculate effect size
        effect_size = self._calculate_cohens_d(g1_clean, g2_clean)
        effect_size_interpretation = self._interpret_effect_size(effect_size, EffectSizeType.COHENS_D)
        ...
        interpretation = self._generate_interpretation(
            test_name, p_value, effect_size, effect_size_interpretation, 
    ...
    def _compare_multiple_groups(self, 
                               pipelines_data: Dict[str, List[float]], 
                               metric_name: str,
                               paired: bool) -> StatisticalTestResult:
        ...
        effect_size_interpretation = self._interpret_effect_size(effect_size, EffectSizeType.ETA_SQUARED)
        
        # Confidence interval (approximate)
        confidence_interval = (0.0, 1.0)  # Placeholder for overall tests
        
        # Sample sizes
        sample_sizes = [len([x for x in data if np.isfinite(x)]) for data in pipelines_data.values()]
        
        # Interpretation
        pipeline_names = list(pipelines_data.keys())
        interpretation = f"{test_name} comparing {len(pipeline_names)} pipelines on {metric_name}. "
    ...
    def _pairwise_comparisons(self, 
                            pipelines_data: Dict[str, List[float]], 
                            metric_name: str,
                            paired: bool) -> List[StatisticalTestResult]:
    ...
    def _check_two_sample_assumptions(self, 
                                    group1: List[float], 
                                    group2: List[float], 
    ...
    def _check_multiple_group_assumptions(self, 
                                        pipelines_data: Dict[str, List[float]], 
                                        paired: bool) -> Dict[str, bool]:
    ...
    def _calculate_eta_squared(self, pipelines_data: Dict[str, List[float]]) -> float:
        ...
        try:
            ...
            ss_between = sum(len(pipelines_data[group]) * (group_means[group] - overall_mean)**2 
    ...
    def _calculate_post_hoc_power(self, 
                                group1: List[float], 
                                group2: List[float], 
    ...
    def _generate_interpretation(self, 
        ...
        interpretation = f"{test_name} comparing {group1_name} vs {group2_name} on {metric_name}. "
    ...
    def _apply_multiple_comparison_correction(self, 
                                            results: List[StatisticalTestResult]) -> List[StatisticalTestResult]:
        ...
        rejected, p_adjusted, alpha_sidak, alpha_bonf = multipletests(
            p_values, 
            alpha=self.alpha, 
            method=self.multiple_correction_method
        )
        ...
        logger.info(f"Applied {self.multiple_correction_method} correction to {len(results)} tests")
        
        return results
    
    def conduct_bayesian_analysis(self, 
                                 pipelines_data: Dict[str, List[float]], 
        ...
        try:
            ...
            with pm.Model() as model:
                # Priors
                mu = pm.Normal('mu', mu=np.mean(all_scores), sigma=np.std(all_scores), shape=n_groups)
                sigma = pm.HalfNormal('sigma', sigma=np.std(all_scores))
                
                # Likelihood
                likelihood = pm.Normal('likelihood', mu=mu[group_indices], sigma=sigma, observed=all_scores)
    ...
    def generate_statistical_report(self, 
                                  ...
                                  bayesian_results: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            'summary': {
                ...
                'significant_tests': sum(1 for r in results if (r.p_value_adjusted or r.p_value) < self.alpha),
...
def create_statistical_framework(config: Optional[Dict[str, Any]] = None) -> StatisticalEvaluationFramework:
...
if __name__ == "__main__":
    ...
    comparison_results = framework.compare_pipelines_statistical(sample_data, 'faithfulness')
