...
class BiomedicalQuestionGenerator:
    ...
    def _load_medical_knowledge(self):
        ...
        self.medical_entities = {
            # Diseases
            "diabetes", "hypertension", "cancer", "alzheimer", "parkinson",
            "stroke", "heart disease", "pneumonia", "tuberculosis", "malaria",
            "covid-19", "influenza", "asthma", "copd", "arthritis",
            
            # Treatments/Procedures
            "chemotherapy", "radiotherapy", "surgery", "immunotherapy",
            "dialysis", "transplantation", "vaccination", "antibiotic",
            "insulin", "stent", "bypass", "angioplasty",
            
            # Body systems/anatomy
            "cardiovascular", "respiratory", "nervous", "digestive",
            "immune", "endocrine", "musculoskeletal", "reproductive",
            "heart", "lung", "brain", "liver", "kidney", "pancreas",
            
            # Biomarkers/tests
            "biomarker", "blood test", "mri", "ct scan", "ecg", "eeg",
            "biopsy", "genetic test", "protein", "enzyme", "hormone"
        }
        
        # Domain-specific question templates
        self.domain_keywords = {
            "cardiology": ["heart", "cardiac", "coronary", "artery", "blood pressure", "cholesterol"],
            "oncology": ["cancer", "tumor", "metastasis", "chemotherapy", "radiation", "biopsy"],
            "neurology": ["brain", "neural", "cognitive", "memory", "seizure", "dementia"],
            "endocrinology": ["diabetes", "insulin", "hormone", "thyroid", "metabolism"],
            "infectious_disease": ["bacteria", "virus", "infection", "antibiotic", "vaccine"],
            "pulmonology": ["lung", "respiratory", "asthma", "copd", "pneumonia"],
            "gastroenterology": ["digestive", "stomach", "liver", "intestine", "gut"]
        }
    
    def generate_questions_from_documents(self, documents: List[Dict[str, Any]]) -> List[BiomedicalQuestion]:
        """
        ...
        all_questions = []
        target_per_type = self.config.total_questions // len(self.config.question_types_distribution)
        
        for question_type, distribution in self.config.question_types_distribution.items():
            target_count = int(self.config.total_questions * distribution)
            questions = self._generate_questions_by_type(passages, question_type, target_count)
    ...
    def _filter_biomedical_documents(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for doc in documents:
            ...
            medical_entity_count = sum(1 for entity in self.medical_entities if entity in content)
            biomedical_score += medical_entity_count * 2
            
            # Check title for biomedical keywords
            title_medical_count = sum(1 for entity in self.medical_entities if entity in title)
            biomedical_score += title_medical_count * 5
            
            # Check for journal/source indicators
            biomedical_indicators = ['pubmed', 'pmc', 'medline', 'clinical', 'medical', 'journal']
            source_score = sum(1 for indicator in biomedical_indicators 
        ...
        return sorted(biomedical_docs, key=lambda x: x['biomedical_score'], reverse=True)
    
    def _extract_informative_passages(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for doc in documents:
            ...
            sentences = [sent.text.strip() for sent in doc_nlp.sents if len(sent.text.strip()) > 20]
            
            # Create overlapping passages
            passage_size = 5  # sentences per passage
            overlap = 2       # sentence overlap
            
            for i in range(0, len(sentences) - passage_size + 1, passage_size - overlap):
                ...
                if info_score > 0.6:  # Quality threshold
                    passages.append({
                        ...
                        'medical_domain': self._identify_medical_domain(passage_text)
                    })
    ...
    def _score_passage_informativeness(self, passage: str) -> float:
        ...
        medical_entity_count = sum(1 for entity in self.medical_entities if entity in passage_lower)
        ...
        scientific_terms = ['study', 'research', 'analysis', 'treatment', 'diagnosis', 
                          'therapy', 'clinical', 'patient', 'mechanism', 'pathway']
    ...
    def _generate_questions_by_type(self, passages: List[Dict[str, Any]], 
                                  question_type: str, target_count: int) -> List[BiomedicalQuestion]:
                    ...
                    if question_text and answer_text:
                        ...
                        if confidence >= self.config.min_confidence_score:
                            question = BiomedicalQuestion(
                                ...
                                complexity_level=self._assess_complexity(question_text, answer_text),
    ...
    def _get_question_templates(self, question_type: str) -> List[Dict[str, str]]:
        ...
        templates = {
            'factual': [
                ...
                {'pattern': 'function', 'template': 'What is the function of {entity}?'},
                {'pattern': 'characteristic', 'template': 'What are the characteristics of {entity}?'}
            ],
            'analytical': [
                {'pattern': 'mechanism', 'template': 'How does {process} work?'},
                {'pattern': 'relationship', 'template': 'What is the relationship between {entity1} and {entity2}?'},
                {'pattern': 'analysis', 'template': 'What factors contribute to {condition}?'}
            ],
            'procedural': [
                {'pattern': 'treatment', 'template': 'How is {condition} treated?'},
                {'pattern': 'diagnosis', 'template': 'How is {condition} diagnosed?'},
                {'pattern': 'procedure', 'template': 'What are the steps in {procedure}?'}
            ],
            'comparative': [
                {'pattern': 'comparison', 'template': 'How does {entity1} compare to {entity2}?'},
                {'pattern': 'effectiveness', 'template': 'Which is more effective: {option1} or {option2}?'}
            ],
            'causal': [
                {'pattern': 'cause', 'template': 'What causes {condition}?'},
                {'pattern': 'effect', 'template': 'What are the effects of {intervention}?'},
                {'pattern': 'risk', 'template': 'What are the risk factors for {condition}?'}
            ]
        }
        return templates.get(question_type, [])
    
    def _apply_question_template(self, passage: Dict[str, Any], template: Dict[str, str], 
                                question_type: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        medical_entities_found = [entity for entity in self.medical_entities 
    ...
    def _validate_question_answer_pair(self, question: str, answer: str, context: str) -> float:
        """Validate question-answer pair quality and return confidence score."""
        score = 0.0
        
        # Length validation
        if self.config.min_answer_length <= len(answer) <= self.config.max_answer_length:
            score += 0.3
    ...
    def _validate_and_filter_questions(self, questions: List[BiomedicalQuestion]) -> List[BiomedicalQuestion]:
        ...
        for question in questions:
            ...
            for existing in validated:
                if self._calculate_text_similarity(question.question, existing.question) > 0.8:
                    is_duplicate = True
                    break
    ...
    def _balance_question_distribution(self, questions: List[BiomedicalQuestion]) -> List[BiomedicalQuestion]:
        ...
        if len(balanced_questions) > self.config.total_questions:
            balanced_questions = random.sample(balanced_questions, self.config.total_questions)
        
        return balanced_questions
    
    def save_questions_dataset(self, questions: List[BiomedicalQuestion], output_path: str):
    ...
    def _generate_dataset_summary(self, questions: List[BiomedicalQuestion], output_dir: Path):
...
def create_biomedical_question_generator(total_questions: int = 1000) -> BiomedicalQuestionGenerator:
