...
class EvaluationExperimentConfig:
    ...
    pipelines_to_evaluate: List[str] = field(default_factory=lambda: [
...
class EvaluationOrchestrator:
    ...
    def run_complete_evaluation(self, 
                              config_override: Optional[Dict[str, Any]] = None) -> EvaluationRun:
        """
        ...
        try:
            ...
            for stage_idx, stage in enumerate(stages):
                ...
                logger.info(f"Executing stage {stage_idx + 1}/{len(stages)}: {stage.name}")
                
                # Execute stage
                success = self._execute_evaluation_stage(stage)
                
                if not success and stage.required:
                    logger.error(f"Required stage {stage.name} failed - aborting evaluation")
                    run.status = "failed"
                    break
    ...
    def run_partial_evaluation(self, 
                             stages: List[str],
                             config_override: Optional[Dict[str, Any]] = None) -> EvaluationRun:
        """
        ...
        try:
            ...
            for stage_idx, stage in enumerate(filtered_stages):
                ...
                logger.info(f"Executing partial stage {stage_idx + 1}/{len(filtered_stages)}: {stage.name}")
    ...
    def resume_evaluation(self, run_id: str) -> EvaluationRun:
        ...
        try:
            for stage_idx in range(start_stage, len(run.stages)):
                ...
                logger.info(f"Resuming stage {stage_idx + 1}/{len(run.stages)}: {stage.name}")
    ...
    def compare_runs(self, run_ids: List[str]) -> Dict[str, Any]:
        ...
        comparison_file = self.base_output_dir / f"run_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _execute_data_preparation(self, stage: EvaluationStage) -> bool:
        ...
        if stats.successful_documents < 100:
            logger.warning(f"Only {stats.successful_documents} documents processed successfully")
            return False
        
        logger.info(f"Data preparation completed: {stats.successful_documents} documents processed")
        return True
    ...
    def _execute_question_generation(self, stage: EvaluationStage) -> bool:
        ...
        # Generate questions
        questions = self.question_generator.generate_questions_from_documents(processed_docs)
        
        # Save questions
        questions_file = Path(self.current_run.run_directory) / "evaluation_questions.json"
        questions_data = [
            {
        ...
        logger.info(f"Question generation completed: {len(questions)} questions generated")
    ...
    def _execute_pipeline_initialization(self, stage: EvaluationStage) -> bool:
        ...
        self.comparative_analysis = create_comparative_analysis_system(comparison_config)
        ...
        logger.info(f"Pipeline initialization completed: {len(available_pipelines)} pipelines available")
    ...
    def _execute_pipeline_evaluation(self, stage: EvaluationStage) -> bool:
        ...
        results = self.comparative_analysis.run_comprehensive_comparison(
            ...
            pipeline_names=self.config.pipelines_to_evaluate
        )
        ...
        stage.output_files = [
            f"{self.current_run.run_directory}/comparative_analysis"
        ]
        
        logger.info(f"Pipeline evaluation completed: {len(results.pipeline_results)} pipelines evaluated")
        return True
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.current_run.comparative_analysis:
            best_overall = self.current_run.comparative_analysis.get('best_pipeline_per_metric', {})
            
            # Overall best pipeline
            if best_overall:
                most_common = max(set(best_overall.values()), key=list(best_overall.values()).count)
...
"""
            best_per_metric = self.current_run.comparative_analysis.get('best_pipeline_per_metric', {})
...
"""
    ...
    def _compare_run_metrics(self, runs: List[EvaluationRun], comparison: Dict[str, Any]):
...
def create_evaluation_orchestrator(config: Optional[EvaluationExperimentConfig] = None) -> EvaluationOrchestrator:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Biomedical RAG Pipeline Evaluation Orchestrator")
    
    parser.add_argument('--config', type=str, help='Configuration file path')
    parser.add_argument('--experiment-name', type=str, default='biomedical_rag_evaluation')
