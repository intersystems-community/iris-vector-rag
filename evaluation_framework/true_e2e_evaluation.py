...
class TrueE2EEvaluator:
    ...
    def __init__(self):
        ...
        logger.info("Loading real embedding function: sentence-transformers/all-MiniLM-L6-v2")
        self.embedding_func = get_embedding_func(model_name_override='sentence-transformers/all-MiniLM-L6-v2')
    ...
    def _populate_database(self, num_documents: int = 50) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Populating IRIS database with {num_documents} real biomedical documents")
        ...
        for i in range(num_documents):
            ...
            expanded_doc = {
                ...
                'content': base_doc['content'] + f" This research study #{i+1} provides additional clinical insights and evidence-based recommendations for healthcare practitioners.",
        ...
        try:
            ...
            logger.info(f"Successfully added {len(expanded_documents)} documents using IRISVectorStore.add_documents()")
    ...
    def _generate_biomedical_questions(self, documents: List[Dict], count: int = 25) -> List[Dict[str, Any]]:
...
Answer:"""
    ...
    def _evaluate_pipeline_real(self, pipeline_name: str, pipeline, questions: List[Dict]) -> Dict[str, Any]:
        ...
        for i, question_data in enumerate(questions):
            try:
                ...
                if (i + 1) % 5 == 0:
                    logger.info(f"{pipeline_name}: Processed {i+1}/{total_questions} questions, {documents_retrieved} total docs retrieved")
                    
            except Exception as e:
                logger.error(f"Error evaluating question {i+1} with {pipeline_name}: {e}")
                responses.append({
                    'question': question_data['question'],
        ...
        return {
            ...
            'avg_docs_per_query': documents_retrieved / len(responses) if responses else 0
        }
    
    def _calculate_real_ragas_metrics(self, responses: List[Dict], pipeline_name: str) -> Dict[str, float]:
        ...
        responses_with_context = sum(1 for r in responses if r.get('docs_retrieved', 0) > 0)
...
Provide only a numeric score (0.0-1.0):"""
        ...
        metrics['retrieval_success_rate'] = responses_with_context / total_responses if total_responses > 0 else 0
        
        logger.info(f"Final RAGAS metrics for {pipeline_name}: {metrics['overall_score']:.3f}")
        logger.info(f"Document retrieval success rate: {metrics['retrieval_success_rate']:.3f}")
        return metrics
    ...
    def run_true_e2e_evaluation(self, num_documents: int = 50, num_questions: int = 25):
        ...
        self.results = {
            ...
            'e2e_validation': {
                'database_populated': True,
                'documents_retrievable': any(r['total_docs_retrieved'] > 0 for r in pipeline_results.values()),
...
"""
    ...
    def _print_summary(self):
        ...
        execution_time = self.results['execution_time_minutes']
        total_retrieved = sum(r['total_docs_retrieved'] for r in self.results['pipeline_results'].values())
        ...
        for pipeline_name, result in self.results['pipeline_results'].items():
            ...
            print(f"  {pipeline_name}: {score:.3f} ({docs} docs, {success_rate:.2f} retrieval rate)")
