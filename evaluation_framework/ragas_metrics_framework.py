...
try:
    ...
    from ragas.metrics import (
        ...
        context_recall, context_utilization, answer_correctness, 
        answer_similarity
    )
...
class BiomedicalRAGASFramework:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            logger.info(f"Loaded biomedical QA model: {self.config['biomedical_qa_model']}")
    ...
    def evaluate_pipeline(self, 
                         ...
                         pipeline_name: str) -> ComprehensiveRAGASResults:
        """
        ...
        evaluation_data = self._prepare_evaluation_data(pipeline_results, ground_truth_answers)
        ...
        for metric_name, metric_func in metrics_to_evaluate:
            try:
                ...
                logger.info(f"{metric_name}: {metric_result.value:.4f} ± {metric_result.standard_error:.4f}")
            except Exception as e:
        ...
        logger.info(f"RAGAS evaluation completed in {total_time:.2f}s. "
                   f"Overall score: {results.overall_score:.4f}")
        
        return results
    
    def _validate_evaluation_inputs(self, pipeline_results: List[Dict[str, Any]], 
                                  ground_truth_answers: List[str]):
        """Validate inputs for RAGAS evaluation."""
        if len(pipeline_results) != len(ground_truth_answers):
            raise ValueError("Mismatch between pipeline results and ground truth answers")
    ...
    def _prepare_evaluation_data(self, pipeline_results: List[Dict[str, Any]], 
                               ground_truth_answers: List[str]) -> List[Dict[str, Any]]:
        """Prepare data for RAGAS evaluation."""
        evaluation_data = []
        
        for i, (result, ground_truth) in enumerate(zip(pipeline_results, ground_truth_answers)):
            ...
            if (evaluation_item['question'] and 
                evaluation_item['answer'] and 
                evaluation_item['contexts'] and 
                evaluation_item['ground_truth']):
    ...
    def _evaluate_faithfulness(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
    ...
    def _evaluate_answer_relevancy(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
        ...
        try:
            question_embeddings = self.embedding_model.encode(questions, batch_size=self.config['batch_size'])
            answer_embeddings = self.embedding_model.encode(answers, batch_size=self.config['batch_size'])
    ...
    def _evaluate_context_precision(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
    ...
    def _evaluate_context_recall(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
    ...
    def _evaluate_context_utilization(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
        ...
        for item in evaluation_data:
            try:
                ...
                utilization = cosine_similarity(answer_embedding, context_embedding)[0][0]
    ...
    def _evaluate_answer_correctness(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
        ...
        try:
            ...
            answer_embeddings = self.embedding_model.encode(answers, batch_size=self.config['batch_size'])
            gt_embeddings = self.embedding_model.encode(ground_truths, batch_size=self.config['batch_size'])
            ...
            for i, (answer, gt) in enumerate(zip(answers, ground_truths)):
                try:
                    ...
                    entailment1 = self.entailment_model(f"Premise: {answer} Hypothesis: {gt}")
                    entailment2 = self.entailment_model(f"Premise: {gt} Hypothesis: {answer}")
    ...
    def _evaluate_answer_similarity(self, evaluation_data: List[Dict[str, Any]]) -> RAGASResult:
        """
    ...
    def _compute_metric_statistics(self, metric_name: str, scores: List[float], 
                                 computation_time: float) -> RAGASResult:
        ...
        standard_error = std_score / np.sqrt(len(valid_scores)) if len(valid_scores) > 0 else 0.0
    ...
    def _compute_confidence_interval(self, scores: List[float]) -> Tuple[float, float]:
        ...
        try:
            ...
            bootstrap_means = [bootstrap_mean(data) for _ in range(self.config['bootstrap_samples'])]
    ...
    def compare_pipelines(self, pipeline_results: Dict[str, ComprehensiveRAGASResults]) -> Dict[str, Any]:
        """
        ...
        for metric_name in ['faithfulness', 'answer_relevancy', 'context_precision',
            ...
            if len(metric_data) > 1:
                ...
                if len(individual_scores) >= 2:
                    significance_results = self._test_statistical_significance(individual_scores)
                    comparison['statistical_significance'][metric_name] = significance_results
    ...
    def _test_statistical_significance(self, score_groups: Dict[str, List[float]]) -> Dict[str, Any]:
        ...
        for i in range(len(pipeline_names)):
            for j in range(i + 1, len(pipeline_names)):
                ...
                try:
                    ...
                    if p_value < 0.05:
                        better_pipeline = pipeline1 if np.mean(scores1) > np.mean(scores2) else pipeline2
                    ...
                    logger.warning(f"Statistical test failed for {pipeline1} vs {pipeline2}: {e}")
        
        return significance_results
    
    def _calculate_effect_size(self, scores1: List[float], scores2: List[float]) -> float:
    ...
    def save_results(self, results: Union[ComprehensiveRAGASResults, Dict[str, Any]], 
                    output_path: str):
...
def create_biomedical_ragas_framework(config: Optional[Dict[str, Any]] = None) -> BiomedicalRAGASFramework:
    ...
    sample_pipeline_results = [
        {
            ...
            'contexts': ['Diabetes mellitus symptoms include polydipsia, polyuria, and general weakness.']
        }
    ]
