...
class PMCDataPipeline:
    ...
    def _initialize_nlp_models(self):
        ...
        try:
            ...
            logger.warning(f"SpaCy model {self.config.spacy_model} not found, using default")
    ...
    def process_documents(self, 
                         ...
                         resume_from_checkpoint: bool = False) -> ProcessingStats:
        """
        ...
        try:
            for batch in self._batch_iterator(documents, self.config.batch_size):
                ...
                if (self.config.max_documents and 
                    self.stats.successful_documents >= self.config.max_documents):
    ...
    def _batch_iterator(self, documents: Iterator[PMCDocument], batch_size: int) -> Iterator[List[PMCDocument]]:
    ...
    def _process_single_document(self, document: PMCDocument) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._validate_document(document):
                return {'pmc_id': document.pmc_id, 'status': 'failed', 'error': 'validation_failed'}
            ...
            if document.quality_score < self.config.min_quality_score:
                return {'pmc_id': document.pmc_id, 'status': 'failed', 'error': 'low_quality'}
    ...
    def _preprocess_document(self, document: PMCDocument) -> PMCDocument:
        ...
        document.full_text = re.sub(r'[^\w\s\.\,\;\:\!\?\-\(\)]', ' ', document.full_text)
    ...
    def _assess_biomedical_relevance(self, document: PMCDocument) -> float:
        ...
        biomedical_keywords = [
            'patient', 'treatment', 'disease', 'clinical', 'medical', 'therapy',
            'diagnosis', 'symptom', 'drug', 'medication', 'hospital', 'doctor',
            'health', 'medicine', 'study', 'trial', 'research', 'analysis'
        ]
        ...
        # Check journal name
        if document.journal and any(term in document.journal.lower() 
    ...
    def _create_document_chunks(self, document: PMCDocument) -> List[DocumentChunk]:
        ...
        if document.abstract:
            chunks.append(self._create_chunk(
                document.pmc_id, document.abstract, 'abstract', 0
            ))
    ...
    def _create_chunk(self, pmc_id: str, content: str, chunk_type: str, index: int) -> DocumentChunk:
    ...
    def _calculate_chunk_biomedical_relevance(self, content: str) -> float:
        ...
        biomedical_keywords = [
            'patient', 'treatment', 'disease', 'clinical', 'medical', 'therapy',
            'diagnosis', 'symptom', 'drug', 'medication'
        ]
        
        content_lower = content.lower()
        keyword_count = sum(1 for keyword in biomedical_keywords if keyword in content_lower)
    ...
    def _store_document_and_chunks(self, document: PMCDocument, chunks: List[DocumentChunk]):
    ...
    def _store_in_metadata_db(self, document: PMCDocument, chunks: List[DocumentChunk]):
        ...
        try:
            ...
            cursor.execute("""
                ...
                (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                document.pmc_id, document.title, document.abstract,
                json.dumps(document.authors), document.publication_date,
                document.journal, document.doi, json.dumps(document.keywords),
                json.dumps(document.mesh_terms), document.content_type,
                document.language, document.word_count, document.chunk_count,
                document.quality_score, document.processing_timestamp,
            ...
            for chunk in chunks:
                cursor.execute("""
                    ...
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    chunk.chunk_id, chunk.pmc_id, chunk.content,
                    chunk.chunk_type, chunk.chunk_index, chunk.token_count,
                    chunk.overlap_with_previous, chunk.information_density,
                    chunk.readability_score, chunk.biomedical_relevance
                ))
    ...
    def _log_processing_error(self, pmc_id: str, error_message: str):
        ...
        """, (
            None, pmc_id, 'process_document', 'failed',
    ...
    def _log_progress(self):
        ...
        success_rate = (self.stats.successful_documents / 
                       max(1, self.stats.total_documents)) * 100
    ...
    def _save_checkpoint(self):
        ...
        checkpoint_data = {
            'processing_state': {
                ...
                'processed_documents': list(self.processing_state['processed_documents']),
    ...
    def _load_checkpoint(self):
        ...
        if checkpoint_file.exists():
            ...
            self.processing_state['processed_documents'] = set(state['processed_documents'])
    ...
    def _calculate_avg_quality(self) -> float:
        ...
        cursor.execute("SELECT AVG(quality_score) FROM documents WHERE quality_score > 0")
    ...
    def _generate_processing_report(self):
        ...
        report_data = {
            ...
            'success_rate': (self.stats.successful_documents / 
                           max(1, self.stats.total_documents)) * 100,
            'average_chunks_per_doc': (self.stats.total_chunks / 
                                     max(1, self.stats.successful_documents)),
...
def create_pmc_data_pipeline(config: Optional[ProcessingConfig] = None) -> PMCDataPipeline:
