#!/usr/bin/env python3
"""
Test Direct VECTOR Column with HNSW Index in IRIS 2025.1

This script tests creating a table with a VECTOR column, inserting data using TO_VECTOR
to convert string embeddings to VECTOR type during insertion, and creating an HNSW index
on the VECTOR column.

The approach:
1. Create a table with a VECTOR column (not VARCHAR)
2. Insert data using TO_VECTOR to convert string embeddings to VECTOR type during insertion
...
logger = logging.getLogger("test_direct_vector_hnsw")
...
class DirectVectorHNSWTest:
    def __init__(self):
        ...
        self.table_name = "DirectVectorHNSWTest"
        self.embedding_dim = 384  # dimension for all-MiniLM-L6-v2
        
    def setup_database(self):
        """Set up the database table with a VECTOR column."""
        logger.info("\n=== SETUP: Creating Table with VECTOR Column ===\n")
        ...
        try:
            ...
            # Try different syntax variations for creating a table with a VECTOR column
            
            # Syntax 1: VECTOR(dim, type) without quotes
            logger.info("\nTrying to create table with VECTOR column...")
            create_table_sql = f"""
            ...
            try:
                ...
                self.conn.commit()
                logger.info("✅ Successfully created table with VECTOR column")
            except Exception as e:
                logger.error(f"❌ Failed to create table with VECTOR column: {e}")
                
                # If that fails, try creating a table with a VARCHAR column for embeddings
                logger.info("\nFalling back to VARCHAR column...")
    ...
    def insert_documents(self):
        ...
        try:
            # Check if the table has a VECTOR column or VARCHAR column
            check_columns_sql = f"""
            ...
            if has_vector_column:
                logger.info("Table has VECTOR column, inserting with TO_VECTOR...")
                ...
                for i, doc in enumerate(TEST_DOCUMENTS):
                    ...
                    # Insert document with TO_VECTOR to convert string to VECTOR
                    insert_sql = f"""
                    INSERT INTO {self.table_name} (id, text_content, vector_embedding)
                    ...
                    try:
                        ...
                        self.conn.commit()
                        logger.info(f"✅ Successfully inserted document {i+1} with TO_VECTOR")
                    except Exception as e:
                        logger.error(f"❌ Failed to insert document {i+1} with TO_VECTOR: {e}")
                        self.conn.rollback()
                        
                        # Try alternative syntax
                        logger.info("Trying alternative syntax...")
                        insert_alt_sql = f"""
                        INSERT INTO {self.table_name} (id, text_content, vector_embedding)
                        SELECT ?, ?, TO_VECTOR(?, double, {self.embedding_dim})
                        """
                        
                        try:
                            cursor.execute(insert_alt_sql, (f"doc{i+1}", doc, embedding_str))
                            self.conn.commit()
                            logger.info(f"✅ Successfully inserted document {i+1} with alternative syntax")
                        except Exception as e2:
                            logger.error(f"❌ Failed to insert document {i+1} with alternative syntax: {e2}")
                            self.conn.rollback()
            else:
                logger.info("Table has VARCHAR column, inserting embeddings as strings...")
                ...
                self.conn.commit()
                logger.info(f"✅ Successfully inserted {len(embeddings)} documents with embeddings as strings")
    ...
    def run_tests(self):
        """Run all tests."""
        logger.info("Starting Direct VECTOR Column with HNSW Index Tests")
