...
class EntityDeduplicator:
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.merge_strategy == MergeStrategy.CUSTOM and self.custom_merge_fn is None:
            raise ValueError("Custom merge function required when using CUSTOM merge strategy")
    ...
    def deduplicate_entities(
        ...
        try:
            ...
            merged_entities = self._merge_entity_clusters(clusters, transaction_id)
            
            # Calculate results
            processing_time = (time.time() - start_time) * 1000
            duplicates_removed = sum(len(cluster.duplicate_entities) for cluster in clusters)
    ...
    def _find_duplication_clusters(
        ...
        for entity in entities:
            ...
            similar_entities = self._find_similar_entities(entity, entities, include_existing)
    ...
    def _merge_entity_clusters(
        ...
        for cluster in clusters:
            try:
                ...
                merged_entity = self._merge_entities(all_entities, cluster.merge_strategy)
                cluster.merged_entity = merged_entity
                merged_entities.append(merged_entity)
                
            except Exception as e:
                logger.error(f"Failed to merge cluster with primary {cluster.primary_entity.id}: {e}")
    ...
    def _merge_entities(self, entities: List[Entity], strategy: MergeStrategy) -> Entity:
