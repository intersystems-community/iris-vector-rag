...
The [`ReconciliationController`](iris_rag/controllers/reconciliation.py:118) class in `iris_rag/controllers/reconciliation.py` is currently 1064 lines and contains several distinct responsibilities that can be extracted into separate modules for better maintainability and testability.
...
- New drift detection strategies can be added to `DriftAnalyzer`
- New remediation actions can be added to `RemediationEngine`
...
- The main `ReconciliationController` class maintains its existing public interface
