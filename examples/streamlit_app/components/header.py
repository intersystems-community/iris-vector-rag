...
def render_breadcrumb(path: list):
    ...
    for i, (name, url) in enumerate(path):
        if i == len(path) - 1:  # Last item (current page)
            ...
            breadcrumb_html.append(f'<a href="{url}" style="color: #1f77b4; text-decoration: none;">{name}</a>')
        
        if i < len(path) - 1:
            breadcrumb_html.append('<span style="color: #ccc; margin: 0 0.5rem;"> / </span>')
...
def render_notification_banner(message: str, type: str = "info", dismissible: bool = True):
    """
