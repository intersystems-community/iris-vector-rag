...
class SampleDataManager:
    ...
    def get_random_query(self, category: Optional[str] = None, 
                        difficulty: Optional[str] = None) -> Optional[SampleQuery]:
        ...
        if difficulty and category:
            ...
            queries = [q for q in self.get_queries_by_category(category) 
    ...
    def get_category_info(self) -> List[Tuple[str, str, int]]:
        ...
        """
        categories = self.load_sample_queries()
        return [(key, cat.name, len(cat.queries)) 
    ...
    def search_queries(self, search_term: str) -> List[SampleQuery]:
        ...
        for query in all_queries:
            if (search_term_lower in query.query.lower() or
                any(search_term_lower in topic.lower() for topic in query.expected_topics)):
...
def get_random_sample_query(category: Optional[str] = None, 
                           difficulty: Optional[str] = None) -> Optional[SampleQuery]:
