...
from components.header import render_page_header, render_notification
from utils.app_config import get_config, AppConfig
...
def main():
    ...
    # Configuration tabs
    tab1, tab2, tab3, tab4 = st.tabs(["🤖 Pipeline Settings", "🌍 Environment", "📋 Preferences", "🔧 Advanced"])
...
def render_basic_rerank_settings(settings: Dict[str, Any]):
    ...
    with col2:
        rerank_model = st.selectbox(
            "Rerank Model",
            ["cross-encoder/ms-marco-MiniLM-L-6-v2", "cross-encoder/ms-marco-TinyBERT-L-2", "custom"],
            index=0,
    ...
    with st.expander("🔧 General Pipeline Options"):
        ...
        with col1:
            model_name = st.selectbox(
                "LLM Model",
                ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo", "claude-3-sonnet", "claude-3-haiku"],
                index=0,
...
def test_pipeline_configuration(pipeline_type: str):
    ...
    with st.spinner(f"Testing {pipeline_type} configuration..."):
        ...
        import time
        time.sleep(2)
        render_notification(f"{pipeline_type} configuration test successful!", "success")
