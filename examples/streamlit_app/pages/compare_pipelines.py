...
def render_pipeline_selection():
    ...
    if selected_pipelines:
        ...
        for pipeline in selected_pipelines:
            # Mock status for now
            status = (
                "🟢 Ready" if pipeline in ["BasicRAG", "CRAG"] else "🟡 Initializing"
            )
    ...
    if use_same_config:
        ...
        with col2:
            temperature = st.slider(
                "Temperature:", 0.0, 1.0, 0.7, step=0.1, key="shared_temp"
            )
        ...
        st.info(
            "Individual pipeline configurations can be set in the Configuration page"
        )
...
def render_comparison_results():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📋 Side-by-Side", "🎯 Answer Quality", "📄 Documents", "⚡ Performance"]
    )
...
def render_comparison_overview():
    ...
    with col3:
        if pipeline_results:
            avg_time = sum(r.execution_time for r in pipeline_results.values()) / len(
                pipeline_results
            )
    ...
    with col4:
        if pipeline_results:
            total_docs = sum(
                len(r.retrieved_documents) for r in pipeline_results.values()
            )
        ...
        if result.success:
            ...
            quality_metrics = {
                ...
                "Has Citations": (
                    "✅"
                    if any(
                        ref in answer.lower() for ref in ["study", "research", "source"]
                    )
                    else "❌"
                ),
            }
...
def execute_comparison(query: str, pipelines: List[str]):
    ...
    with st.spinner("Executing comparison..."):
        ...
        st.success(
            f"✅ Comparison complete! {successful_count}/{len(pipelines)} pipelines succeeded"
        )
...
def get_pipeline_icon(pipeline: str) -> str:
    ...
    icons = {"BasicRAG": "📝", "BasicRerank": "🎯", "CRAG": "🔄", "GraphRAG": "🕸️"}
...
def calculate_category_winners(pipeline_results: Dict[str, Any]) -> Dict[str, str]:
    ...
    if successful_results:
        ...
        # Most documents
        most_docs = max(
            successful_results.items(), key=lambda x: len(x[1].retrieved_documents)
        )
...
def render_key_differences(results: Dict[str, Any]):
    ...
    if len(results) >= 2:
        pipelines = list(results.keys())
        st.markdown(
            f"• {pipelines[0]} focuses on: *[Analysis would highlight key themes]*"
        )
