...
from components.header import render_page_header, render_loading_spinner, render_error_message
from components.query_input import render_query_input, render_query_configuration, render_query_history
from components.sidebar import render_pipeline_selector, render_advanced_settings
...
def render_documents_tab():
    ...
    with col2:
        avg_score = sum(doc.get('metadata', {}).get('score', 0) for doc in docs) / len(docs)
...
def execute_single_query(query: str, pipeline: str, config: dict):
    ...
    with st.spinner(f"Executing query with {pipeline}..."):
        try:
            ...
            if result.success:
                st.success(f"✅ Query executed successfully in {result.execution_time:.2f}s")
            else:
...
def render_historical_performance():
    ...
    pipeline_metrics = [m for m in metrics if m.get("pipeline") == pipeline][-10:]  # Last 10
    
    if pipeline_metrics:
        avg_time = sum(m.get("execution_time", 0) for m in pipeline_metrics) / len(pipeline_metrics)
        success_rate = sum(1 for m in pipeline_metrics if m.get("success", False)) / len(pipeline_metrics)
