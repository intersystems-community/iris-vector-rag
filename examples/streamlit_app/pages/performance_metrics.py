...
def render_overview_metrics():
    ...
    with col1:
        render_metric_card(
            "Total Queries", str(summary.get("total_queries", 0)), "", "🔍"
        )

    with col2:
        avg_time = summary.get("average_execution_time", 0)
        render_metric_card("Avg Response Time", f"{avg_time:.2f}s", "", "⚡")

    with col3:
        pipelines_used = summary.get("unique_pipelines_used", 0)
        render_metric_card("Pipelines Used", str(pipelines_used), "", "🤖")
...
def render_detailed_analytics():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["⚡ Performance", "🎯 Quality", "📊 Usage", "🔍 Deep Dive"]
    )
...
def render_pipeline_performance_comparison(df: pd.DataFrame):
    ...
    pipeline_avg = (
        df.groupby("pipeline")["execution_time"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
            ...
            lengths.append(
                {
                    ...
                    "quality_estimate": min(
                        1.0, results.get("answer_length", 0) / 500
    ...
    if lengths:
        ...
        with col1:
            ...
            fig = px.bar(
                ...
                x="pipeline",
                y="length",
                title="Average Answer Length by Pipeline",
            )
            st.plotly_chart(fig, use_container_width=True)
...
def render_pipeline_usage_chart(summary: Dict):
    ...
    fig = px.pie(
        df, values="Count", names="Pipeline", title="Pipeline Usage Distribution"
    )
    st.plotly_chart(fig, use_container_width=True)
...
def render_usage_insights(summary: Dict, history: List[Dict]):
    ...
    if usage:
        ...
        insights.append(
            f"🔥 Most used pipeline: {most_used[0]} ({most_used[1]} queries)"
        )
...
def render_advanced_pipeline_comparison():
    ...
    comparison_metrics = (
        df.groupby("pipeline")
        .agg({"execution_time": ["mean", "std"], "success": "mean"})
        .round(3)
    )
...
def render_query_complexity_analysis():
    ...
    st.info(
        "Query complexity analysis would examine query characteristics and performance correlation"
    )
...
def render_demo_metrics():
    ...
    st.markdown(
        "*These are example metrics. Real data will appear after running queries.*"
    )
