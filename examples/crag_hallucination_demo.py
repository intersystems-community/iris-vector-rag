...
class CorrectionMetrics:
    ...
    def detection_rate(self) -> float:
        return (self.hallucinations_detected / self.total_queries) if self.total_queries > 0 else 0.0
    
    @property
    def correction_accuracy(self) -> float:
        return (self.corrections_made / self.hallucinations_detected) if self.hallucinations_detected > 0 else 0.0
...
class HallucinationDetector:
    ...
    def detect_hallucinations(self, text: str, retrieved_docs: List[Any]) -> List[HallucinationDetection]:
        """
    ...
    def _analyze_match(self, match: re.Match, halluc_type: HallucinationType, 
                      text: str, doc_contents: List[str]) -> Optional[HallucinationDetection]:
    ...
    def _generate_correction(self, original: str, halluc_type: HallucinationType, 
                           doc_contents: List[str]) -> Tuple[str, str]:
        ...
        return corrections.get(halluc_type, (original, "No specific correction available"))
    
    def _detect_unsupported_claims(self, text: str, doc_contents: List[str]) -> List[HallucinationDetection]:
...
class CorrectionVisualizer:
    ...
    def show_side_by_side_comparison(self, query: str, basic_result: Dict[str, Any], 
                                   crag_result: Dict[str, Any], 
                                   detections: List[HallucinationDetection]) -> str:
        ...
        # Basic RAG Result
        output.append(f"{self._colorize('BASIC RAG RESPONSE:', Fore.RED, Style.BRIGHT)}")
        output.append("-" * 40)
        ...
        output.append(f"{self._colorize('CRAG CORRECTED RESPONSE:', Fore.GREEN, Style.BRIGHT)}")
        output.append("-" * 40)
        ...
        if detections:
            output.append(f"{self._colorize('CORRECTIONS MADE:', Fore.YELLOW, Style.BRIGHT)}")
            output.append("-" * 40)
            for i, detection in enumerate(detections, 1):
                output.append(f"{i}. {self._colorize(detection.type.value.upper(), Fore.MAGENTA, Style.BRIGHT)}")
                output.append(f"   Original: {self._colorize(detection.original_text, Fore.RED)}")
                output.append(f"   Corrected: {self._colorize(detection.corrected_text, Fore.GREEN)}")
                output.append(f"   Confidence: {detection.confidence_score:.2f}")
                output.append(f"   Explanation: {detection.explanation}")
                output.append("")
        else:
            output.append(f"{self._colorize('No hallucinations detected.', Fore.GREEN)}")
            output.append("")
        ...
        output.append(f"{self._colorize('PERFORMANCE METRICS:', Fore.BLUE, Style.BRIGHT)}")
        output.append("-" * 40)
        output.append(f"Basic RAG execution time: {basic_time:.3f}s")
        output.append(f"CRAG execution time: {crag_time:.3f}s")
        output.append(f"Processing overhead: {(crag_time - basic_time):.3f}s ({((crag_time/basic_time - 1) * 100):.1f}% increase)")
        output.append(f"Hallucinations detected: {len(detections)}")
        output.append("")
        
        return "\n".join(output)
    
    def _highlight_corrections(self, text: str, detections: List[HallucinationDetection]) -> str:
        ...
        for detection in sorted_detections:
            ...
            if 0 <= start < len(highlighted_text) and start < end <= len(highlighted_text):
                correction = self._colorize(detection.corrected_text, Fore.GREEN, Back.BLACK)
                highlighted_text = (
    ...
    def _colorize(self, text: str, fore_color: str = "", back_color: str = "", 
                 style: str = "") -> str:
...
<body>
    ...
    def _create_result_html(self, result: Dict[str, Any], index: int) -> str:
        ...
        if detections:
            ...
            corrections_html = "<div class='corrections'>No hallucinations detected.</div>"
...
class CRAGHallucinationDemo:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.visualizer = CorrectionVisualizer(use_colors=self.config.get('use_colors', True))
    ...
    def _setup_pipelines(self):
        ...
        try:
            ...
            self.logger.info("Real iris_rag pipelines initialized with database connections")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize iris_rag pipelines: {e}")
            raise RuntimeError(f"Could not initialize RAG pipelines for demonstration: {e}")
    ...
    def _get_llm_function(self):
            ...
            try:
                ...
                def openai_llm(prompt: str) -> str:
                    ...
                    response = client.chat.completions.create(
                        model="gpt-4o-mini",
                        messages=[
                            {"role": "system", "content": "You are a helpful medical assistant. Provide accurate, evidence-based information and acknowledge uncertainty when appropriate."},
        ...
        self.logger.warning("Using dummy LLM - set OPENAI_API_KEY for real LLM responses")
    ...
    def demonstrate_correction(self, query: str) -> Dict[str, Any]:
        ...
        self.metrics.corrections_made += len([d for d in detections if d.corrected_text != d.original_text])
        self.metrics.processing_time_ms += (time.time() - start_time) * 1000
        if detections:
            self.metrics.confidence_avg = sum(d.confidence_score for d in detections) / len(detections)
    ...
    def run_interactive_demo(self):
        ...
        print(self.visualizer._colorize(
        ...
        print("Type 'medical' for medical examples, 'metrics' for performance stats, or 'quit' to exit.")
        print("")
        ...
        while True:
            try:
                ...
                comparison = self.visualizer.show_side_by_side_comparison(
                    query,
                    {'answer': result['basic_answer'], 'execution_time': result['basic_time']},
                    {'answer': result['crag_answer'], 'execution_time': result['crag_time']},
                    detections
                )
    ...
    def _show_final_summary(self):
        ...
        if self.results_history:
            print(f"Session completed with {len(self.results_history)} queries analyzed.")
            
            # Offer to export results
            export = input(f"{Fore.YELLOW}Export results to HTML? (y/N): {Style.RESET_ALL}").strip().lower()
            if export == 'y':
    ...
    def run_test_cases(self, test_cases_file: str):
        ...
        try:
            ...
            for i, test_case in enumerate(test_cases, 1):
                ...
                print(f"\nTest Case {i}: {query}")
                print(f"Expected hallucination types: {', '.join(expected_hallucinations)}")
                print("-" * 40)
