...
from iris_rag.memory import (
    KnowledgePatternExtractor, TemporalMemoryManager, IncrementalLearningManager,
    MemoryEnabledRAGPipeline, MemoryConfig, TemporalWindow, TemporalQuery
)
...
async def demonstrate_basic_memory_integration():
    ...
    memory_wrapper = wrap_existing_pipeline(base_pipeline, memory_config, connection_manager)
...
async def demonstrate_temporal_memory_patterns():
    ...
    temporal_windows = [
        ...
        {"name": "domain_knowledge", "duration_days": 365, "retention_policy": "archive"}
    ]
    ...
    memories = [
        ("User asked about diabetes symptoms", TemporalWindow.SHORT_TERM, "diabetes query"),
        ("User prefers detailed explanations", TemporalWindow.MEDIUM_TERM, "user preference"),
        ("Medical terminology definitions", TemporalWindow.LONG_TERM, "domain knowledge")
    ]
...
async def demonstrate_incremental_learning():
    ...
    class MockSchemaManager:
        def ensure_table_schema(self, table_name): pass
    ...
    new_documents = [
        Document(id="doc1", page_content="Diabetes is a metabolic disorder.", metadata={"source": "medical"}),
        Document(id="doc2", page_content="Symptoms include increased thirst.", metadata={"source": "medical"}),
        Document(id="doc3", page_content="Treatment involves insulin therapy.", metadata={"source": "medical"})
    ]
    ...
    print(f"Average learning time: {metrics['learning_performance']['avg_time_ms']:.2f}ms")
...
async def demonstrate_complete_integration():
    ...
    for name, pipeline in pipelines.items():
        memory_pipeline = MemoryEnabledRAGPipeline(pipeline, memory_config, connection_manager)
    ...
    for name, pipeline in memory_pipelines.items():
        ...
        print(f"  {name}: {stats['pipeline_metrics']['total_queries']} queries, "
...
async def main():
    ...
    try:
        # Run all examples
        wrapper = await demonstrate_basic_memory_integration()
        temporal_mgr = await demonstrate_temporal_memory_patterns() 
        learning_mgr = await demonstrate_incremental_learning()
        pipelines = await demonstrate_complete_integration()
