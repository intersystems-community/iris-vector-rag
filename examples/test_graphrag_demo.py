...
def test_json_structure():
    ...
    try:
        ...
        required_keys = ['metadata', 'queries', 'evaluation_metrics', 'usage_instructions']
    ...
    try:
        ...
        for query_list in [hop2_queries, hop3_queries, complex_queries]:
            for query in query_list:
                required_fields = ['id', 'query', 'category', 'difficulty', 'reasoning_path']
                for field in required_fields:
                    if field not in query:
                        print(f"❌ Missing field {field} in query {query.get('id', 'unknown')}")
                        return False
...
def test_medical_domain_coverage():
    ...
    try:
        ...
        # Extract all queries
        all_queries = []
        for query_type in ['2_hop_queries', '3_hop_queries', 'complex_reasoning_queries']:
            all_queries.extend(data['queries'][query_type])
        
        # Check for medical concepts
        medical_concepts = [
            'diabetes', 'hypertension', 'cardiovascular', 'medication', 'drug',
            'treatment', 'symptom', 'complication', 'interaction', 'side effect'
        ]
