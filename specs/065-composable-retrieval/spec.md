...
- What happens when a retrieval mode is requested on a pipeline whose retrieval is intrinsically different (e.g. `pylate_colbert` late-interaction)? The pipeline maps the requested mode onto its native retrieval where sensible, and otherwise raises the FR-012 prerequisite error naming the unsupported mode. Reranking (`rerank`) still applies universally as a post-retrieval step regardless of the pipeline's native retrieval.
...
- **Full parity across pipelines**: Query-time `rerank` and the `vector`/`text`/`hybrid`/`rrf` retrieval modes are supported on every registered pipeline (resolved 2026-07-22). The registered pipelines are `basic`, `basic_rerank`, `crag`, `graphrag` (→ HybridGraphRAGPipeline), `pylate_colbert` (→ PyLateColBERTPipeline), and `multi_query_rrf`. Reranking is a universal post-retrieval step. Fusion modes rely on iris-vector-graph BM25 for the text side, which is independent of a populated knowledge graph; graph-traversal retrieval remains gated on an actual knowledge graph. Where a genuine prerequisite is absent, the query errors clearly (FR-012) rather than silently falling back. Parity means every pipeline *accepts* every mode argument and either serves it natively or raises the prerequisite error — not that every pipeline reimplements every backend (e.g. `pylate_colbert` may serve only the modes compatible with late-interaction retrieval).
