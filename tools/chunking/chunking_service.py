    ...
    def chunk(self, text: str, doc_id: str = None) -> List[Chunk]:
        ...
        while start < len(text):
            ...
            if self.preserve_sentences and end < len(text):
                ...
                search_start = max(
                    start + int(self.chunk_size * 0.8), start + self.min_chunk_size
                )
                    ...
                    Chunk(
                        ...
                        metadata={
                            ...
                            "overlap_with_previous": (
                                min(self.overlap_size, start) if start > 0 else 0
                            ),
                            "strategy_params": {
        ...
        for i, sentence in enumerate(sentences):
            ...
            should_split = (
                ...
                or (
                    ...
                    and self._should_split_here(sentences, i)
                )
    ...
    def chunk(self, text: str, doc_id: str = None) -> List[Chunk]:
        ...
        for chunk in primary_chunks:
            if len(chunk.text) <= self.max_chunk_size:
                ...
                chunk.metadata["primary_strategy"] = (
                    self.primary_strategy.get_strategy_name()
                )
...
class DocumentChunkingService:
    ...
    def chunk_document(
        self, doc_id: str, text: str, strategy_name: str
    ) -> List[Dict[str, Any]]:
        ...
        for strategy_name, strategy in self.strategies.items():
            try:
                ...
                strategy_analysis = {
                    ...
                    "avg_chunk_length": (
                        statistics.mean(chunk_lengths) if chunk_lengths else 0
                    ),
                    "min_chunk_length": min(chunk_lengths) if chunk_lengths else 0,
                    "max_chunk_length": max(chunk_lengths) if chunk_lengths else 0,
                    "std_dev_length": (
                        statistics.stdev(chunk_lengths) if len(chunk_lengths) > 1 else 0
                    ),
                    "chunks_exceeding_512_tokens": sum(
                        1 for length in chunk_lengths if length > 1920
                    ),  # ~512 tokens
                    "chunks_exceeding_384_tokens": sum(
                        1 for length in chunk_lengths if length > 1440
                    ),  # ~384 tokens
                    "total_chunked_length": sum(chunk_lengths),
                    "coverage_ratio": (
                        sum(chunk_lengths) / len(text) if len(text) > 0 else 0
                    ),
                }
    ...
    def store_chunks(
        self, chunk_records: List[Dict[str, Any]], connection=None
    ) -> bool:
    ...
    def process_sample_documents(
        self, limit: int = 10, strategy_names: List[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {
                ...
                "strategy_results": {
                    name: {"chunks": 0, "documents": 0} for name in strategy_names
                },
                "analysis_results": [],
                "errors": [],
            }
                ...
                try:
                    ...
                    for strategy_name in strategy_names:
                        try:
                            ...
                            error_msg = (
                                f"Error processing {doc_id} with {strategy_name}: {e}"
                            )
...
def main():
    ...
    results = service.process_sample_documents(
        limit=5, strategy_names=["fixed_size", "semantic", "hybrid"]
    )
    ...
    for strategy, stats in results["strategy_results"].items():
        ...
        print(
            f"  {strategy}: {stats['chunks']} chunks from {stats['documents']} docs (avg: {avg_chunks:.1f} chunks/doc)"
        )
    ...
    for i, analysis in enumerate(results["analysis_results"][:3]):
        ...
        print(
            f"    Length: {doc_info['original_length']:,} chars, {doc_info['word_count']} words"
        )
