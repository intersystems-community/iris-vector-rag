...
class NodeRAGPipeline:
    ...
    def _identify_initial_search_nodes(self, query_text: str, top_n_seed: int = 5, similarity_threshold: float = 0.1) -> List[str]: # Returns list of node_ids
                ...
                if row_count == 0:
                    ...
                    try:
                        ...
                        vector_search_sql = f"""
                            ...
                            ORDER BY score DESC
                        """
                        logger.info(f"NodeRAG: Testing vector search with simple test vector: {test_vector_str[:60]}...")
        ...
        try:
            ...
            if use_source_docs:
                params = (iris_vector_str,)
            else:
                params = (iris_vector_str, iris_vector_str, similarity_threshold)
            ...
            try:
                ...
                logger.warning(f"NodeRAG: Vector function error during main query: {vector_error}")
                ...
                ORDER BY score DESC
                """
                logger.warning(f"NodeRAG: Attempting fallback SQL query due to: {vector_error}")
                cursor.execute(fallback_sql, (iris_vector_str,))
            
            # Fetch results
            fetched_rows = cursor.fetchall()
            scores: List[float] = [] # To store scores for potential Python-side filtering
            
            # Format results into list of node_ids, applying threshold
            if fetched_rows:
                for row_tuple in fetched_rows: # row_tuple is (node_id, score)
                    node_id, score_val = row_tuple
                    if score_val is not None and score_val >= similarity_threshold:
                        node_ids.append(str(node_id)) # Ensure node_id is string
                        scores.append(score_val)
                    elif score_val is None:
                         logger.debug(f"NodeRAG: Node {node_id} had NULL score, skipping.")
            
            logger.info(f"NodeRAG: Identified {len(node_ids)} initial search nodes after thresholding.")
            
            # If you need to return nodes with their scores, you'd adjust the return type
            # For now, it returns List[str] of node_ids
            return node_ids
    ...
    def _retrieve_content_for_nodes(self, node_ids: Set[str]) -> List[Document]:
        ...
        try:
            ...
            for row in results:
                node_id = row[0]
                content = row[1] # description_text or text_content
                if hasattr(content, 'read'): # Check if it's a stream
