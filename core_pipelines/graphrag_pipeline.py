    ...
    def _traverse_knowledge_graph(self, seed_entities: List[Tuple[str, str, float]],
        ...
        try:
            ...
            for depth in range(max_depth):
                ...
                all_relevant_list_for_not_in_clause = list(relevant_entities)

                if not entity_list_for_in_clause:
                    logger.debug("GraphRAG DEBUG: entity_list_for_in_clause is empty, cannot build query for traversal.")
                    break
                
                placeholders_in = ','.join(['?' for _ in entity_list_for_in_clause])
                
                not_in_clause_target_sql = ""
                not_in_clause_source_sql = ""
                
                # Parameters start with those for the IN clauses
                params_for_traversal = entity_list_for_in_clause + entity_list_for_in_clause
                
                if all_relevant_list_for_not_in_clause:
                    placeholders_not_in = ','.join(['?' for _ in all_relevant_list_for_not_in_clause])
                    not_in_clause_target_sql = f"AND r.target_entity_id NOT IN ({placeholders_not_in})"
                    not_in_clause_source_sql = f"AND r.source_entity_id NOT IN ({placeholders_not_in})"
                    # Append parameters for the NOT IN clauses
                    # The order is: IN_list1, NOT_IN_list1, IN_list2, NOT_IN_list2
                    params_for_traversal = entity_list_for_in_clause + all_relevant_list_for_not_in_clause + \
                                           entity_list_for_in_clause + all_relevant_list_for_not_in_clause
                ...
                if limit_val > 0:
                    traversal_query = f"""
                        SELECT TOP {limit_val} * FROM (
                            ...
                            WHERE r.source_entity_id IN ({placeholders_in})
                              {not_in_clause_target_sql}
                            ...
                            WHERE r.target_entity_id IN ({placeholders_in})
                              {not_in_clause_source_sql}
                              AND e.entity_type IN ('PERSON', 'ORG', 'DISEASE', 'DRUG', 'TREATMENT', 'SYMPTOM')
                        ) AS CombinedResults
                    """
                    logger.debug(f"GraphRAG DEBUG: _traverse_knowledge_graph query: {traversal_query.strip()} with params length: {len(params_for_traversal)}")
...
if __name__ == '__main__':
    # This demo is more complex due to KG setup.
    # For a simple run, ensure your IRIS instance has the RAG.Entities and RAG.Relationships tables populated.
    print("Testing GraphRAG Pipeline...")
    
    try:
        # Setup
        db_conn = get_iris_connection() # Uses environment variables
        embed_fn = get_embedding_func()
        llm_fn = get_llm_func(provider="stub") # Use stub for testing

        pipeline = create_graphrag_pipeline(iris_connector=db_conn, llm_func=llm_fn)

        # Example Query
        test_query = "What are treatments for COVID-19?"
        ...
        for i, doc in enumerate(result['retrieved_documents']):
            print(f"  Doc {i+1}: ID={doc['id']}, Score={doc.get('score', 0):.4f}, Content='{doc['content'][:60]}...'")
    ...
    print("\nGraphRAG Pipeline Test Finished.")