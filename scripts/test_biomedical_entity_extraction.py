        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Using general spacy model - biomedical entity recognition may be limited"
                    )
                    logger.info(
                        "Install scispacy for better results: pip install scispacy"
                    )
                    logger.info(
    ...
    for i, doc in enumerate(documents):
        ...
        key_concepts = {
            ...
            "diseases": [
                ...
                if any(
                    word in t.lower()
                    for word in ["cancer", "disease", "disorder", "syndrome"]
                )
            ],
            "mutations": [
                t
                ...
                if any(
                    word in t.lower()
                    for word in ["mutation", "variant", "polymorphism"]
                )
            ],
            "treatments": [
                t
                for t in terms
                if any(word in t.lower() for word in ["therapy", "treatment", "drug"])
            ],
        }
        ...
        for concept_type, concepts in key_concepts.items():
            if concepts:
                logger.info(
                    f"  {concept_type}: {', '.join(concepts[:5])}{'...' if len(concepts) > 5 else ''}"
                )
...
if __name__ == "__main__":
    ...
    logger.info(
        "2. Consider using BioBERT or similar models for domain-specific extraction"
    )
    logger.info(
        "3. Implement custom rules for specific entity types (genes, diseases, drugs)"
    )
