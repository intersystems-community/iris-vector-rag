...
class PMCDatasetLoader:
    ...
    def download_documents(self, resume: bool = True) -> Dict[str, Any]:
        ...
        logger.info(
            f"🚀 Starting PMC document download (limit: {self.config.document_limit})"
        )
        ...
        if existing_count < self.config.document_limit:
            ...
            if pmc_ids:
                ...
                batch_size = min(
                    self.config.batch_size // 10, 20
                )  # Smaller batches for downloading

                for i in range(0, min(len(pmc_ids), needed), batch_size):
                    batch = pmc_ids[i : i + batch_size]

                    logger.info(
                        f"Downloading batch {i//batch_size + 1}: {len(batch)} documents"
                    )
                    ...
                    for pmc_id in batch:
                        ...
                        if progress["downloaded_count"] % 50 == 0:
                            logger.info(
                                f"Downloaded {progress['downloaded_count']} documents"
                            )
    ...
    def process_documents(
        ...
        logger.info(
            f"📄 Starting document processing with chunk_size={self.config.chunk_size}"
        )
        ...
        for i, doc in enumerate(
            process_pmc_files(
                str(self.config.download_dir), limit=self.config.document_limit
            )
        ):
    ...
    def generate_standardized_output(
        ...
        metadata = {
            ...
            "statistics": {
                ...
                "average_content_length": (
                    total_content_length / total_docs if total_docs > 0 else 0
                ),
                "documents_with_chunks": sum(
                    1 for doc in documents if doc.get("chunks")
                ),
                "average_chunks_per_doc": (
                    total_chunks / total_docs if total_docs > 0 else 0
                ),
            },
        ...
        logger.info(
            f"  📄 {self.documents_file.name}: {self.documents_file.stat().st_size / (1024*1024):.1f} MB"
        )
    ...
    def load_to_database(self, resume: bool = True) -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ Database loading complete: {progress['loaded_count']} documents"
        )
        return load_result
    ...
    def run_complete_pipeline(self, resume: bool = True) -> Dict[str, Any]:
        ...
        mode_str = (
            ...
            else f"FULL MODE ({self.config.document_limit} docs)"
        )
        ...
        try:
            ...
            final_result = {
                ...
                "processing_result": {
                    ...
                    "total_chunks": sum(
                        len(doc.get("chunks", [])) for doc in documents
                    ),
                },
            ...
            logger.info(f"📁 Output Directory: {self.config.output_dir}")
            logger.info(
                f"📊 Documents Downloaded: {download_result.get('downloaded_count', 0)}"
            )
            logger.info(f"📄 Documents Processed: {len(documents)}")
            logger.info(
                f"📦 Total Chunks: {final_result['processing_result']['total_chunks']}"
            )
            logger.info(
                f"💾 Documents Loaded: {load_result.get('loaded_doc_count', 0)}"
            )
