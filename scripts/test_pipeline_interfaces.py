    ...
    try:
        ...
        if hasattr(pipeline, 'execute'):
            try:
                ...
                result["execute_test"]["response"] = str(response)[:200] + "..." if len(str(response)) > 200 else str(response)
            except Exception as e:
...
def main():
    ...
    for pipeline_name, pipeline_class in PIPELINE_CLASSES.items():
        ...
        # Analyze interface
        interface_results[pipeline_name] = analyze_pipeline_interface(pipeline_name, pipeline_class)
        
        # Test execution
        execution_results[pipeline_name] = test_pipeline_execution(pipeline_name, pipeline_class)
        ...
        important_methods = ['execute', 'query', 'load_documents', 'ingest', 'retrieve', 'clear']
        ...
        print(f"Execute Test: {'✓ Success' if execution['execute_test']['success'] else '✗ Failed'}")
    ...
    analysis_file = os.path.join(output_dir, f"pipeline_interface_analysis_{timestamp}.json")
    ...
    # Summary statistics
    working_pipelines = [name for name, result in execution_results.items() 
                        if result['initialization']['success'] and result['execute_test']['success']]
    initialized_only = [name for name, result in execution_results.items() 
                       if result['initialization']['success'] and not result['execute_test']['success']]
    broken_pipelines = [name for name, result in execution_results.items() 
                       if not result['initialization']['success']]
    
    print(f"\nWorking Pipelines ({len(working_pipelines)}): {', '.join(working_pipelines) or 'None'}")
    print(f"Initialized Only ({len(initialized_only)}): {', '.join(initialized_only) or 'None'}")
    print(f"Broken Pipelines ({len(broken_pipelines)}): {', '.join(broken_pipelines) or 'None'}")
