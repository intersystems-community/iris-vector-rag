"""
Test script for HyDERAG Pipeline.

This script tests the HyDERAGPipeline implementation by executing a sample query
...
def main():
    ...
    print("Creating HyDERAG Pipeline with Auto-Setup")
    # Create pipeline using iris_rag factory with auto_setup=True
    hyde_rag_pipeline = iris_rag.create_pipeline(
        pipeline_type="hyde",
        llm_func=llm_func,  
        auto_setup=True,     # Crucial: handles schema initialization automatically
        validate_requirements=True
    )
    print("✓ HyDERAG Pipeline created successfully")

    print("Loading data")
    # Load documents from a folder (update the path as necessary)
    doc_path = "../../data/test_txt_docs"
    hyde_rag_pipeline.load_documents(doc_path)

    print("Running HyDERAG Pipeline")
    # Run a sample query
    query = "What are the benefits of using RAG pipelines?"
    response = hyde_rag_pipeline.query(query, top_k=3)

    # Print final answer
    print("\n========== HyDERAG Pipeline Output ==========")
    ...
    try:
        ...
        connection = hyde_rag_pipeline.connection_manager.get_connection()
