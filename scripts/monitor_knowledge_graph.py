...
class KnowledgeGraphMonitor:
    ...
    def query_total_counts(self, client):
        ...
        try:
            ...
            entity_total = client.query(
                "SELECT COUNT(*) as total_count FROM RAG.Entities"
            )
            if entity_total:
                total_stats["total_entities"] = entity_total[0]["total_count"]

            # Total relationships
            rel_total = client.query(
                "SELECT COUNT(*) as total_count FROM RAG.Relationships"
            )
            if rel_total:
    ...
    def display_monitoring_report(self, entity_data, relationship_data, total_stats):
        ...
        if docs_with_entities > 0:
            ...
            avg_relationships = (
                total_relationships / docs_with_entities
        ...
        if entity_types:
            ...
            for i, etype in enumerate(entity_types[:10]):
                print(
                    f"  {i+1:2d}. {etype['entity_type']:<20} {etype['type_count']:>8,}"
                )
        ...
        if rel_types:
            ...
            for i, rtype in enumerate(rel_types[:10]):
                print(
                    f"  {i+1:2d}. {rtype['relationship_type']:<20} {rtype['type_count']:>8,}"
                )
            ...
            while True:
                ...
                try:
                    with client:
                        ...
                        # Display report
                        self.display_monitoring_report(
                            entity_data, relationship_data, total_stats
                        )
