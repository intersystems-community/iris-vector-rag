...
class GraphRAGBenchmark:
    ...
    def run_query_benchmark(self, pipeline: GraphRAGMergedPipeline, 
                          query: str, iterations: int = 1) -> Dict:
        ...
        if valid_times:
            stats = {
                ...
                'p95_response_time': statistics.quantiles(valid_times, n=20)[18] if len(valid_times) > 5 else max(valid_times),
                'p99_response_time': statistics.quantiles(valid_times, n=100)[98] if len(valid_times) > 10 else max(valid_times),
    ...
    def run_baseline_benchmark(self, iterations: int = 10) -> List[Dict]:
        ...
        logger.info(f"Running baseline benchmark with {iterations} iterations per query...")
        
        pipeline = self.setup_baseline_pipeline()
        results = []
        
        for i, query in enumerate(self.test_queries):
            logger.info(f"Running baseline query {i+1}/{len(self.test_queries)}: {query[:50]}...")
            result = self.run_query_benchmark(pipeline, query, iterations)
            results.append(result)
            
            # Log individual results
            logger.info(f"Baseline result: {result['avg_response_time']:.1f}ms avg, "
    ...
    def run_optimized_benchmark(self, iterations: int = 10) -> Tuple[List[Dict], Dict]:
        ...
        logger.info(f"Running optimized benchmark with {iterations} iterations per query...")
        ...
        for i, query in enumerate(self.test_queries):
            logger.info(f"Running optimized query {i+1}/{len(self.test_queries)}: {query[:50]}...")
            result = self.run_query_benchmark(pipeline, query, iterations)
            results.append(result)
            
            # Log individual results
            logger.info(f"Optimized result: {result['avg_response_time']:.1f}ms avg, "
                       f"{result['accuracy']:.1%} accuracy")
    ...
    def run_concurrent_load_test(self, concurrent_users: int = 10, 
                                queries_per_user: int = 5) -> Dict:
        """Run concurrent load test."""
        logger.info(f"Running concurrent load test with {concurrent_users} users, "
        ...
        response_times = [r['avg_response_time'] for r in all_results if r['avg_response_time'] != float('inf')]
        ...
        concurrent_metrics = {
            ...
            'p95_response_time': statistics.quantiles(response_times, n=20)[18] if len(response_times) > 5 else float('inf'),
            'error_rate': sum(1 for r in all_results if r['error_rate'] > 0) / len(all_results),
    ...
    def generate_comparison_report(self) -> Dict:
        ...
        def calculate_summary_stats(results: List[Dict]) -> Dict:
            ...
            response_times = [r['avg_response_time'] for r in results if r['avg_response_time'] != float('inf')]
            ...
            if response_times:
                ...
                return {
                    ...
                    'avg_error_rate': statistics.mean(error_rates) if error_rates else 1.0,
                    'total_queries': len(results),
        ...
        if baseline_summary and optimized_summary:
            for key in ['avg_response_time', 'median_response_time', 'p95_response_time', 'p99_response_time']:
                if baseline_summary.get(key, float('inf')) > 0 and baseline_summary.get(key, float('inf')) != float('inf'):
        ...
        if optimized_summary:
            validation_results['response_time_met'] = optimized_summary.get('avg_response_time', float('inf')) < target_validation['response_time_target']
            validation_results['p95_response_time_met'] = optimized_summary.get('p95_response_time', float('inf')) < target_validation['response_time_target'] * 1.5
            
            if baseline_summary and optimized_summary:
                accuracy_change = abs(optimized_summary.get('avg_accuracy', 0) - baseline_summary.get('avg_accuracy', 0)) * 100
                validation_results['accuracy_maintained'] = accuracy_change <= target_validation['accuracy_loss_tolerance']
    ...
    def generate_markdown_report(self, report: Dict) -> str:
        ...
        with open(md_path, 'w') as f:
            ...
            if baseline and optimized:
                f.write(f"- **Baseline Performance:** {baseline.get('avg_response_time', 'N/A'):.1f}ms average\n")
                f.write(f"- **Optimized Performance:** {optimized.get('avg_response_time', 'N/A'):.1f}ms average\n")
                
                improvements = report.get('improvements', {})
                if 'avg_response_time' in improvements:
                    imp = improvements['avg_response_time']
                    f.write(f"- **Performance Improvement:** {imp.get('improvement_pct', 0):.1f}% ({imp.get('improvement_ms', 0):.1f}ms reduction)\n")
            
            f.write(f"- **Target <200ms:** {'✅ ACHIEVED' if validation.get('response_time_met', False) else '❌ NOT MET'}\n")
            f.write(f"- **Accuracy Maintained:** {'✅ YES' if validation.get('accuracy_maintained', False) else '❌ NO'}\n\n")
            ...
            if baseline and optimized:
                ...
                for metric in metrics:
                    ...
                    if metric in report.get('improvements', {}):
                        imp_pct = report['improvements'][metric].get('improvement_pct', 0)
                        f.write(f"| {metric.replace('_', ' ').title()} | {baseline_val:.1f}ms | {optimized_val:.1f}ms | {imp_pct:.1f}% |\n")
                    else:
                        f.write(f"| {metric.replace('_', ' ').title()} | {baseline_val:.1f}ms | {optimized_val:.1f}ms | N/A |\n")
            
            # Target Validation
            f.write("\n### Target Validation\n\n")
            targets = report.get('target_validation', {})
            f.write(f"- **Response Time Target:** <{targets.get('response_time_target', 200)}ms\n")
            f.write(f"- **Cache Hit Rate Target:** >{targets.get('cache_hit_rate_target', 60)}%\n")
            f.write(f"- **Memory Usage Target:** <{targets.get('memory_usage_target', 2048)}MB\n")
            f.write(f"- **Accuracy Loss Tolerance:** <{targets.get('accuracy_loss_tolerance', 5)}%\n\n")
            ...
            if self.baseline_results:
                ...
                for result in self.baseline_results:
                    ...
                    f.write(f"| {query_short} | {result.get('avg_response_time', 0):.1f}ms | {result.get('p95_response_time', 0):.1f}ms | {result.get('accuracy', 0):.1%} |\n")
            ...
            f.write("\n## Conclusion\n\n")
            if validation.get('response_time_met', False) and validation.get('accuracy_maintained', False):
                f.write("✅ **SUCCESS:** All optimization targets achieved with maintained accuracy.\n")
            elif validation.get('response_time_met', False):
                f.write("⚠️ **PARTIAL SUCCESS:** Response time target achieved but accuracy may be impacted.\n")
            else:
                f.write("❌ **NEEDS IMPROVEMENT:** Response time target not achieved. Additional optimization required.\n")
...
def main():
    ...
    parser.add_argument("--optimized-only", action="store_true", help="Run optimized only")
    parser.add_argument("--concurrent-test", action="store_true", help="Run concurrent load test")
    ...
    try:
        ...
        if baseline_summary and optimized_summary:
            logger.info(f"Baseline average: {baseline_summary.get('avg_response_time', 'N/A'):.1f}ms")
            logger.info(f"Optimized average: {optimized_summary.get('avg_response_time', 'N/A'):.1f}ms")
            
            improvements = report.get('improvements', {})
            if 'avg_response_time' in improvements:
                imp = improvements['avg_response_time']
                logger.info(f"Improvement: {imp.get('improvement_pct', 0):.1f}% ({imp.get('improvement_ms', 0):.1f}ms reduction)")
        
        logger.info(f"Target <200ms: {'✅ ACHIEVED' if validation.get('response_time_met', False) else '❌ NOT MET'}")
        logger.info(f"Accuracy maintained: {'✅ YES' if validation.get('accuracy_maintained', False) else '❌ NO'}")
