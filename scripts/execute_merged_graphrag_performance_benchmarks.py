...
try:
    ...
    print("Warning: Monitoring packages not available. Install with: pip install psutil matplotlib seaborn numpy")
...
class PerformanceBenchmarkSuite:
    ...
    def __init__(self, config_path: Optional[str] = None, use_mocks: bool = False, output_dir: str = "outputs/performance_benchmarks"):
        """
        ...
        self.parallel_test_script = self.scripts_dir / "test_merged_graphrag_comprehensive.py"
        self.multihop_demo_script = self.scripts_dir / "test_merged_graphrag_multihop_demo.py"
        self.ragas_eval_script = self.scripts_dir / "test_graphrag_ragas_evaluation.py"
        
        logger.info(f"Performance Benchmark Suite initialized (mock_mode={self.use_mocks})")
    ...
    def run_parallel_testing_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                results_files = list((self.output_dir / "parallel_tests").glob("graphrag_comparison_results_*.json"))
                if results_files:
    ...
    def run_multihop_demo_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Try to load results file
                results_files = list((self.output_dir / "multihop_demo").glob("merged_graphrag_multihop_report_*.json"))
                if results_files:
                    with open(results_files[-1], 'r') as f:
                        test_results = json.load(f)
                else:
    ...
    def run_ragas_evaluation_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Try to load results file
                results_files = list((self.output_dir / "ragas_evaluation").glob("ragas_evaluation_report_*.json"))
                if results_files:
                    with open(results_files[-1], 'r') as f:
                        test_results = json.load(f)
                else:
    ...
    def extract_performance_metrics(self, test_results: Dict[str, Any], test_name: str) -> List[PerformanceMetrics]:
        ...
        if test_name == "parallel_testing" and "test_results" in test_results:
            comparison_report = test_results["test_results"].get("comparison_report", {})
            ...
            for impl, results in implementations.items():
                ...
                if successful_results:
                    ...
                    metrics.append(PerformanceMetrics(
                        ...
                        response_time_p95=np.percentile(execution_times, 95) if MONITORING_AVAILABLE else max(execution_times),
                        response_time_p99=np.percentile(execution_times, 99) if MONITORING_AVAILABLE else max(execution_times),
                        total_queries=len(results),
                        successful_queries=len(successful_results),
                        failed_queries=len(results) - len(successful_results),
                        queries_per_second=len(successful_results) / (sum(execution_times) / 1000) if execution_times else 0,
                        total_db_executions=sum(r.get("db_exec_count", 0) for r in successful_results),
                        average_confidence=statistics.mean([r.get("confidence_score", 0) for r in successful_results]),
                        average_documents_retrieved=statistics.mean([r.get("retrieved_documents", 0) for r in successful_results]),
            ...
            for impl, stats in perf_summary.items():
                metrics.append(PerformanceMetrics(
                    ...
                    average_entities_traversed=stats.get("average_entities_traversed", 0),
                    **test_results.get("system_metrics", {})
                ))
            ...
            pipeline_performance = test_results["test_results"].get("pipeline_performance", {})
    ...
    def run_comprehensive_benchmarks(self) -> BenchmarkResult:
        ...
        if parallel_results["success"]:
            parallel_metrics = self.extract_performance_metrics(parallel_results, "parallel_testing")
        ...
        if multihop_results["success"]:
            multihop_metrics = self.extract_performance_metrics(multihop_results, "multihop_demo")
        ...
        if ragas_results["success"]:
            ragas_metrics = self.extract_performance_metrics(ragas_results, "ragas_evaluation")
        ...
        implementation_comparison = self._analyze_implementation_performance(all_metrics)
        regression_analysis = self._analyze_regressions(all_metrics)
        recommendations = self._generate_performance_recommendations(all_metrics, test_results)
        summary = self._generate_performance_summary(all_metrics, test_results, total_duration)
        ...
        benchmark_result = BenchmarkResult(
            ...
            parallel_test_results=parallel_results if parallel_results["success"] else None,
            multihop_demo_results=multihop_results if multihop_results["success"] else None,
            ragas_evaluation_results=ragas_results if ragas_results["success"] else None,
            summary=summary
        )
    ...
    def _analyze_implementation_performance(self, metrics: List[PerformanceMetrics]) -> Dict[str, Dict[str, float]]:
        ...
        for impl, impl_metrics in by_implementation.items():
            ...
            if successful_metrics:
                comparison[impl] = {
                    "average_response_time_ms": statistics.mean([m.execution_time_ms for m in successful_metrics]),
                    "median_response_time_ms": statistics.median([m.execution_time_ms for m in successful_metrics]),
                    "average_throughput_qps": statistics.mean([m.queries_per_second for m in successful_metrics if m.queries_per_second > 0]),
                    "total_queries": sum([m.total_queries for m in successful_metrics]),
                    "total_successful": sum([m.successful_queries for m in successful_metrics]),
                    "average_error_rate": statistics.mean([m.error_rate for m in successful_metrics]),
                    "peak_memory_mb": max([m.peak_memory_mb for m in successful_metrics if m.peak_memory_mb > 0], default=0),
                    "average_confidence": statistics.mean([m.average_confidence for m in successful_metrics if m.average_confidence > 0])
                }
    ...
    def _analyze_regressions(self, metrics: List[PerformanceMetrics]) -> Dict[str, Any]:
        ...
        if current_metrics and merged_metrics:
            ...
            current_avg_time = statistics.mean([m.execution_time_ms for m in current_metrics])
            merged_avg_time = statistics.mean([m.execution_time_ms for m in merged_metrics])
            
            time_change_pct = ((merged_avg_time - current_avg_time) / current_avg_time) * 100 if current_avg_time > 0 else 0
            
            regressions["performance_changes"]["response_time_change_pct"] = time_change_pct
            
            if time_change_pct > 20:  # 20% slower
                regressions["regressions_found"].append(f"Response time regression: {time_change_pct:.1f}% slower")
            elif time_change_pct < -10:  # 10% faster
                regressions["improvements_found"].append(f"Response time improvement: {abs(time_change_pct):.1f}% faster")
            ...
            if merged_avg_error > current_avg_error + 0.05:  # 5% higher error rate
                regressions["regressions_found"].append(f"Error rate regression: {merged_avg_error:.2%} vs {current_avg_error:.2%}")
            elif merged_avg_error < current_avg_error - 0.02:  # 2% lower error rate
                regressions["improvements_found"].append(f"Error rate improvement: {merged_avg_error:.2%} vs {current_avg_error:.2%}")
        
        return regressions
    
    def _generate_performance_recommendations(self, metrics: List[PerformanceMetrics], test_results: Dict[str, Any]) -> List[str]:
        ...
        if merged_metrics:
            avg_response_time = statistics.mean([m.execution_time_ms for m in merged_metrics])
            avg_error_rate = statistics.mean([m.error_rate for m in merged_metrics])
            avg_memory = statistics.mean([m.peak_memory_mb for m in merged_metrics if m.peak_memory_mb > 0])
            
            # Performance recommendations
            if avg_response_time > 2000:  # >2 seconds
                recommendations.append("⚡ Optimize response times - currently averaging >2 seconds")
            elif avg_response_time < 500:  # <500ms
                recommendations.append("✅ Excellent response times - under 500ms average")
            
            if avg_error_rate > 0.1:  # >10% error rate
                recommendations.append("🐛 High error rate detected - investigate failure causes")
        ...
        if successful_tests == total_tests:
            ...
            recommendations.append(f"⚠️ {total_tests - successful_tests}/{total_tests} tests failed - investigate issues")
        
        # RAGAS evaluation specific
        if "ragas_evaluation" in test_results and test_results["ragas_evaluation"].get("success"):
            ...
            if merged_scores.get("overall_score", 0) >= 0.8:
                ...
                recommendations.append("🎯 RAGAS evaluation target missed (<80%) - quality improvements needed")
        
        return recommendations
    
    def _generate_performance_summary(self, metrics: List[PerformanceMetrics], test_results: Dict[str, Any], total_duration: float) -> str:
        ...
        lines.append(f"- Test success rate: {successful_tests}/{len(test_results)} ({successful_tests/len(test_results)*100:.1f}%)")
        
        # Performance highlights
        if metrics:
            merged_metrics = [m for m in metrics if m.implementation == "merged"]
            if merged_metrics:
                avg_time = statistics.mean([m.execution_time_ms for m in merged_metrics])
    ...
    def _write_markdown_summary(self, file, result: BenchmarkResult) -> None:
        ...
        if result.implementation_comparison:
            ...
            file.write("| Implementation | Avg Response (ms) | Throughput (q/s) | Error Rate | Peak Memory (MB) | Confidence |\n")
            file.write("|----------------|-------------------|------------------|------------|-------------------|-------------|\n")
            
            for impl, stats in result.implementation_comparison.items():
                file.write(f"| {impl} | {stats.get('average_response_time_ms', 0):.1f} | {stats.get('average_throughput_qps', 0):.2f} | {stats.get('average_error_rate', 0):.2%} | {stats.get('peak_memory_mb', 0):.1f} | {stats.get('average_confidence', 0):.2f} |\n")
            file.write("\n")
        
        if result.regression_analysis.get("regressions_found") or result.regression_analysis.get("improvements_found"):
    ...
    def _create_performance_visualizations(self, result: BenchmarkResult, timestamp: str) -> None:
        """Create performance visualization charts."""
        try:
            # Response time comparison
            fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 10))
            fig.suptitle('GraphRAG Performance Benchmark Results', fontsize=16, fontweight='bold')
            
            # 1. Response time comparison
            if result.implementation_comparison:
                implementations = list(result.implementation_comparison.keys())
                response_times = [result.implementation_comparison[impl].get('average_response_time_ms', 0) for impl in implementations]
                
                ax1.bar(implementations, response_times, alpha=0.7, color=['#3498db', '#e74c3c', '#2ecc71'])
            ...
            if result.implementation_comparison:
                error_rates = [result.implementation_comparison[impl].get('average_error_rate', 0) * 100 for impl in implementations]
                
                ax2.bar(implementations, error_rates, alpha=0.7, color=['#3498db', '#e74c3c', '#2ecc71'])
            ...
            # 3. Memory usage
            if result.implementation_comparison:
                memory_usage = [result.implementation_comparison[impl].get('peak_memory_mb', 0) for impl in implementations]
                
                ax3.bar(implementations, memory_usage, alpha=0.7, color=['#3498db', '#e74c3c', '#2ecc71'])
            ...
            # 4. Confidence scores
            if result.implementation_comparison:
                confidence_scores = [result.implementation_comparison[impl].get('average_confidence', 0) for impl in implementations]
                
                ax4.bar(implementations, confidence_scores, alpha=0.7, color=['#3498db', '#e74c3c', '#2ecc71'])
            ...
            # Save visualization
            viz_file = self.output_dir / f"performance_benchmark_visualization_{timestamp}.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Comprehensive GraphRAG Performance Benchmarking")
    parser.add_argument("--config", help="Path to configuration file")
    parser.add_argument("--use-mocks", action="store_true", help="Use mock data instead of real database")
    parser.add_argument("--output-dir", default="outputs/performance_benchmarks", help="Output directory for results")
