            ...
            if isinstance(value, str):
                ...
                for match in matches:
                    ...
                    if replacement is not None:
                        new_value = new_value.replace('${' + match + '}', str(replacement))
                        changed = True
                    else:
...
def test_mem0_with_persistence():
    ...
    try:
        ...
        if not openai_key or re.match(r"^\s*\$\{[^}]+\}\s*$", openai_key):
            print("❌ OPENAI_API_KEY is missing or placeholder. Set a real key in .env or .env.mem0 (e.g., OPENAI_API_KEY=sk-proj-...)")
            return False
        ...
        search_results = m.search(query="validation system health", user_id="validation_test_user")
        ...
        def _normalize_memory_item(item, idx):
            if isinstance(item, dict):
                ...
                _text = item.get("memory") or item.get("text") or item.get("content") or ""
