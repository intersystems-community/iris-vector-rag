...
def main():
    ...
    if sd_exists:
        report["SourceDocuments"]["count_total"] = fetch_int(cur, f"SELECT COUNT(*) FROM {sd_table}")
        report["SourceDocuments"]["count_with_embedding"] = fetch_int(cur, f"SELECT COUNT(*) FROM {sd_table} WHERE embedding IS NOT NULL")
        report["SourceDocuments"]["count_chunks"] = fetch_int(cur, f"SELECT COUNT(*) FROM {sd_table} WHERE doc_id LIKE '%_chunk_%'")
        report["SourceDocuments"]["samples"] = get_samples(cur, sd_table, ["doc_id", "title"], limit=5)
    ...
    if ent_exists:
        report["Entities"]["count_total"] = fetch_int(cur, f"SELECT COUNT(*) FROM {ent_table}")
        report["Entities"]["samples"] = get_samples(cur, ent_table, ["entity_id", "entity_name", "entity_type"], limit=5)
    ...
    if rel_exists:
        report["EntityRelationships"]["count_total"] = fetch_int(cur, f"SELECT COUNT(*) FROM {rel_table}")
        report["EntityRelationships"]["samples"] = get_samples(cur, rel_table, ["relationship_id", "relationship_type"], limit=5)
