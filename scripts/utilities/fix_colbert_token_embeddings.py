...
def create_token_embeddings_table(iris_connector):
    """Create the DocumentTokenEmbeddings table if it doesn't exist."""
    try:
        ...
        logger.error(f"Error creating DocumentTokenEmbeddings table: {e}")
        raise
...
def generate_mock_token_embedding(token: str, dim: int = 128) -> List[float]:
    ...
    # Use token hash to generate consistent embeddings
...
def populate_token_embeddings_for_document(iris_connector, doc_id: str, text_content: str) -> int:
    ...
    try:
        ...
        if cursor.fetchone()[0] > 0:
            logger.debug(f"Token embeddings already exist for {doc_id}")
            cursor.close()
            return 0
        ...
        for i, token in enumerate(tokens):
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (doc_id, i, token, embedding_str, metadata))
...
def main():
    """Main function."""
    logger.info("🚀 Starting ColBERT Token Embeddings Fix...")
    ...
    try:
        ...
        create_token_embeddings_table(iris_connector)
