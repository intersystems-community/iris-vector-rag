#!/usr/bin/env python3
"""
Enterprise RAG Benchmark - Final Working Version
"""
...
class EnterpriseRAGBenchmarkFinal:
    """Final working benchmark with all parameter fixes"""
    ...
    def run_single_query(self, pipeline_name: str, query: str) -> Dict[str, Any]:
        """Run a single query with proper parameters for each pipeline"""
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        # Save results
        results_file = f"benchmark_results_final_{timestamp}.json"
    ...
    def _create_performance_charts(self, techniques: List[str], response_times: List[float], 
        ...
        plt.tight_layout()
        plt.savefig(f"rag_performance_comparison_final_{timestamp}.png", dpi=300, bbox_inches='tight')
        plt.close()
        logger.info(f"✅ Performance charts saved: rag_performance_comparison_final_{timestamp}.png")
    ...
    def _create_performance_spider_chart(self, techniques: List[str], response_times: List[float],
        ...
        fig.update_layout(
            ...
            showlegend=True,
            title="RAG Techniques Performance Comparison - Final Results",
            font=dict(size=14)
        )
        
        fig.write_html(f"rag_spider_chart_final_{timestamp}.html")
        try:
            fig.write_image(f"rag_spider_chart_final_{timestamp}.png", width=800, height=600)
        except Exception as e:
            logger.warning(f"Could not save PNG: {e}")
        
        logger.info(f"✅ Spider chart saved: rag_spider_chart_final_{timestamp}.html")
...
def main():
    ...
    benchmark = EnterpriseRAGBenchmarkFinal()
