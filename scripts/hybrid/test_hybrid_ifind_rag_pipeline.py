"""
Test script for HybridIFindRAG Pipeline.

This script tests the HybridIFindRAGPipeline implementation by executing a sample query
...
def main():
    ...
    print("✓ HybridIFindRAG Pipeline created successfully")

    print("Loading data")
    # Load documents from a folder (update the path as necessary)
    doc_path = "../../data/test_txt_docs"
    hybrid_ifind_rag_pipeline.load_documents(doc_path)

    print("Running HybridIFindRAG Pipeline")
    # Run a sample query
    query = "What are the benefits of using RAG pipelines?"
    response = hybrid_ifind_rag_pipeline.query(query, top_k=3)

    # Print final answer
    print("\n========== HybridIFindRAG Pipeline Output ==========")
    ...
    try:
        ...
        connection = hybrid_ifind_rag_pipeline.connection_manager.get_connection()
