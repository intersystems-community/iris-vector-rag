...
class Mem0ComprehensiveTest:
    ...
    def test_mem0_basic_functionality(self) -> bool:
        ...
        try:
            ...
            details = {
                ...
                'stored_memory_ids': [str(m.get('id', 'unknown')) for m in stored_memories if isinstance(m, dict)]
            }
            
            success = (len(stored_memories) == len(test_memories) and 
                      len(search_results) > 0 and 
                      len(all_memories) >= len(test_memories))
            ...
            self.log_result("mem0 Basic Operations", success, duration, message, details)
            return success
            ...
            self.log_result("mem0 Basic Operations", False, duration, message, error=error)
            return False
    ...
    def test_mcp_server_availability(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                process_found = 'mem0' in ps_output.stdout and 'server' in ps_output.stdout
            except:
                process_found = False
            ...
            self.log_result("MCP Server Availability", success, duration, message, details)
            return success
            ...
            self.log_result("MCP Server Availability", False, duration, message, error=error)
            return False
    ...
    def test_mcp_tools_via_direct_call(self) -> bool:
        ...
        try:
            ...
            success = (store_result is not None and 
    ...
    def test_memory_persistence_and_retrieval(self) -> bool:
        ...
        try:
            ...
            success = (len(stored_ids) == len(test_data) and
                      all(count > 0 for count in search_results.values()) and
                      all_memories and len(all_memories) >= len(test_data))
            
            if success:
                message = f"Persistence test successful. {len(test_data)} stored, all searches returned results"
            else:
                message = "Persistence test failed - some operations returned no results"
    ...
    def test_performance_and_timeouts(self) -> bool:
        ...
        try:
            ...
            self.log_result("Performance & Timeouts", success, duration, message, details)
            return success
            ...
            self.log_result("Performance & Timeouts", False, duration, message, error=error)
            return False
    ...
    def _get_recommendations(self) -> List[str]:
        ...
        if any(r.test_name == "Environment Setup" and not r.success for r in failed_tests):
            recommendations.append("Configure missing environment variables in .env.mem0")
        ...
        if any(r.test_name == "MCP Server Availability" and not r.success for r in failed_tests):
        ...
        if not failed_tests:
            recommendations.append("All tests passed! mem0 MCP server is functioning correctly")
