...
class RAGASEvaluationOrchestrator:
    ...
    def _setup_logging(self):
        ...
        log_file = self.config.output_dir / f"ragas_evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _initialize_pipelines(self):
        ...
        if not PIPELINES_AVAILABLE:
            raise RuntimeError("Pipeline imports failed - cannot proceed with evaluation")
        
        # Mock connection and config managers for pipeline initialization
        ...
        logger.info(f"Initialized {len(self.pipelines)} pipelines: {list(self.pipelines.keys())}")
    ...
    def _execute_pipeline_query(self, pipeline_name: str, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Pipeline {pipeline_name} failed for query '{query[:50]}...': {e}")
            return error_result
    
    def _execute_pipeline_queries(self, pipeline_name: str, queries: List[str]) -> List[Dict[str, Any]]:
        ...
        if self.config.parallel_execution:
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_query = {
                    executor.submit(self._execute_pipeline_query, pipeline_name, query): query
            ...
            for i, query in enumerate(queries):
                logger.info(f"Processing query {i+1}/{len(queries)} for {pipeline_name}")
                result = self._execute_pipeline_query(pipeline_name, query)
                results.append(result)
        
        successful_queries = sum(1 for r in results if r['success'])
        logger.info(f"{pipeline_name}: {successful_queries}/{len(queries)} queries successful")
    ...
    def _calculate_metric_statistics(self, scores: List[float]) -> Dict[str, float]:
        ...
        if not scores:
            return {
                ...
                'confidence_95': [0.0, 0.0], 'median': 0.0
            }
    ...
    def _evaluate_pipeline_with_ragas(self, pipeline_name: str, 
        ...
        successful_results = [r for r in pipeline_results if r['success'] and r['answer']]
        ...
        try:
            ...
            metrics = {}
            for metric_name in ['answer_correctness', 'faithfulness', 'context_precision', 
    ...
    def _generate_comparative_analysis(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        best_per_metric = {}
        metric_names = ['answer_correctness', 'faithfulness', 'context_precision', 
                       'context_recall', 'answer_relevancy']
        ...
        for pipeline_name, pipeline_metrics in results.items():
            ...
            overall_scores[pipeline_name] = weighted_score / total_weight if total_weight > 0 else 0
        
        best_overall = max(overall_scores.items(), key=lambda x: x[1])[0] if overall_scores else 'None'
...
"""
        ...
        for pipeline_name, metrics in pipeline_metrics.items():
            row_class = 'best' if pipeline_name == comparative_analysis['best_overall'] else ''
            html += f"""
        <tr class="{row_class}">
...
"""
    ...
    def run_comprehensive_evaluation(self) -> Dict[str, Any]:
        ...
        for pipeline_name in self.config.pipelines:
            ...
            metrics = self._evaluate_pipeline_with_ragas(pipeline_name, pipeline_results)
            pipeline_metrics[pipeline_name] = metrics
            
            # Log primary metric
            answer_correctness = metrics.get('answer_correctness', {}).get('mean', 0)
            status = "✅ PASS" if answer_correctness >= self.config.target_accuracy else "❌ FAIL"
            logger.info(f"{pipeline_name} Answer Correctness: {answer_correctness:.3f} {status}")
    ...
    def _print_evaluation_summary(self, results: Dict[str, Any]):
        ...
        logger.info(f"⏱️  Total Execution Time: {metadata['execution_time_minutes']:.1f} minutes")
        ...
        for pipeline_name, metrics in pipeline_metrics.items():
            ...
            status_icon = "✅" if answer_correctness >= metadata['target_accuracy'] else "❌"
            logger.info(f"  {status_icon} {pipeline_name}: {answer_correctness:.3f} (95% CI: [{ci[0]:.3f}, {ci[1]:.3f}])")
        
        logger.info(f"\n🏆 BEST OVERALL PIPELINE: {comparative_analysis['best_overall']}")
        
        # Check if target achieved
        target_achieved = any(
            pm.get('answer_correctness', {}).get('mean', 0) >= metadata['target_accuracy']
            for pm in pipeline_metrics.values()
        )
...
def get_evaluation_config() -> EvaluationConfig:
    ...
    pipelines_str = os.getenv('RAGAS_PIPELINES', 'BasicRAG,BasicRerank,CRAG,GraphRAG')
    pipelines = [p.strip() for p in pipelines_str.split(',')]
    output_dir = Path(os.getenv('RAGAS_OUTPUT_DIR', 'outputs/reports/ragas_evaluations'))
