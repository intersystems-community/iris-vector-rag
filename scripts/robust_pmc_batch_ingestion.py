...
class RobustPMCBatchProcessor:
    ...
    def load_progress(self) -> Dict[str, Any]:
        ...
        if self.progress_file.exists():
            try:
                ...
                self.logger.info(f"📂 Loaded progress: {progress['processed_count']}/{progress['total_files']} files processed")
                return progress
            except Exception as e:
    ...
    def process_single_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            pipeline_result = self.pipeline.process(document["content"], document["metadata"])
            
            # Extract metrics from result
            if pipeline_result and hasattr(pipeline_result, 'metadata'):
                result["entities_extracted"] = pipeline_result.metadata.get("entities_count", 0)
                result["relationships_extracted"] = pipeline_result.metadata.get("relationships_count", 0)
    ...
    def process_batch(self, batch_size: int = 10) -> Dict[str, Any]:
        ...
        self.logger.info(f"🚀 Starting batch processing of {len(pending_files)} files (batch size: {batch_size})")
        ...
        for i, file_path in enumerate(pending_files):
            try:
                ...
                if result["success"]:
                    ...
                    self.progress["total_entities_extracted"] += result["entities_extracted"]
                    self.progress["total_relationships_extracted"] += result["relationships_extracted"]
                else:
                ...
                if (i + 1) % 10 == 0:
                    ...
                    self.logger.info(f"📊 Progress checkpoint: {self.progress['processed_count']}/{self.progress['total_files']} "
                ...
                if (i + 1) % batch_size == 0:
                    ...
                    self.logger.info(f"📈 Batch progress: {i+1}/{len(pending_files)} files "
        ...
        self.logger.info(f"   📊 Total processed: {self.progress['processed_count']}/{self.progress['total_files']}")
        self.logger.info(f"   ✅ Successful: {self.progress['successful_count']}")
        self.logger.info(f"   ❌ Failed: {self.progress['failed_count']}")
        self.logger.info(f"   🏷️ Total entities: {self.progress['total_entities_extracted']}")
        self.logger.info(f"   🔗 Total relationships: {self.progress['total_relationships_extracted']}")
...
def main():
    ...
    if processor.progress["processed_count"] > 0:
        print(f"📂 Resuming from checkpoint: {processor.progress['processed_count']}/{processor.progress['total_files']} files already processed")
        response = input("Continue from checkpoint? (y/n): ").lower()
        if response == 'n':
            processor.progress = processor.load_progress()  # This will reset if file is deleted
    ...
    try:
        ...
        print("=" * 50)
        print(f"📊 Files processed: {final_progress['processed_count']}/{final_progress['total_files']}")
        print(f"✅ Successful: {final_progress['successful_count']}")
        print(f"❌ Failed: {final_progress['failed_count']}")
        print(f"🏷️ Total entities extracted: {final_progress['total_entities_extracted']}")
        print(f"🔗 Total relationships extracted: {final_progress['total_relationships_extracted']}")
