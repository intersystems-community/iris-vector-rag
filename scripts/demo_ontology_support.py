        ...
        config["sources"] = [
            {
                ...
                "domain": "medical",
                "concepts": [
                    {"id": "diabetes", "label": "Diabetes Mellitus", "synonyms": ["diabetes", "DM"]},
                    {"id": "hypertension", "label": "Hypertension", "synonyms": ["high blood pressure", "HTN"]},
                    {"id": "medication", "label": "Medication", "synonyms": ["drug", "medicine"]},
            ...
            },
            {
                ...
                "domain": "technology",
                "concepts": [
                    {"id": "server", "label": "Server", "synonyms": ["host", "machine"]},
                    {"id": "database", "label": "Database", "synonyms": ["DB", "data store"]},
                    {"id": "cpu", "label": "CPU", "synonyms": ["processor", "central processing unit"]},
...
def demonstrate_general_ontology_plugin(ontology_config: Dict[str, Any]):
    ...
    try:
        ...
        if ontology_config.get("sources"):
            ...
            print(f"\nSample concepts:")
            for i, (concept_id, concept) in enumerate(plugin.hierarchy.concepts.items()):
                if i >= 10:
                    break
...
def demonstrate_entity_extraction(documents: List[Document], ontology_config: Dict[str, Any]):
    ...
    try:
        ...
        if plugin.hierarchy.concepts:
            ...
            if test_concept:
                ...
                descendants = plugin.hierarchy.get_descendants(test_concept.id, max_depth=2)
...
def demonstrate_graphrag_integration(documents: List[Document], ontology_config: Dict[str, Any]):
    ...
    try:
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            try:
                ...
                print(f"Basic query found {basic_result['metadata']['num_retrieved']} documents")
...
def run_performance_tests(ontology_config: Dict[str, Any]):
    ...
    try:
        ...
        print(f"  - Loading rate: {total_concepts / max(load_time, 0.001):.0f} concepts/second")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Demonstrate general-purpose ontology support capabilities')
    parser.add_argument('--ontology', type=str, 
                       help='Path to ontology file (OWL/RDF/SKOS/TTL/N3)')
    ...
    parser.add_argument('--skip-graphrag', action='store_true',
                       help='Skip GraphRAG integration demo')
    parser.add_argument('--domain', type=str,
                       help='Filter documents by domain for focused testing')
    ...
    if args.domain:
        documents = [doc for doc in documents if doc.metadata.get('domain') == args.domain]
