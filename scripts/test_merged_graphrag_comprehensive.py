...
try:
    ...
    print("Warning: Visualization packages not available. Install with: pip install pandas matplotlib seaborn")
...
class GraphRAGTestHarness:
    ...
    def __init__(self, config_path: Optional[str] = None, use_mocks: bool = False, output_dir: str = "outputs"):
        """
    ...
    def create_test_suite(self) -> List[TestCase]:
        ...
        return [
            ...
            TestCase(
                ...
                expected_entities=["diabetes", "treatments", "mechanisms", "side effects"],
    ...
    def execute_test_case(self, test_case: TestCase, implementation: str, pipeline) -> TestResult:
    ...
    def _create_mock_pipeline(self, implementation_type: str):
        ...
        class MockPipeline:
            ...
            def query(self, query_text: str, **kwargs) -> Dict[str, Any]:
                ...
                mock_docs = [
                    {"id": f"mock_doc_{i}", "content": f"Mock content for {query_text[:50]}..."}
                    for i in range(random.randint(1, 4))
                ]
                
                base_time = random.uniform(100, 1000)
                variation = 1.2 if self.impl_type == "merged" else 1.0  # Merged might be slightly slower
                ...
                return {
                    ...
                    "sources": [{"document_id": doc["id"], "source": "mock"} for doc in mock_docs],
                    "metadata": {
                        "num_retrieved": len(mock_docs),
                        "processing_time_ms": base_time * variation,
                        "pipeline_type": f"graphrag_{self.impl_type}",
                        "retrieval_method": "knowledge_graph_traversal" if random.random() > 0.2 else "vector_fallback",
    ...
    def run_comprehensive_tests(self) -> ComparisonReport:
        ...
        for test_case in test_cases:
            ...
            current_result = self.execute_test_case(test_case, "current", current_pipeline)
        ...
        self.comparison_report = self._generate_comparison_report(test_cases, all_results)
    ...
    def _generate_comparison_report(self, test_cases: List[TestCase], results: List[TestResult]) -> ComparisonReport:
        ...
        current_success_rate = sum(1 for r in current_results if r.success) / len(current_results)
        merged_success_rate = sum(1 for r in merged_results if r.success) / len(merged_results)
        
        # Performance comparison
        current_avg_time = statistics.mean([r.execution_time_ms for r in current_results if r.success])
        merged_avg_time = statistics.mean([r.execution_time_ms for r in merged_results if r.success])
        
        current_avg_docs = statistics.mean([r.retrieved_documents for r in current_results if r.success])
        merged_avg_docs = statistics.mean([r.retrieved_documents for r in merged_results if r.success])
        
        performance_comparison = {
            "average_execution_time_ms": {
                "current": current_avg_time,
                "merged": merged_avg_time,
                "improvement_percentage": ((current_avg_time - merged_avg_time) / current_avg_time) * 100
            },
            ...
            "average_db_executions": {
                "current": statistics.mean([r.db_exec_count for r in current_results if r.success]),
                "merged": statistics.mean([r.db_exec_count for r in merged_results if r.success])
            }
        }
        ...
        quality_comparison = {
            ...
            "average_answer_length": {
                "current": statistics.mean([r.answer_length for r in current_results if r.success and r.answer_length > 0]),
                "merged": statistics.mean([r.answer_length for r in merged_results if r.success and r.answer_length > 0])
            }
        }
        ...
        for test_case in test_cases:
            current_result = next((r for r in current_results if r.test_id == test_case.id), None)
            merged_result = next((r for r in merged_results if r.test_id == test_case.id), None)
            
            if current_result and merged_result:
                # Check for regressions
                if current_result.success and not merged_result.success:
                    regressions.append(f"{test_case.id}: Merged implementation failed where current succeeded")
                elif current_result.success and merged_result.success:
                    if merged_result.execution_time_ms > current_result.execution_time_ms * 1.5:
                        regressions.append(f"{test_case.id}: Significant performance regression (50%+ slower)")
                
                # Check for improvements
                if not current_result.success and merged_result.success:
                    improvements.append(f"{test_case.id}: Merged implementation succeeded where current failed")
                elif current_result.success and merged_result.success:
                    if merged_result.execution_time_ms < current_result.execution_time_ms * 0.8:
                        improvements.append(f"{test_case.id}: Performance improvement (20%+ faster)")
                    if merged_result.retrieved_documents > current_result.retrieved_documents:
                        improvements.append(f"{test_case.id}: Better document retrieval ({merged_result.retrieved_documents} vs {current_result.retrieved_documents})")
    ...
    def _save_results(self):
        ...
        with open(results_file, 'w') as f:
            json.dump({
                ...
                "comparison_report": asdict(self.comparison_report) if self.comparison_report else None
            }, f, indent=2)
    ...
    def _write_markdown_report(self, file):
        ...
        file.write(f"- **Average Execution Time:** Current: {perf['average_execution_time_ms']['current']:.1f}ms, Merged: {perf['average_execution_time_ms']['merged']:.1f}ms\n")
        file.write(f"- **Performance Change:** {perf['average_execution_time_ms']['improvement_percentage']:+.1f}%\n")
        file.write(f"- **Average Documents Retrieved:** Current: {perf['average_documents_retrieved']['current']:.1f}, Merged: {perf['average_documents_retrieved']['merged']:.1f}\n")
        file.write(f"- **Average DB Executions:** Current: {perf['average_db_executions']['current']:.1f}, Merged: {perf['average_db_executions']['merged']:.1f}\n\n")
        
        file.write("## Quality Comparison\n\n")
        quality = report.quality_comparison
        file.write(f"- **Success Rates:** Current: {quality['success_rates']['current']:.2%}, Merged: {quality['success_rates']['merged']:.2%}\n")
        file.write(f"- **Success Rate Change:** {quality['success_rates']['improvement']:+.2%}\n")
        file.write(f"- **Average Answer Length:** Current: {quality['average_answer_length']['current']:.0f}, Merged: {quality['average_answer_length']['merged']:.0f}\n\n")
        ...
        current_results = [r for r in report.detailed_results if r.implementation == "current"]
        merged_results = [r for r in report.detailed_results if r.implementation == "merged"]
        
        file.write("| Test ID | Current Success | Current Time (ms) | Merged Success | Merged Time (ms) | Performance Change |\n")
        file.write("|---------|----------------|-------------------|---------------|------------------|-------------------|\n")
        
        for current_result in current_results:
            merged_result = next((r for r in merged_results if r.test_id == current_result.test_id), None)
            if merged_result:
                current_time = current_result.execution_time_ms if current_result.success else "FAIL"
                merged_time = merged_result.execution_time_ms if merged_result.success else "FAIL"
                
                if current_result.success and merged_result.success:
                    perf_change = ((current_result.execution_time_ms - merged_result.execution_time_ms) / current_result.execution_time_ms) * 100
                    perf_str = f"{perf_change:+.1f}%"
                else:
                    perf_str = "N/A"
                
                file.write(f"| {current_result.test_id} | {current_result.success} | {current_time} | {merged_result.success} | {merged_time} | {perf_str} |\n")


def main():
    """Main entry point for the comprehensive testing harness."""
    parser = argparse.ArgumentParser(description="Comprehensive GraphRAG Implementation Testing")
    ...
    try:
        ...
        # Recommendation
        if report.merged_success_rate >= report.current_success_rate and len(report.regressions) == 0:
