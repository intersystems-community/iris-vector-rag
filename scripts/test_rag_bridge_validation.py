...
from adapters.rag_templates_bridge import RAGTemplatesBridge, RAGTechnique, CircuitBreakerState
...
class RAGBridgeValidator:
    ...
    async def test_performance_requirements(self, bridge: RAGTemplatesBridge) -> bool:
        ...
        try:
            ...
            if slo_compliant:
                print(f"✅ Performance SLO met - P95: {p95_latency:.2f}ms (target: <500ms)")
                self.results["performance"]["passed"] = True
                return True
            else:
                print(f"❌ Performance SLO failed - P95: {p95_latency:.2f}ms (target: <500ms)")
                return False
    ...
    async def test_all_techniques(self, bridge: RAGTemplatesBridge) -> bool:
        ...
        try:
            ...
            all_accessible = all(
                ...
                for result in techniques_tested.values()
            )
            
            if all_accessible:
                print(f"✅ All RAG techniques accessible: {list(techniques_tested.keys())}")
                self.results["techniques"]["passed"] = True
                return True
            else:
                failed = [t for t, r in techniques_tested.items() if not r.get("accessible", False)]
    ...
    async def test_configuration_management(self, bridge: RAGTemplatesBridge) -> bool:
        ...
        try:
            ...
            required_metric_keys = ["total_queries", "successful_queries", "slo_compliance"]
