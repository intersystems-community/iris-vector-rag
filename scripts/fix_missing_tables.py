...
def create_missing_tables():
    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            # Create indexes
            cursor.execute("CREATE INDEX idx_chunked_docs_doc_id ON RAG.ChunkedDocuments (document_id)")
            cursor.execute("CREATE INDEX idx_chunked_docs_type ON RAG.ChunkedDocuments (chunk_type)")
        ...
        if cursor.fetchone()[0] == 0:
            ...
            # Create indexes
            cursor.execute("CREATE INDEX idx_token_embeddings_doc ON RAG.DocumentTokenEmbeddings (doc_id)")
