...
def fix_embeddings():
    ...
    try:
        ...
        logger.info(
            "Using embedding model: sentence-transformers/all-MiniLM-L6-v2 (384 dims)"
        )
        ...
        for i, (doc_id, text_content) in enumerate(documents):
            ...
            try:
                ...
                if len(embedding) != 384:
                    logger.error(
                        f"Unexpected embedding dimension: {len(embedding)} for doc {doc_id}"
                    )
                    continue
        ...
        connection.commit()
        logger.info(
            f"Successfully re-embedded {success_count}/{len(documents)} documents"
        )
