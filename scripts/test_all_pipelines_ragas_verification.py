...
class ComprehensivePipelineRAGASTest:
    ...
    def test_pipeline_imports(self) -> Dict[str, bool]:
        ...
        pipeline_imports = {
            ...
            "BasicRAGReranking": (
                "iris_rag.pipelines.basic_rerank",
                "BasicRAGRerankingPipeline",
            ),
            "PyLateColBERT": (
                "iris_rag.pipelines.colbert_pylate.pylate_pipeline",
        ...
        for pipeline_name, pipeline_path in pipeline_classes.items():
            ...
            try:
                ...
                if hasattr(pipeline, "fallback_used"):
                    ...
                    logger.warning(
                        f"    ⚠️  {pipeline_name}: Fallback mechanism detected!"
                    )

                # Check for mock indicators
                if hasattr(pipeline, "MockColBERTPipeline") or hasattr(
                    pipeline, "_mock_"
                ):
                ...
                if "pylate" in str(e).lower() and pipeline_name == "PyLateColBERT":
                    ...
                    logger.info(
                        f"    ✓ {pipeline_name}: Correctly fails hard without PyLate (expected)"
                    )
                else:
                    result["error_type"] = "ImportError"
                    result["error_message"] = str(e)
                                ...
                                with open(file_path, "r", encoding="utf-8") as f:
                                    ...
                                    for pattern in mock_patterns:
                                        if pattern in content:
                                            ...
                                            logger.warning(
                                                f"  ⚠️  Found '{pattern}' in {file_path}"
                                            )
                            except Exception as e:
    ...
    def test_pipeline_factory_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {
                ...
                "all_pipelines_registered": all(
                    name in pipeline_names for name in expected_pipelines
                ),
                "colbert_config": None,
            }
            ...
            for pipeline in pipelines:
                if pipeline["name"] == "ColBERT":
                    ...
                    if (
                        ...
                        logger.info(
                            "  ✓ ColBERT pipeline correctly configured for PyLate"
                        )
                    else:
                        logger.warning(
                            f"  ⚠️  ColBERT pipeline module: {pipeline['module']}"
                        )
                    break
    ...
    def _generate_summary(
        ...
        mocks_detected = sum(
            1 for result in instantiation_results.values() if result["mock_detected"]
        )

        mock_patterns_found = sum(
            1 for found in mock_detection_results.values() if found
        )

        pylate_fail_hard = instantiation_results.get("PyLateColBERT", {}).get(
            "fail_hard_verified", False
        )
        ...
        return {
            "pipeline_imports": {
                ...
                "success_rate": (
                    successful_imports / total_pipelines if total_pipelines > 0 else 0
                ),
            },
            "pipeline_instantiation": {
                ...
                "total": total_pipelines,
                "success_rate": (
                    successful_instantiations / total_pipelines
            ...
            "pylate_verification": {
                ...
                "no_fallback_logic": not instantiation_results.get(
                    "PyLateColBERT", {}
                ).get("fallback_detected", True),
            },
            "overall_status": {
                ...
                "verification_passed": (
                    fallbacks_detected == 0
                    and mocks_detected == 0
                    and mock_patterns_found == 0
                    and pylate_fail_hard
                ),
            },
        }
    ...
    def _print_summary(self, results: Dict[str, Any]):
        ...
        logger.info(
            f"📦 Pipeline Imports: {imports['successful']}/{imports['total']} successful ({imports['success_rate']:.1%})"
        )

        # Instantiation status
        instantiation = summary["pipeline_instantiation"]
        logger.info(
            f"🏗️  Pipeline Instantiation: {instantiation['successful']}/{instantiation['total']} successful ({instantiation['success_rate']:.1%})"
        )

        # Fallback detection
        fallback = summary["fallback_detection"]
        logger.info(
            f"🚫 Fallback Detection: {fallback['fallbacks_detected']} fallbacks found, {fallback['clean_pipelines']} clean pipelines"
        )

        # Mock detection
        mock = summary["mock_detection"]
        logger.info(
            f"🎭 Mock Detection: {mock['mocks_in_pipelines']} mocks in pipelines, {mock['mock_patterns_in_code']} patterns in code"
        )

        # PyLate verification
        pylate = summary["pylate_verification"]
        logger.info(
            f"⚡ PyLate Verification: Fail-hard confirmed: {pylate['fail_hard_confirmed']}, No fallback: {pylate['no_fallback_logic']}"
        )
        ...
        logger.info(
            f"   • PyLate properly implemented: {'✓' if overall['pylate_properly_implemented'] else '✗'}"
        )
        logger.info(
            f"   • Overall verification: {'✅ PASSED' if overall['verification_passed'] else '❌ FAILED'}"
        )
        ...
        if overall["verification_passed"]:
            ...
            logger.info(
                "   • PyLate ColBERT correctly fails hard when dependencies unavailable"
            )
...
def main():
    ...
    # Save results
    output_file = (
        f"outputs/pipeline_verification_{int(datetime.now().timestamp())}.json"
    )
