...
def run_pytest_benchmarks(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    cmd = [
        ...
        str(benchmark_dir),
        '--benchmark-only',
        '--benchmark-json', str(results_dir / 'benchmark_results.json'),
        '--benchmark-sort=mean',
        '--benchmark-warmup=on',
...
def generate_report(results: Dict[str, Any], comparison: Dict[str, Any], output_file: Path) -> None:
...
def main():
    ...
    parser.add_argument('--type', choices=['pytest', 'custom', 'all'], default='all',
