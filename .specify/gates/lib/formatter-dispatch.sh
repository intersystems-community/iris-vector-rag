...
_gates_collect_files() { # <tool> <root>
    # LOCAL PATCH (iris-vector-rag, spec-gates 0.3.3): the upstream loop called
    # gates_policy_list (a jq spawn) twice PER FILE. On this tree (~90k files
    # under .claude/worktrees, .mypy_cache, tools/) that took >5 minutes per
    # tool. The policy lists are hoisted out of the loop and cache/tool
    # directories that no policy could sensibly include are pruned at find
    # time. Behaviour for matched files is unchanged. Proposed upstream.
    local tool="$1" root="$2" f rel inc glob matched excluded
    local includes excludes
    includes="$(gates_policy_list "$tool" include 2>/dev/null || true)"
    excludes="$(gates_policy_list "$tool" exclude 2>/dev/null || true)"
    [[ -z "$includes" ]] && return 0
    while IFS= read -r -d '' f; do
        ...
        while IFS= read -r glob; do
            [[ -z "$glob" ]] && continue
            if _gates_glob_match "$rel" "$glob" || _gates_glob_match "$f" "$glob"; then
                excluded=1
                break
            fi
        done <<<"$excludes"
        ...
        \( -name .git -o -name node_modules -o -name .venv -o -name target -o -name dist \
           -o -name .claude -o -name .mypy_cache -o -name .pytest_cache -o -name .ruff_cache \
           -o -name __pycache__ -o -name .tox -o -name htmlcov \) -prune \
        -o -type f -print0)
}
