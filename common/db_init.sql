...
-- Index name might be schema-qualified automatically or needs to be.
-- Let's assume simple name drop is fine if table is correctly schema'd, or use ON RAG.SourceDocuments if supported.
-- For now, focusing on table creation. If index drop fails later, we'll address it.
...
/*
CREATE VECTOR INDEX idx_hnsw_source_embeddings ON RAG.SourceDocuments (embedding)
WITH ( M = 16, efConstruction = 200, distanceMetric = 'COSINE' );

CREATE VECTOR INDEX idx_hnsw_token_embeddings ON RAG.DocumentTokenEmbeddings (token_embedding)
WITH ( M = 16, efConstruction = 200, distanceMetric = 'COSINE' );

CREATE VECTOR INDEX idx_hnsw_kg_node_embeddings ON RAG.KnowledgeGraphNodes (embedding)
WITH ( M = 16, efConstruction = 200, distanceMetric = 'COSINE' );
*/
