...
def get_iris_connection(config: Optional[Dict[str, Any]] = None, prefer_dbapi: bool = True) -> Any:
    ...
    try:
        ...
        conn = get_iris_dbapi_connection(config)
        ...
        if not prefer_dbapi:
            ...
            try:
                ...
                logger.warning("⚠️ Falling back to JDBC connection - this indicates a DBAPI problem!")
                return conn
            except Exception as jdbc_error:
                logger.error(f"❌ JDBC fallback also failed: {jdbc_error}")
                raise Exception(f"Both DBAPI and JDBC connections failed. DBAPI: {dbapi_error}, JDBC: {jdbc_error}")
        else:
            # If DBAPI fails and we prefer it, this is a critical error
            raise Exception(f"DBAPI connection failed and fallback disabled: {dbapi_error}")
