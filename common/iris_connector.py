    ...
    try:
        ...
        if not prefer_dbapi:
            ...
            try:
                ...
                raise IRISConnectionError(f"Both DBAPI and JDBC connections failed. DBAPI: {dbapi_error}, JDBC: {jdbc_error}")
        else:
            # If DBAPI fails and we prefer it, this is a critical error
            raise IRISConnectionError(f"DBAPI connection failed and fallback disabled: {dbapi_error}")
