    ...
    def _update_cached_response_time(self, response_time: float) -> None:
        ...
        if self.hits > 1:
            self.avg_response_time_cached = (
                (self.avg_response_time_cached * (self.hits - 1) + response_time) / self.hits
            )
        else:
    ...
    def _update_uncached_response_time(self, response_time: float) -> None:
        ...
        if self.misses > 1:
            self.avg_response_time_uncached = (
                (self.avg_response_time_uncached * (self.misses - 1) + response_time) / self.misses
            )
        else:
...
class LangchainIRISCacheWrapper:
    ...
    def _lookup_sync(self, prompt: str, llm_string: str) -> Optional[List['Generation']]:
        """
            ...
            if result:
                ...
                try:
                    ...
                    if not isinstance(generations_data, list):
                        raise TypeError(f"Expected list of generations, got {type(generations_data)}")
                        ...
                        if isinstance(gen_data, dict):
                            try:
                                ...
                                if 'lc' in gen_data and 'kwargs' in gen_data:
                                    ...
                                    logger.warning(f"Loading generation from string fallback: {gen_data['type']}")
                                ...
                                logger.warning(f"Failed to load generation from cache data, skipping: {load_error}")
                            ...
                            logger.warning(f"Expected dict for generation data, got {type(gen_data)}, attempting string conversion")
    ...
    def _update_sync(self, prompt: str, llm_string: str, return_val: List['Generation']) -> None:
        """
    ...
    def update(self, prompt: str, llm_string: str, return_val: Union[str, List['Generation']]) -> None:
        ...
        if isinstance(return_val, list):
            ...
            try:
                serialized_value = self._serialize_generations_for_sync_update(return_val)
            except Exception as e:
