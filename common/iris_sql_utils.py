...
try:
    ...
    logger.warning("Could not import get_iris_dbapi_connection. Self-managed connections will not be available.")
...
class IRISSQLUtils:
    ...
    @staticmethod
    def create_vector_index(connection: Any, table_name: str, column_name: str, 
                           index_name: Optional[str] = None, 
                           metric: str = 'COSINE', 
                           index_type: str = 'HNSW') -> bool:
        """
        ...
        try:
            ...
            connection.commit()
            cursor.close()
            logger.info(f"✓ Created vector index {index_name} on {table_name}.{column_name}")
            return True
    ...
    @staticmethod
    def create_simple_index(connection: Any, table_name: str, column_name: str,
    ...
    def check_table_exists(connection: Any, table_name: str) -> bool:
        ...
        if get_iris_dbapi_connection is None:
            logger.warning("Self-managed connections not available, falling back to provided connection")
            return IRISSQLUtils._check_table_exists_with_connection(connection, table_name)
        ...
        try:
            ...
            if not temp_connection:
                logger.error("Failed to obtain temporary connection for table existence check")
                return False
    ...
    @staticmethod
    def check_index_exists(connection: Any, index_name: str, table_name: Optional[str] = None) -> bool:
        """
        ...
        if get_iris_dbapi_connection is None:
            logger.warning("Self-managed connections not available, falling back to provided connection")
            return IRISSQLUtils._check_index_exists_with_connection(connection, index_name, table_name)
        ...
        try:
            temp_connection = get_iris_dbapi_connection()
            if not temp_connection:
                logger.error("Failed to obtain temporary connection for index existence check")
                return False
    ...
    @staticmethod
    def _check_index_exists_with_connection(connection: Any, index_name: str, table_name: Optional[str] = None) -> bool:
        """
    ...
    def get_table_columns(connection: Any, table_name: str) -> List[Dict[str, Any]]:
        ...
        if get_iris_dbapi_connection is None:
            logger.warning("Self-managed connections not available, falling back to provided connection")
            return IRISSQLUtils._get_table_columns_with_connection(connection, table_name)
        ...
        try:
            temp_connection = get_iris_dbapi_connection()
            if not temp_connection:
                logger.error("Failed to obtain temporary connection for table columns check")
                return []
    ...
    @staticmethod
    def _get_table_columns_with_connection(connection: Any, table_name: str) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def optimize_vector_table(connection: Any, table_name: str, vector_column: str = 'embedding',
                             vector_config: dict = None) -> bool:
        """
    ...
    @staticmethod
    def create_vector_index_with_config(connection: Any, table_name: str, column_name: str,
                                       vector_config: dict, index_name: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            connection.commit()
            cursor.close()
            logger.info(f"✓ Created vector index {index_name} on {table_name}.{column_name} with parameters: {params_str}")
            return True
...
def test_iris_sql_utils():
    ...
    try:
        ...
        index_exists = utils.check_index_exists(None, "RAG_SourceDocuments_embedding_vector_idx", "RAG.SourceDocuments")
        ...
        if get_iris_dbapi_connection:
            ...
            if connection:
                ...
                success = utils.create_vector_index(
                    connection, "RAG.SourceDocuments", "embedding",
                    "test_vector_idx", "COSINE", "HNSW"
                )
