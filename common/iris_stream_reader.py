...
def read_iris_stream(stream_obj: Any) -> str:
    ...
    try:
        ...
        if hasattr(stream_obj, 'readAllBytes') and str(type(stream_obj)).find('java') != -1:
            ...
            try:
                ...
                if isinstance(content_bytes, bytes):
                    ...
                    if str(type(stream_obj)).find('java') != -1:
                        logger.debug(f"JDBC stream.read() returned length ({content_bytes}), not content")
                        return ""
                    else:
                        return str(content_bytes)
            except Exception as e:
        ...
        if hasattr(stream_obj, '__int__') or str(type(stream_obj)).find('JInt') != -1:
            logger.warning(f"Stream object appears to be length ({stream_obj}), not content")
            return ""
