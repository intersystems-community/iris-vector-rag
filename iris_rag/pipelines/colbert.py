...
class ColBERTRAGPipeline(RAGPipeline):
    ...
    def _validate_candidate_relevance(self, candidate_doc_ids: List[int], query_text: str) -> None:
        ...
        try:
            ...
            for i, row in enumerate(results):
                ...
                medical_terms = ['medical', 'health', 'disease', 'treatment', 'patient', 'clinical', 'therapy', 'diagnosis']
                ...
                if query_has_medical and not content_has_medical:
                    logger.warning(f"ColBERT HNSW: Potential relevance issue - medical query but non-medical document: {doc_id}")
    ...
    def _validate_maxsim_scores(self, scores: List[float], query_text: str) -> bool:
        ...
        if len(unique_scores) / len(scores) < (1 - identical_threshold):
            ...
            logger.warning(f"ColBERT Score Validation: This suggests mock encoder issues for query: '{query_text[:50]}...'")
            return False
                ...
                if query_is_medical:
                    ...
                    if not doc_has_medical:
                        ...
                        if not doc_has_research:
                            is_relevant = False
                            filtered_count += 1
                            logger.debug(f"ColBERT Relevance Filter: Filtered out doc {doc_id} - medical query but no medical/research content")
                    ...
                    if not doc_has_tech:
                        is_relevant = False
                        filtered_count += 1
                        logger.debug(f"ColBERT Relevance Filter: Filtered out doc {doc_id} - tech query but no tech content")
                
                if is_relevant:
                    filtered_scores.append((doc_id, score))
            
            logger.info(f"ColBERT Relevance Filter: Filtered out {filtered_count} irrelevant documents, kept {len(filtered_scores)}")
