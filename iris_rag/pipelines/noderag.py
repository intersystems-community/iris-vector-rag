...
class NodeRAGPipeline(RAGPipeline):
    ...
    def _identify_initial_search_nodes(
        self, query_text: str, top_n_seed: int = 5, similarity_threshold: float = 0.1
    ) -> List[str]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"NodeRAG: Identified {len(final_node_ids)} relevant initial search nodes (from {len(results)} candidates)"
            )
    ...
    def _filter_relevant_nodes(self, candidate_results: List[tuple], query_text: str) -> List[str]:
        ...
        for result in candidate_results:
            ...
            if query_is_medical:
                ...
                if has_medical or has_science:
                    ...
                    self.logger.debug(
                        f"NodeRAG Relevance Filter: Medical query - filtered out node {node_id} (no medical/research content)"
                    )
                    filtered_count += 1
                ...
                if has_tech or has_science:
                    ...
                    self.logger.debug(
                        f"NodeRAG Relevance Filter: Tech query - filtered out node {node_id} (no tech/research content)"
                    )
                    filtered_count += 1
                ...
                if has_science or has_medical or has_tech:
                    ...
                    if is_irrelevant:
                        self.logger.debug(
                            f"NodeRAG Relevance Filter: Science query - filtered out irrelevant node {node_id}"
                        )
                        filtered_count += 1
                    else:
                        is_relevant = True
        ...
        self.logger.info(
            f"NodeRAG Relevance Filter: Filtered out {filtered_count} irrelevant nodes, kept {len(filtered_nodes)} relevant nodes"
        )
        return filtered_nodes

    def _traverse_graph(
        self, seed_node_ids: List[str], query_text: str, max_depth: int = 2, max_nodes: int = 20
    ) -> Set[str]:
        """
        ...
        try:
            ...
            for row in results:
                ...
                if content_length < 50:
                    self.logger.warning(
                        f"NodeRAG: Short content ({content_length} chars) for node {node_id}: {page_content[:100]}..."
                    )
                else:
                    self.logger.debug(f"NodeRAG: Good content ({content_length} chars) for node {node_id}")

                documents.append(Document(id=node_id, page_content=page_content, metadata=metadata))
...
Answer:"""
    ...
    def load_documents(self, documents_path: str, **kwargs) -> None:
        ...
        """
        self.logger.info(
            f"NodeRAG: Document loading not implemented. Documents should be pre-loaded in RAG.SourceDocuments table."
        )
