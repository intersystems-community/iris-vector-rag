...
class HybridVectorTextPipeline(BasicRAGPipeline):
    ...
    def query(self, query_text: str, top_k: int = 5, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "execution_time": time.time() - start_time,
                "error": str(e),
                "metadata": {"pipeline_type": "hybrid_vector_text", "search_method": "failed"},
            }
        ...
        try:
            if hasattr(self, "vector_store") and self.vector_store:
                ...
                for doc in retrieved_documents:
                    doc.metadata.update({"search_type": "vector", "pipeline_source": "hybrid_vector_text"})
    ...
    def _fuse_results(self, vector_docs: List[Document], text_docs: List[Document], top_k: int) -> List[Document]:
        ...
        for rank, doc in enumerate(vector_docs):
            ...
            # Update metadata
            doc.metadata.update({"vector_rank": rank + 1, "vector_rank_score": vector_rank_score})
        ...
        for rank, doc in enumerate(text_docs):
            ...
            if doc_id in doc_scores:
                ...
                # Update metadata
                doc.metadata.update({"text_rank": rank + 1, "text_rank_score": text_rank_score})

        # Calculate hybrid scores and sort
        for doc_id, doc_data in doc_scores.items():
            # Combine rank scores with weights
            hybrid_score = (
                self.vector_weight * doc_data["vector_rank_score"] + self.text_weight * doc_data["text_rank_score"]
            )

            # Update document metadata with final scores
            doc_data["document"].metadata.update(
                {"hybrid_score": hybrid_score, "vector_weight": self.vector_weight, "text_weight": self.text_weight}
            )

        # Sort by hybrid score and return top_k documents
        sorted_docs = sorted(
            doc_scores.values(),
            key=lambda x: (self.vector_weight * x["vector_rank_score"] + self.text_weight * x["text_rank_score"]),
            reverse=True,
        )
