...
from iris_rag.memory.models import (
    EnrichedRAGResponse, MemoryConfig, KnowledgePattern, 
    MemoryItem, TemporalWindow, TemporalQuery
)
...
class RAGPipelineWrapper:
    ...
    def __init__(self, base_pipeline: Any, memory_config: MemoryConfig, 
        ...
        self.knowledge_extractor = KnowledgePatternExtractor(memory_config.knowledge_extraction)
        self.temporal_manager = TemporalMemoryManager(
            connection_manager, 
            memory_config.temporal_windows
        )
    ...
    async def query_with_memory(self, query: str, **kwargs) -> EnrichedRAGResponse:
        ...
        try:
            ...
            if self.enhancement_config.enable_pattern_extraction:
                extracted_patterns = self.knowledge_extractor.extract_patterns(base_response)
            ...
            logger.debug(f"RAG query with memory completed: {len(memory_context)} context items, "
    ...
    async def _get_memory_context(self, query: str) -> List[MemoryItem]:
        ...
        try:
            ...
            temporal_context = await self.temporal_manager.retrieve_temporal_context(temporal_query)
    ...
    def _enrich_query_with_context(self, query: str, memory_context: List[MemoryItem]) -> str:
        ...
        if context_snippets:
            ...
            logger.debug(f"Query enriched with {len(context_snippets)} context snippets")
    ...
    async def _execute_base_pipeline(self, query: str, **kwargs) -> Any:
        ...
        if hasattr(self.base_pipeline, 'query'):
            ...
            raise ValueError(f"Cannot determine how to execute pipeline: {type(self.base_pipeline)}")
    
    async def _store_patterns_in_memory(self, patterns: List[KnowledgePattern], source_query: str) -> None:
        ...
        try:
            for pattern in patterns:
                ...
                memory_content = {
                    ...
                    "relationships": [f"{rel.source_entity_id}-{rel.relationship_type}-{rel.target_entity_id}" 
...
class MemoryEnabledRAGPipeline:
    ...
    def __init__(self, base_pipeline: Any, memory_config: MemoryConfig, 
        ...
        self.wrapper = RAGPipelineWrapper(base_pipeline, memory_config, connection_manager)
    ...
    async def learn_from_feedback(self, query: str, response: EnrichedRAGResponse, 
        ...
        try:
            if feedback.get("helpful", False):
                ...
                for pattern in response.extracted_patterns:
                    pattern.extraction_confidence = min(1.0, pattern.extraction_confidence + 0.1)
...
def wrap_existing_pipeline(pipeline: Any, memory_config: MemoryConfig, 
                          connection_manager: ConnectionManager) -> RAGPipelineWrapper:
    """
...
def create_memory_enabled_pipeline(base_pipeline: Any, memory_config: MemoryConfig,
                                  connection_manager: ConnectionManager) -> MemoryEnabledRAGPipeline:
    """
