...
from iris_rag.memory.models import (
    Entity, Relationship, KnowledgePattern, 
    KnowledgeExtractionConfig, MemoryConfig
)
...
class KnowledgePatternExtractor:
    ...
    def __init__(self, config: KnowledgeExtractionConfig):
        ...
        logger.info(f"Knowledge extractor initialized with method: {self.entity_config['method']}")
    ...
    def extract_patterns(self, rag_response: Any) -> List[KnowledgePattern]:
        ...
        try:
            ...
            if processing_time > 50.0:  # 50ms target
                logger.warning(f"Knowledge extraction took {processing_time:.2f}ms (target: <50ms)")
            
            logger.debug(f"Extracted {len(patterns)} patterns with {len(entities)} entities, "
    ...
    def extract_relationships(self, content: str, entities: List[Entity]) -> List[Relationship]:
        ...
        try:
            ...
            confidence_threshold = self.relationship_config.get("confidence_threshold", 0.7)
            
            if method == "dependency_parsing" and self.nlp_processor:
                relationships = self._extract_relationships_dependency(content, entities, max_distance)
            else:
                # Fallback to pattern-based extraction
                relationships = self._extract_relationships_pattern(content, entities)
            
            # Filter by confidence
            relationships = [r for r in relationships if r.confidence_score >= confidence_threshold]
    ...
    def cluster_similar_patterns(self, patterns: List[KnowledgePattern]) -> List[List[KnowledgePattern]]:
        """
        ...
        try:
            ...
            for i, pattern in enumerate(patterns):
                ...
                for j, other_pattern in enumerate(patterns[i+1:], i+1):
                    ...
                    if self._calculate_pattern_similarity(pattern, other_pattern) >= similarity_threshold:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "avg_extraction_time_ms": sum(self._extraction_times) / len(self._extraction_times),
            ...
            "target_met_percentage": len([t for t in self._extraction_times if t <= 50.0]) / len(self._extraction_times) * 100
        }
    ...
    def _extract_content_from_response(self, rag_response: Any) -> str:
        ...
        try:
            ...
            elif hasattr(rag_response, 'retrieved_docs') and rag_response.retrieved_docs:
    ...
    def _extract_relationships_dependency(self, content: str, entities: List[Entity], max_distance: int) -> List[Relationship]:
            ...
            for token in doc:
                if token.dep_ in ["nsubj", "dobj", "pobj"] and token.head.pos_ == "VERB":
                    ...
                    for ent_name, entity in entity_spans.items():
                        if abs(token.i - entity.properties.get("start", 0)) <= max_distance:
                    ...
                    if source_entity and target_entity and source_entity != target_entity:
                        relationship = Relationship(
                            relationship_id=self._generate_relationship_id(source_entity.entity_id, target_entity.entity_id, token.head.lemma_),
    ...
    def _extract_relationships_pattern(self, content: str, entities: List[Entity]) -> List[Relationship]:
        """Extract relationships using pattern matching (fallback)."""
        relationships = []
        
        # Simple pattern: Entity1 [verb] Entity2
        verb_patterns = ["is", "was", "has", "contains", "includes", "causes", "affects"]
            ...
            for entity2 in entities[i+1:]:
                for verb in verb_patterns:
                    ...
                    if re.search(pattern, content, re.IGNORECASE):
                        relationship = Relationship(
                            relationship_id=self._generate_relationship_id(entity1.entity_id, entity2.entity_id, verb),
    ...
    def _calculate_pattern_similarity(self, pattern1: KnowledgePattern, pattern2: KnowledgePattern) -> float:
        ...
        entity_similarity = len(entities1 & entities2) / len(entities1 | entities2) if entities1 or entities2 else 0.0
        ...
        context_similarity = len(context1_words & context2_words) / len(context1_words | context2_words) if context1_words or context2_words else 0.0
    ...
    def _generate_entity_id(self, name: str, entity_type: str) -> str:
        ...
        entity_hash = hashlib.sha256(f"{name.lower()}:{entity_type}".encode()).hexdigest()
    ...
    def _generate_relationship_id(self, source_id: str, target_id: str, rel_type: str) -> str:
        ...
        rel_hash = hashlib.sha256(f"{source_id}:{target_id}:{rel_type}".encode()).hexdigest()
        return f"rel_{rel_hash[:16]}"
    
    def _determine_pattern_type(self, entities: List[Entity], relationships: List[Relationship]) -> str:
