...
from iris_rag.memory.models import (
    MemoryItem, TemporalWindow, TemporalWindowConfig, 
...
class TemporalMemoryManager:
    ...
    def __init__(self, connection_manager: ConnectionManager, window_configs: List[TemporalWindowConfig]):
        """
        ...
        logger.info(f"Temporal memory manager initialized with {len(window_configs)} windows")
    
    async def store_with_window(self, content: Any, window: TemporalWindow, 
                               context: Dict[str, Any] = None, 
                               source_query: str = "") -> str:
        """
        ...
        try:
            ...
            logger.debug(f"Stored memory item {item_id} in {window.value} window ({processing_time:.2f}ms)")
            return item_id
    ...
    async def retrieve_temporal_context(self, query: TemporalQuery) -> TemporalContext:
        ...
        try:
            ...
            context = TemporalContext(
                ...
                avg_relevance_score=sum(m.relevance_score for m in relevant_memories) / len(relevant_memories) if relevant_memories else 0.0,
            ...
            if processing_time > 100.0:  # 100ms target
                logger.warning(f"Temporal retrieval took {processing_time:.2f}ms (target: <100ms)")
            
            logger.debug(f"Retrieved {len(relevant_memories)} relevant memories from {query.window.value} "
    ...
    async def cleanup_expired_memories(self, window: Optional[TemporalWindow] = None) -> int:
        """
        ...
        try:
            ...
            for temp_window in windows_to_clean:
                ...
                cleanup_threshold = datetime.utcnow() - timedelta(days=window_config.duration_days)
                
                # Perform cleanup based on retention policy
                if window_config.retention_policy == "expire":
                    count = await self._delete_expired_memories(temp_window, cleanup_threshold)
                elif window_config.retention_policy == "archive":
                    count = await self._archive_expired_memories(temp_window, cleanup_threshold)
                else:  # keep
                    continue
    ...
    async def get_window_statistics(self, window: TemporalWindow) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                hourly_distribution = {str(hour): count for hour, count in cursor.fetchall()}
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "retrieval_performance": {
                "avg_time_ms": sum(self._retrieval_times) / len(self._retrieval_times) if self._retrieval_times else 0,
                "max_time_ms": max(self._retrieval_times) if self._retrieval_times else 0,
                "total_retrievals": len(self._retrieval_times),
                "target_met_percentage": len([t for t in self._retrieval_times if t <= 100.0]) / len(self._retrieval_times) * 100 if self._retrieval_times else 0
            },
            "storage_performance": {
                "avg_time_ms": sum(self._storage_times) / len(self._storage_times) if self._storage_times else 0,
    ...
    def _get_window_config(self, window: TemporalWindow) -> Optional[TemporalWindowConfig]:
        """Get configuration for temporal window."""
        return self.window_configs.get(window.value)
    
    def _calculate_window_start(self, window: TemporalWindow, end_time: datetime) -> datetime:
    ...
    async def _store_memory_item(self, memory_item: MemoryItem, expiration_time: datetime) -> None:
    ...
    async def _retrieve_memories_in_window(self, query: TemporalQuery, 
                                         window_start: datetime, window_end: datetime) -> List[MemoryItem]:
    ...
    def _filter_by_relevance(self, memories: List[MemoryItem], query: TemporalQuery) -> List[MemoryItem]:
        ...
        for memory in memories:
            ...
            content_overlap = len(query_words & content_words) / len(query_words | content_words) if query_words or content_words else 0
            source_overlap = len(query_words & source_words) / len(query_words | source_words) if query_words or source_words else 0
            
            # Combine relevance scores
            memory.relevance_score = (content_overlap + source_overlap) / 2.0
        
        # Filter by threshold and sort by relevance
        relevant_memories = [m for m in memories if m.relevance_score >= query.relevance_threshold]
        relevant_memories.sort(key=lambda m: m.relevance_score, reverse=True)
        
        return relevant_memories
    
    async def _delete_expired_memories(self, window: TemporalWindow, threshold: datetime) -> int:
    ...
    async def _archive_expired_memories(self, window: TemporalWindow, threshold: datetime) -> int:
    ...
    def _generate_memory_id(self, content: Any, source_query: str, window: TemporalWindow) -> str:
