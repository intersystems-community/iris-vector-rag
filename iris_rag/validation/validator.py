...
class PreConditionValidator:
    ...
    def validate_pipeline_requirements(self, requirements: PipelineRequirements) -> ValidationReport:
        ...
        required_table_valid = all(
            result.is_valid for name, result in table_validations.items()
            if not name.endswith("_optional")
        )
        required_embedding_valid = all(
            result.is_valid for name, result in embedding_validations.items()
            if not name.endswith("_optional")
        )
    ...
    def _generate_summary(self, overall_valid: bool, 
                         table_validations: Dict[str, ValidationResult],
                         embedding_validations: Dict[str, ValidationResult]) -> str:
    ...
    def _generate_setup_suggestions(self, requirements: PipelineRequirements,
                                   table_validations: Dict[str, ValidationResult],
                                   embedding_validations: Dict[str, ValidationResult]) -> List[str]:
