...
class EmbeddingValidator:
    ...
    def __init__(self, config_manager: ConfigurationManager):
        ...
        self.logger.debug(
            f"EmbeddingValidator initialized with diversity_threshold={self.diversity_threshold}, "
    ...
    def analyze_quality(self, sample_embeddings: List[Tuple[str, str]]) -> EmbeddingQualityIssues:
        ...
        if not sample_embeddings:
            return EmbeddingQualityIssues(total_analyzed=0, issues_summary=["No embeddings provided for analysis"])
        ...
        if len(parsed_embeddings) > 1 and self.mock_detection_enabled:
            ...
            if cross_similarity > self.mock_detection_threshold:
                ...
                issues.append(
                    f"High cross-embedding similarity detected ({cross_similarity:.3f}), indicating potential mock data"
                )
        ...
        self.logger.info(
            f"Embedding quality analysis complete: {len(sample_embeddings)} analyzed, "
            f"mock_detected={mock_embeddings_detected}, avg_diversity={avg_diversity_score:.3f}"
        )

        return result

    def sample_embeddings_from_database(
        self, table_name: str = "RAG.DocumentTokenEmbeddings", sample_size: int = 100
    ) -> List[Tuple[str, str]]:
        """
    ...
    def _calculate_embedding_diversity(self, embedding: np.ndarray) -> float:
        ...
        combined_diversity = (
            0.5 * diversity_score + 0.3 * unique_ratio + 0.2 * range_diversity  # Variance-based  # Uniqueness  # Range
        )
