...
class ColBERTInterface(ABC):
    ...
    @abstractmethod
    def calculate_maxsim(self, query_embeddings: List[List[float]], doc_embeddings: List[List[float]]) -> float:
        """
    ...
    def _ensure_model_loaded(self):
        ...
        if self._model is None:
            try:
                ...
                self._tokenizer, self._model = download_huggingface_model(self.model_name, trust_remote_code=True)
...
def get_colbert_interface_from_config(config_manager, connection_manager=None) -> ColBERTInterface:
    ...
    if connection_manager:
        try:
            ...
            logger.info(
                f"Using schema manager ColBERT config: {backend} backend, {token_dimension}D, model {model_name}"
            )
