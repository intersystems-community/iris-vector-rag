...
@click.option(
    "--interval", "-i", default=3600, type=int, help="Reconciliation interval in seconds (default: 3600 = 1 hour)"
)
@click.option("--max-iterations", default=0, type=int, help="Maximum iterations (0 = infinite)")
@click.pass_context
def daemon(ctx, pipeline, interval, max_iterations):
    ...
    try:
        ...
        controller.run_continuous_reconciliation(
            pipeline_type=pipeline, interval_seconds=interval, max_iterations=max_iterations
        )
