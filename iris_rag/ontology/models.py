...
class Concept:
    ...
    def has_ancestor(self, concept_hierarchy: 'ConceptHierarchy', ancestor_id: str) -> bool:
...
class ConceptHierarchy:
    ...
    def add_relationship(self, relationship: OntologyRelationship) -> None:
        ...
        if relationship.is_hierarchical():
            source_concept = self.concepts.get(relationship.source_concept_id)
            target_concept = self.concepts.get(relationship.target_concept_id)
            
            if source_concept and target_concept:
                if relationship.relation_type == RelationType.IS_A:
                    source_concept.add_parent(relationship.target_concept_id)
                    target_concept.add_child(relationship.source_concept_id)
                    self.root_concepts.discard(relationship.source_concept_id)
...
class InferenceRule:
    ...
    def _matches_condition(self, condition: Dict[str, Any], facts: Dict[str, Any]) -> bool:
...
class OntologyMetadata:
    ...
    def _calculate_depth(self, hierarchy: ConceptHierarchy, concept_id: str, visited: Set[str] = None) -> int:
