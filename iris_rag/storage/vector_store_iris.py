...
class IRISVectorStore(VectorStore):
    ...
    def _ensure_table_exists(self, cursor):
        ...
        try:
            ...
            try:
                ...
                table_short_name = self.table_name.replace("RAG.", "")
                expected_config = {"vector_dimension": self.vector_dimension, "vector_data_type": "FLOAT"}
    ...
    def _validate_table_name(self, table_name: str) -> None:
        ...
        for pattern in dangerous_patterns:
            if pattern in table_lower:
                ...
                raise VectorStoreConfigurationError(f"Table name contains restricted pattern: {pattern}")
    ...
    def _chunk_document(self, document: Document, chunking_strategy: Optional[str] = None) -> List[Document]:
        ...
        try:
            ...
            chunk_records = self.chunking_service.chunk_document(document.id, document.page_content, strategy_name)
    ...
    def similarity_search_by_embedding(
        ...
        try:
            ...
            for row in rows:
                ...
                document_data = {"doc_id": doc_id, "text_content": text_content, "metadata": metadata_json}
        ...
        try:
            ...
            try:
                ...
                if table_short_name == "SourceDocuments":
                    ...
                    success = self.schema_manager._migrate_document_token_embeddings_table(
                        cursor, expected_config, preserve_data=False
                    )
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        for i, text in enumerate(texts):
            ...
            doc = Document(id=doc_id, page_content=text, metadata=metadata)
    ...
    def search(self, query_vector: List[float], top_k: int = 5, **kwargs: Any) -> List[Tuple[Document, float]]:
        """
        ...
        return self.similarity_search_by_embedding(
            query_embedding=query_vector, top_k=top_k, filter=kwargs.get("filter")
        )
    ...
    def similarity_search_by_vector(
        ...
        return self.similarity_search_by_embedding(query_embedding=embedding, top_k=k, filter=filter)
    ...
    def colbert_search(
        self, query_token_embeddings: List[List[float]], k: int = 5, **kwargs: Any
    ) -> List[Tuple[Document, float]]:
        """
        ...
        logger.info(
            f"ColBERT: Processed {len(candidates)} candidates, calculated MaxSim for {len(maxsim_results)}, returning top {len(enhanced_results)}"
        )
        return enhanced_results
    ...
    def hybrid_search(
        ...
        **kwargs: Any,
        ...
        vector_results = self.similarity_search_by_embedding(
            query_embedding=query_embedding, top_k=k * 2, filter=kwargs.get("filter")  # Get more for potential fusion
        )
    ...
    def graph_search(self, query_entities: List[str], k: int = 5, **kwargs: Any) -> List[Tuple[Document, float]]:
        """
