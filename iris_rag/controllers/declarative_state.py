...
class DeclarativeStateManager:
    ...
    def declare_state(self, 
                     spec: Union[DeclarativeStateSpec, Dict, str]) -> 'DeclarativeStateManager':
        """
        ...
        logger.info(f"Declared state: {self._current_spec.document_count} documents, "
    ...
    def sync(self, force: bool = False, dry_run: bool = False) -> ReconciliationResult:
        ...
        if dry_run:
            ...
            drift = self.reconciliation_controller.drift_analyzer.analyze_drift(
        ...
        return self.reconciliation_controller.reconcile(
            pipeline_type=self._current_spec.pipeline_type,
            force=force
        )
    ...
    def ensure_state(self, 
                    spec: Union[DeclarativeStateSpec, Dict, str],
                    timeout: int = 300) -> ReconciliationResult:
        """
...
def ensure_documents(count: int, 
                    source: Optional[str] = None,
                    pipeline: str = "basic") -> ReconciliationResult:
    """
...
def create_test_state(doc_count: int = 10, 
                     pipeline: str = "basic") -> DeclarativeStateSpec:
    """
