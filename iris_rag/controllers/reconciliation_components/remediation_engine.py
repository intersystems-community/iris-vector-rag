...
class RemediationEngine:
    ...
    def remediate_drift(
        self, drift_analysis: DriftAnalysis, desired_state: DesiredState, current_state: SystemState
    ...
    def _clear_and_regenerate_embeddings(
        self, affected_doc_ids: List[str], desired_state: DesiredState = None
    ) -> ReconciliationAction:
        """
    ...
    def _regenerate_low_diversity_embeddings(
        self, affected_doc_ids: List[str], desired_state: DesiredState = None
    ) -> ReconciliationAction:
        """
    ...
    def _get_document_text_content(self, doc_id: str, cursor) -> Optional[str]:
        ...
        try:
            ...
            if hasattr(text_content_raw, "read") and callable(text_content_raw.read):
                logger.debug(
                    f"Detected stream-like object for text_content for doc {doc_id} (type: {type(text_content_raw)}), attempting to read fully."
                )
                char_list = []
                try:
                    while True:
                    ...
                    logger.debug(
                        f"Fully read stream for doc {doc_id}. Content length: {len(text_content)}. Starts: {text_content[:200]}..."
                    )
                ...
                logger.debug(
                    f"text_content for doc {doc_id} was not a stream, used as string. Content: {text_content[:200]}..."
                )
            else:
    ...
    def _remediate_single_document_embeddings(
        self, doc_id: str, text_content: str, embedding_func, target_dimension: int = None
    ) -> bool:
        """
        ...
        try:
            ...
            if target_dimension is None:
                target_dimension = (
                    len(raw_embeddings[0]) if raw_embeddings and raw_embeddings[0] else 384
                )  # Default dimension

            return self.document_service.save_document_embeddings(
                doc_id=doc_id, tokens=tokens, embeddings=raw_embeddings, target_dimension=target_dimension
            )
