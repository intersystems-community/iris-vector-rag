...
class DaemonController:
    ...
    def __init__(self, reconciliation_controller: "ReconciliationController", config_manager):
        ...
        logger.info(
            f"Default interval: {self.default_interval_seconds}s, Error retry: {self.error_retry_interval_seconds}s"
        )
    ...
    def _daemon_loop(self, interval: int, error_retry_interval: int) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if not self._stop_event.is_set() and (
                self.max_iterations == 0 or self.current_iteration < self.max_iterations
            ):
