...
class DocumentService:
    ...
    def get_document_ids_by_source(self, source_uri: str) -> List[int]:
        ...
        try:
            ...
            cursor.execute("SELECT doc_id FROM RAG.SourceDocuments WHERE source_uri = ?", [source_uri])
    ...
    def get_chunk_ids_for_document(self, doc_id: int) -> List[int]:
        ...
        try:
            ...
            cursor.execute("SELECT id FROM RAG.DocumentChunks WHERE doc_id = ?", [doc_id])
    ...
    def get_embedding_ids_for_chunk(self, chunk_id: int) -> List[int]:
        ...
        try:
            ...
            cursor.execute("SELECT id FROM RAG.DocumentTokenEmbeddings WHERE chunk_id = ?", [chunk_id])
    ...
    def delete_embeddings_by_ids(self, embedding_ids: List[int]) -> int:
        ...
        try:
            ...
            cursor.execute(f"DELETE FROM RAG.DocumentTokenEmbeddings WHERE id IN ({placeholders})", embedding_ids)
    ...
    def delete_chunks_by_ids(self, chunk_ids: List[int]) -> int:
        ...
        try:
            ...
            cursor.execute(f"DELETE FROM RAG.DocumentChunks WHERE id IN ({placeholders})", chunk_ids)
    ...
    def delete_documents_by_ids(self, doc_ids: List[int]) -> int:
        ...
        try:
            ...
            cursor.execute(f"DELETE FROM RAG.SourceDocuments WHERE doc_id IN ({placeholders})", doc_ids)
    ...
    def get_documents_with_mock_embeddings(self) -> List[str]:
        ...
        try:
            ...
            sample_embeddings = self.embedding_validator.sample_embeddings_from_database(
                table_name="RAG.DocumentTokenEmbeddings", sample_size=200
            )
    ...
    def save_document_embeddings(
        self, doc_id: str, tokens: List[str], embeddings: List[List[float]], target_dimension: int
    ) -> bool:
        """
        ...
        try:
            ...
            for token_index, (token_text, embedding_vector) in enumerate(zip(tokens, embeddings)):
                if not isinstance(embedding_vector, list):
                    logger.error(
                        f"Embedding for token '{token_text}' in doc '{doc_id}' is not a list: {type(embedding_vector)}. Skipping."
                    )
                    continue
