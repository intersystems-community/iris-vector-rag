...
class StateObserver:
    ...
    def observe_current_state(self) -> SystemState:
        ...
        try:
            ...
            sample_embeddings = self.embedding_validator.sample_embeddings_from_database(
                table_name="RAG.DocumentTokenEmbeddings", sample_size=50
            )
            ...
            logger.info(
                f"Current state observed: {total_documents} documents, " f"{total_token_embeddings} token embeddings"
            )
    ...
    def get_desired_state(self, pipeline_type: str = "colbert") -> DesiredState:
        ...
        require_token_embeddings = completeness_config.get(
            "require_token_embeddings", pipeline_type.lower() == "colbert"
        )
        ...
        logger.info(
            f"Desired state for {pipeline_type}: {target_document_count} documents, "
