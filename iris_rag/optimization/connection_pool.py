...
class OptimizedConnectionPool:
    ...
    def __init__(
        ...
        logger.info(f"Optimized connection pool initialized: {self.min_connections}-{self.max_connections} connections")
    ...
    def _create_connection(self) -> Optional[PooledConnection]:
        ...
        try:
            raw_connection = self.base_connection_manager.get_connection(self.backend_name)
    ...
    def acquire_connection(self) -> Optional[PooledConnection]:
        ...
        try:
            ...
            logger.warning(f"Connection pool timeout after {time.time() - start_time:.2f}s")
            return None
    ...
    def release_connection(self, connection: PooledConnection) -> None:
        ...
        try:
            ...
            if (connection.metrics.age_seconds > self.max_connection_age or 
                connection.metrics.errors > 10 or
                not connection.is_healthy()):
                
                logger.debug(f"Retiring connection {connection.connection_id} (age: {connection.metrics.age_seconds:.0f}s, errors: {connection.metrics.errors})")
                self._remove_connection(connection)
    ...
    def _cleanup_old_connections(self) -> None:
        ...
        with self._pool_lock:
            for conn in self._pool[:]:  # Copy to avoid modification during iteration
                if conn.metrics.age_seconds > self.max_connection_age and not conn.in_use:
        ...
        if old_connections:
            logger.info(f"Cleanup completed: removed {len(old_connections)} aged connections")
    ...
    def warm_pool(self) -> Dict[str, Any]:
        ...
        elapsed = time.time() - start_time
        logger.info(f"Pool warming completed: created {created_count} connections in {elapsed:.2f}s")
