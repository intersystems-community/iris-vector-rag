...
def _chunk_pmc_content(
    content: str, pmc_id: str, chunk_size: int = 8000, overlap: int = 400
) -> List[Dict[str, Any]]:
    """
        ...
        return [
            {
                ...
                "start_pos": 0,
                "end_pos": len(content),
                "metadata": {"is_complete_doc": True, "chunk_size": len(content)},
            }
        ]
            ...
            chunks.append(
                {
                    ...
                    "metadata": {
                        ...
                        "overlap_with_previous": (
                            min(overlap, start) if start > 0 else 0
                        ),
...
def process_pmc_files(
    ...
    for root, dirs, files in os.walk(directory):
        logger.info(
            f"Walking directory: {root}, found {len(dirs)} dirs, {len(files)} files."
        )
        for file in files:
            if file.endswith(".xml"):  # Process all XMLs, then check limit
                ...
                if processed >= limit:
                    logger.info(
                        f"Limit of {limit} files reached. Stopping further processing."
                    )
                ...
                try:
                    ...
                    if metadata.get(
                        "title"
                    ) == "Error" and "Failed to process" in metadata.get(
                        "abstract", ""
                    ):
                        logger.warning(
                            f"Metadata extraction indicated an error for {file_path}. Skipping yield."
                        )
                        ...
                        processed += 1
                        logger.debug(
                            f"Successfully processed and yielded metadata for: {file_path} (Total processed: {processed})"
                        )
                        if (
                            processed % 100 == 0 and processed > 0
                        ):  # Ensure processed > 0 for division
                            elapsed = time.time() - start_time
                            if elapsed > 0:  # Avoid division by zero
                                logger.info(
                                    f"Processed {processed} files in {elapsed:.2f}s ({processed/elapsed:.2f} files/s)"
                                )
                            else:
                                logger.info(
                                    f"Processed {processed} files very quickly."
                                )
                except Exception as e:
    ...
    if processed < limit:
        logger.info(
            f"Finished walking all directories. Processed {processed} files (limit was {limit})."
        )


class PMCProcessor:
    """
    Thin wrapper around extract_pmc_metadata/process_pmc_files to maintain legacy import path.
    Provides process_file() and process_directory() methods expected by ingestion scripts.
    """
    ...
    def process_file(self, xml_file_path: str) -> Dict[str, Any]:
        return extract_pmc_metadata(xml_file_path)

    def process_directory(
        self, directory: str, limit: int = 1000
    ) -> Generator[Dict[str, Any], None, None]:
        yield from process_pmc_files(directory, limit)
...
if __name__ == "__main__":
    ...
    for i, doc in enumerate(process_pmc_files(pmc_dir, limit)):
        ...
        print(
            f"  Authors: {', '.join(doc['authors'][:3])}{' and more' if len(doc['authors']) > 3 else ''}"
        )
        print(
            f"  Keywords: {', '.join(doc['keywords'][:5])}{' and more' if len(doc['keywords']) > 5 else ''}"
        )
        print()
