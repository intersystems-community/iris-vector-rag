...
- Default cross-encoder: `cross-encoder/ms-marco-MiniLM-L-6-v2` — available via `rerank=True` on any pipeline; loaded once per process via `resolve_reranker()`
- `hf_reranker`: HuggingFace cross-encoder (legacy alias)
...
### Step 3: Query-Time Integration (Recommended)

Pass your reranker as the `rerank=` argument at query time — no registration or subclassing needed:
...
# Pass any callable — receives (query: str, docs: List[Document]) → List[Document]
result = pipeline.query(
    "What is machine learning?",
    top_k=10,
    rerank=my_custom_reranker,
)
```

Callables passed via `rerank=` are **never cached** — a new call is made each time. To
cache the model yourself, build it once and close over it:

```python
from iris_vector_rag.retrieval.rerank import resolve_reranker

# Resolve and cache via the built-in mechanism
fn = resolve_reranker(True)  # cross-encoder, loaded once

result = pipeline.query("What is machine learning?", top_k=10, rerank=fn)
```

#### Legacy: Factory/Registry Integration

If you need a named pipeline type (e.g. to expose a variant via `create_pipeline("my_reranker")`),
the `PipelineFactory` / `PipelineRegistry` pattern still works but requires subclassing:

```python
# Usage example (legacy pattern)
...
Prefer the `rerank=callable` query-time pattern for new code.
