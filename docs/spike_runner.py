...
def q1_schema():
    ...
    record(
        "q1_schema_pattern",
        "per-column" if any("source" in c["name"].lower() for c in col_info) else "json-blob",
    )
...
def q3_performance():
    ...
    def random_text(n=100):
        words = ["chest", "pain", "fever", "radiology", "lab", "patient", "diagnosis",
                 "treatment", "medication", "blood", "pressure", "heart", "lung", "kidney"]
    ...
    docs_1k = [
        Document(
            ...
            metadata={"category": random.choice(["radiology", "lab", "notes"]), "doc_id": i},
        )
        for i in range(200)
    ]
    ...
    t0 = time.time()
    for _ in range(N):
        vs.similarity_search("chest X-ray findings", k=5, filter={"category": "radiology"})
    filtered_ms = (time.time() - t0) / N * 1000
    record("q3_filtered_query_p50_ms", round(filtered_ms, 1))
    record("q3_filter_overhead_pct", round((filtered_ms - unfiltered_ms) / unfiltered_ms * 100, 1))
...
def q4_replace_collection():
    ...
    docs = [Document(page_content=f"Document {i}", metadata={"idx": i}) for i in range(5)]
    ...
    try:
        ...
        record("q4_no_replace_behavior", "APPEND" if count3 == 4 else f"unexpected={count3}")
    except Exception as e:
...
def q5_mmr():
    ...
    docs = [Document(page_content=f"Medical document {i} about chest conditions.", metadata={"i": i})
            for i in range(10)]
    vs.add_documents(docs)
...
def q6_async():
    ...
    if has_aadd:
        try:
            asyncio.run(vs.aadd_documents([Document(page_content="async add test", metadata={})]))
...
def q7_embeddings():
    ...
    for provider_name, model_name in providers_to_test:
        try:
            ...
            docs = [Document(page_content="Embedding provider test document.", metadata={})]
            vs.add_documents(docs)
            r = vs.similarity_search("test", k=1)
            results_by_provider[provider_name] = f"SUCCESS (model={model_name}, results={len(r)})"
        except Exception as e:
...
def q8_langchain_iris_comparison():
    ...
    try:
        ...
        has_predicate = hasattr(langchain_iris, "Predicate") or any(
            "predicate" in str(m).lower()
            for m in dir(langchain_iris)
        )
        record("q8_langchain_iris_has_predicate_system", has_predicate)

        # Check similarity metrics
        has_similarity_metric = any("metric" in str(m).lower() for m in dir(langchain_iris))
    ...
    record("q8_langchain_intersystems_predicates", [
        p for p in dir(Predicate) if not p.startswith("_")
    ])
    record("q8_langchain_intersystems_similarity_metrics", [
    ...
    try:
        ...
        record("q8_langchain_intersystems_connection", "DB-API" if uses_dbapi else "Native API" if uses_native else "unknown")
    except Exception:
...
if __name__ == "__main__":
    ...
    print(f"OpenAI key: {'SET' if os.environ.get('OPENAI_API_KEY') else 'NOT SET (using HuggingFace)'}")

    if not wait_for_iris():
        sys.exit(1)

    errors = []
    for fn in [q1_schema, q2_fhir_metadata, q3_performance, q4_replace_collection,
               q5_mmr, q6_async, q7_embeddings, q8_langchain_iris_comparison]:
        try:
            fn()
        except Exception as e:
            import traceback
