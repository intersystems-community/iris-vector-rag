...
# Equivalent using any pipeline with the rerank= option (added in 065):
...
│  ├─ Yes, and speed is OK? → basic (with rerank=True) or basic_rerank
...
> **Tip**: Since feature 065, `rerank=True`, `retrieval="hybrid"`, and `metadata_filter=`
> work on **any** pipeline at query time — you often don't need to switch pipeline types
> to get reranking or hybrid retrieval. `basic_rerank` remains available and is equivalent
> to `basic` with `rerank=True` as its default.
