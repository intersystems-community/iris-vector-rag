    ...
    def retrieve_documents(self, query: str, hypothetical_doc: str, top_k: int = 5, similarity_threshold: float = 0.0) -> List[Document]:
        ...
        try:
            ...
            for row in filtered_results:
                doc_id = row[0]
                title = row[1] or ""
                raw_content = row[2] # This is the IRISInputStream or None
                similarity_db = row[3] # Original value from DB
                ...
                if raw_content:
                    if hasattr(raw_content, 'read'):
                        try:
                            ...
                            # Ensure content_bytes is actually bytes before decoding
                            if isinstance(content_bytes, bytes) and content_bytes:
                                content_str = content_bytes.decode('utf-8')
                            elif content_bytes: # If not bytes but still truthy (e.g. JInt representing length > 0)
                                logger.warning(f"raw_content.read() returned non-bytes type for doc_id {doc_id}: {type(content_bytes)}. Content may be missing or was small.")
                                # content_str remains ""
                        except Exception as e:
                            logger.error(f"Error reading/decoding content stream for doc_id {doc_id}: {e}")
                    elif isinstance(raw_content, str): # Fallback if it's already a string
                        content_str = raw_content
                
                # Ensure score is float
                similarity_score = float(similarity_db) if similarity_db is not None else 0.0
                
                doc = Document(
                    id=doc_id,
                    content=content_str, # Pass the read and decoded string content
                    score=similarity_score
                )
                # Store metadata separately for later use
                doc._metadata = {
                    "title": title,
                    "similarity_score": similarity_score, # Use the processed float score
                    "source": "HyDE_V2_HNSW"
                }
