...
class JDBCFixedGraphRAGPipeline:
        ...
        try:
            ...
            for depth in range(max_depth):
                ...
                if limit_val > 0:
                    ...
                    connected_entities = []
                # The lines below were redundant and incorrectly placed/parameterized.
                # cursor.execute(traversal_query, entity_list + entity_list + entity_list + entity_list)
                # connected_entities = cursor.fetchall()
                
                # Update for next iteration (uses 'connected_entities' from either the if or else block above)
                next_entities = set()
    ...
    def _get_documents_from_entities(self, entity_ids: Set[str], top_k: int = 20) -> List[Document]:
        ...
        try:
            ...
            # Construct the query string ensuring top_k is a literal
            # This approach might still be problematic if the driver tries to parse {top_k} before parameter binding.
            # A safer way is to format the TOP N part of the string completely before the rest of the f-string.
            # Ensure top_k is an integer literal in the SQL string
            doc_query = "SELECT TOP %d DISTINCT e.source_doc_id, sd.text_content \n" % int(top_k) + \
                        f"FROM RAG.Entities e\n" + \
                        f"JOIN RAG.SourceDocuments sd ON e.source_doc_id = sd.doc_id\n" + \
    ...
    def _fallback_vector_search(self, query_text: str, top_k: int = 20) -> List[Document]:
        ...
        try:
            ...
            results = cursor.fetchall() # Changed all_results to results to match loop
            
            # No pre-filtering by score here, rely on TOP and ORDER BY
            for doc_id, content, score in results: # Changed filtered_results to results
                # Handle JDBC stream objects
                content_str = read_iris_stream(content)
