    ...
    def generate_answer(self, query: str, documents: List[Document]) -> str:
        ...
        for i, doc in enumerate(documents, 1):
            ...
            if content_lines[0].startswith('Title: '):
                title = content_lines[0][7:]
                content = '\n'.join(content_lines[2:])  # Skip title and empty line
            else:
                title = 'Untitled'
                content = doc.content
            
            context_parts.append(f"Document {i} (Title: {title}, Score: {doc.score:.3f}):\n{content[:1000]}...")
...
Please provide a comprehensive answer based on the information provided. If the information is not sufficient to answer the question, please state that clearly."""
    ...
    def run(self, query: str, top_k: int = 5, similarity_threshold: float = 0.1) -> Dict[str, Any]:
        ...
        retrieval_method = "vector_search_hnsw"
        if documents and hasattr(documents[0], 'content'):
            # Check if we used fallback (manual similarity)
            if len(documents) > 0 and documents[0].score < 1.0: # Fallback scores are typically < 1.0
                retrieval_method = "manual_similarity"
